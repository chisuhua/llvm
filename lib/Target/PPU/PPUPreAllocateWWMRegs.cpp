//===- PPUPreAllocateWWMRegs.cpp - WWM Register Pre-allocation -------------===//
//
...
#define DEBUG_TYPE "ppu-pre-allocate-wwm-regs"

namespace {

class PPUPreAllocateWWMRegs : public MachineFunctionPass {
private:
...
public:
  ...
  PPUPreAllocateWWMRegs() : MachineFunctionPass(ID) {
    initializePPUPreAllocateWWMRegsPass(*PassRegistry::getPassRegistry());
  }
...
INITIALIZE_PASS_BEGIN(PPUPreAllocateWWMRegs, DEBUG_TYPE,
                "PPU Pre-allocate WWM Registers", false, false)
INITIALIZE_PASS_DEPENDENCY(LiveIntervals)
INITIALIZE_PASS_DEPENDENCY(VirtRegMap)
INITIALIZE_PASS_DEPENDENCY(LiveRegMatrix)
INITIALIZE_PASS_END(PPUPreAllocateWWMRegs, DEBUG_TYPE,
                "PPU Pre-allocate WWM Registers", false, false)

char PPUPreAllocateWWMRegs::ID = 0;

char &llvm::PPUPreAllocateWWMRegsID = PPUPreAllocateWWMRegs::ID;

FunctionPass *llvm::createPPUPreAllocateWWMRegsPass() {
  return new PPUPreAllocateWWMRegs();
}

bool PPUPreAllocateWWMRegs::processDef(MachineOperand &MO) {
  if (!MO.isReg())
    return false;
...
void PPUPreAllocateWWMRegs::rewriteRegs(MachineFunction &MF) {
...
bool PPUPreAllocateWWMRegs::runOnMachineFunction(MachineFunction &MF) {
  LLVM_DEBUG(dbgs() << "PPUPreAllocateWWMRegs: function " << MF.getName() << "\n");
  ...
  for (MachineBasicBlock *MBB : RPOT) {
    ...
    for (MachineInstr &MI : *MBB) {
        /* FIXME
      if (MI.getOpcode() == PPU::V_SET_INACTIVE_B32 ||
          MI.getOpcode() == PPU::V_SET_INACTIVE_B64)
        RegsAssigned |= processDef(MI.getOperand(0));

      if (MI.getOpcode() == PPU::ENTER_WWM) {
        LLVM_DEBUG(dbgs() << "entering WWM region: " << MI << "\n");
        InWWM = true;
        continue;
      }

      if (MI.getOpcode() == PPU::EXIT_WWM) {
