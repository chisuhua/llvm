//===- PPURewriteOutArgumentsPass.cpp - Create struct returns ----------===//
//
...
#define DEBUG_TYPE "ppu-rewrite-out-arguments"

using namespace llvm;

static cl::opt<bool> AnyAddressSpace(
  "ppu-any-address-space-out-arguments",
...
class PPURewriteOutArguments : public FunctionPass {
private:
...
public:
  ...
  PPURewriteOutArguments() : FunctionPass(ID) {}
...
INITIALIZE_PASS_BEGIN(PPURewriteOutArguments, DEBUG_TYPE,
                      "PPU Rewrite Out Arguments", false, false)
INITIALIZE_PASS_DEPENDENCY(MemoryDependenceWrapperPass)
INITIALIZE_PASS_END(PPURewriteOutArguments, DEBUG_TYPE,
                    "PPU Rewrite Out Arguments", false, false)

char PPURewriteOutArguments::ID = 0;

bool PPURewriteOutArguments::checkArgumentUses(Value &Arg) const {
...
bool PPURewriteOutArguments::isOutArgumentCandidate(Argument &Arg) const {
...
bool PPURewriteOutArguments::doInitialization(Module &M) {
  DL = &M.getDataLayout();
  return false;
}

#ifndef NDEBUG
bool PPURewriteOutArguments::isVec3ToVec4Shuffle(Type *Ty0, Type* Ty1) const {
...
bool PPURewriteOutArguments::runOnFunction(Function &F) {
  if (skipFunction(F))
    return false;

  // TODO: Could probably handle variadic functions.
  if (F.isVarArg() || F.hasStructRetAttr() ||
      PPU::isEntryFunctionCC(F.getCallingConv()))
    return false;
...
FunctionPass *llvm::createPPURewriteOutArgumentsPass() {
  return new PPURewriteOutArguments();
}
