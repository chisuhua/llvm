...
#ifndef LLVM_LIB_TARGET_PPU_PPUMACHINEMODULEINFO_H
#define LLVM_LIB_TARGET_PPU_PPUMACHINEMODULEINFO_H
...
class PPUMachineModuleInfo final : public MachineModuleInfoELF {
private:

  // All supported memory/synchronization scopes can be found here:
  //   http://llvm.org/docs/PPUUsage.html#memory-scopes
  ...
  /// In PPU target synchronization scopes are inclusive, meaning a
...
public:
  PPUMachineModuleInfo(const MachineModuleInfo &MMI);
  ...
  /// \p A or \p B is not supported by the PPU target.
...
#endif // LLVM_LIB_TARGET_PPU_PPUMACHINEMODULEINFO_H
