...
#define DEBUG_TYPE "bi-form-memory-clauses"
...
static cl::opt<unsigned>
    MaxClause("ppu-max-memory-clause", cl::Hidden, cl::init(15),
              cl::desc("Maximum length of a memory clause, instructions"));

namespace {

class PPUFormMemoryClauses : public MachineFunctionPass {
...
public:
  PPUFormMemoryClauses() : MachineFunctionPass(ID) {
    initializePPUFormMemoryClausesPass(*PassRegistry::getPassRegistry());
  }

  bool runOnMachineFunction(MachineFunction &MF) override;

  StringRef getPassName() const override { return "PPU Form memory clauses"; }
...
private:
  ...
  bool checkPressure(const MachineInstr &MI, PPUDownwardRPTracker &RPT);
...
INITIALIZE_PASS_BEGIN(PPUFormMemoryClauses, DEBUG_TYPE, "PPU Form memory clauses",
                      false, false)
INITIALIZE_PASS_DEPENDENCY(LiveIntervals)
INITIALIZE_PASS_END(PPUFormMemoryClauses, DEBUG_TYPE, "PPU Form memory clauses",
                    false, false)

char PPUFormMemoryClauses::ID = 0;

char &llvm::PPUFormMemoryClausesID = PPUFormMemoryClauses::ID;

FunctionPass *llvm::createPPUFormMemoryClausesPass() {
  return new PPUFormMemoryClauses();
}

static bool isVMEMClauseInst(const MachineInstr &MI) {
  return PPUInstrInfo::isFLAT(MI) || PPUInstrInfo::isVMEM(MI);
}

static bool isSMEMClauseInst(const MachineInstr &MI) {
  return PPUInstrInfo::isSMRD(MI);
}
...
static bool isValidClauseInst(const MachineInstr &MI, bool IsVMEMClause) {
  ...
  if (!MI.mayLoad() || MI.mayStore())
    return false;
  /* FIXME
  if (PPU::getAtomicNoRetOp(MI.getOpcode()) != -1 ||
      PPU::getAtomicRetOp(MI.getOpcode()) != -1)
    return false;
    */
...
template <typename Callable>
void PPUFormMemoryClauses::forAllLanes(unsigned Reg, LaneBitmask LaneMask,
                                      Callable Func) const {
  ...
  SmallVector<unsigned, PPU::NUM_TARGET_SUBREGS> CoveringSubregs;
...
bool PPUFormMemoryClauses::canBundle(const MachineInstr &MI, 
...
bool PPUFormMemoryClauses::checkPressure(const MachineInstr &MI,
                                        PPUDownwardRPTracker &RPT) {
  ...
  RPT.advanceToNext();
  PPURegPressure MaxPressure = RPT.moveMaxPressure();
...
void PPUFormMemoryClauses::collectRegUses(const MachineInstr &MI,
                                         RegUse &Defs, RegUse &Uses) const {
  for (const MachineOperand &MO : MI.operands()) {
    if (!MO.isReg())
      continue;
...
bool PPUFormMemoryClauses::processRegUses(const MachineInstr &MI, RegUse &Defs,
                                         RegUse &Uses,
                                         PPUDownwardRPTracker &RPT) {
...
bool PPUFormMemoryClauses::runOnMachineFunction(MachineFunction &MF) {
...
/* FIXME
  ...
  MaxVGPRs = TRI->getAllocatableSet(MF, &PPU::VGPR_32RegClass).count();
  MaxSGPRs = TRI->getAllocatableSet(MF, &PPU::SGPR_32RegClass).count();
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    for (auto I = MBB.instr_begin(), E = MBB.instr_end(); I != E; I = Next) {
      ...
      for (auto PPU = I; PPU != Next; ++PPU) {
        PPU->bundleWithPred();
        Ind->removeSingleMachineInstrFromMaps(*PPU);

        for (MachineOperand &MO : PPU->defs())
          if (MO.readsReg())
