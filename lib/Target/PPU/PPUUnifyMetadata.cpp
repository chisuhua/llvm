//===- PPUUnifyMetadata.cpp - Unify OpenCL metadata --------------------===//
//
...
namespace {
  ...
  namespace kOCLMD {
    ...
    const char CUDAVer[]            = "nvvmir.version";
  ...
  class PPUUnifyMetadata : public ModulePass {
  public:
    static char ID;

    explicit PPUUnifyMetadata() : ModulePass(ID) {}
...
char PPUUnifyMetadata::ID = 0;

char &llvm::PPUUnifyMetadataID = PPUUnifyMetadata::ID;

INITIALIZE_PASS(PPUUnifyMetadata, "ppu-unify-metadata",
                "Unify multiple OpenCL/CUDA metadata due to linking",
                false, false)

ModulePass* llvm::createPPUUnifyMetadataPass() {
  return new PPUUnifyMetadata();
}

bool PPUUnifyMetadata::runOnModule(Module &M) {
  const char* Vers[] = {
      kOCLMD::SpirVer,
      kOCLMD::OCLVer,
      kOCLMD::CUDAVer
