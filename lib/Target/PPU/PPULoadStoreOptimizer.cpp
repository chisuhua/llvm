...
#define DEBUG_TYPE "ppu-load-store-opt"
...
enum InstClassEnum {
  ...
  BUFFER_STORE_OFFSET = PPU::BUFFER_STORE_DWORD_OFFSET,
  BUFFER_LOAD_OFFEN_exact = PPU::BUFFER_LOAD_DWORD_OFFEN_exact,
...
class PPULoadStoreOptimizer : public MachineFunctionPass {
...
private:
  const PPUSubtarget *STM = nullptr;
  ...
  static bool widthsFit(const PPUSubtarget &STM, const CombineInfo &CI);
...
public:
  ...
  PPULoadStoreOptimizer() : MachineFunctionPass(ID) {
    initializePPULoadStoreOptimizerPass(*PassRegistry::getPassRegistry());
  }
  ...
  StringRef getPassName() const override { return "PPU Load Store Optimizer"; }
...
INITIALIZE_PASS_BEGIN(PPULoadStoreOptimizer, DEBUG_TYPE,
                      "PPU Load Store Optimizer", false, false)
INITIALIZE_PASS_DEPENDENCY(AAResultsWrapperPass)
INITIALIZE_PASS_END(PPULoadStoreOptimizer, DEBUG_TYPE, "PPU Load Store Optimizer",
                    false, false)

char PPULoadStoreOptimizer::ID = 0;

char &llvm::PPULoadStoreOptimizerID = PPULoadStoreOptimizer::ID;

FunctionPass *llvm::createPPULoadStoreOptimizerPass() {
  return new PPULoadStoreOptimizer();
}
...
bool PPULoadStoreOptimizer::offsetsCanBeCombined(CombineInfo &CI) {
...
bool PPULoadStoreOptimizer::widthsFit(const PPUSubtarget &STM,
                                     const CombineInfo &CI) {
  const unsigned Width = (CI.Width0 + CI.Width1);
  switch (CI.InstClass) {
  default:
    // FIXME return (Width <= 4) && (STM.hasDwordx3LoadStores() || (Width != 3));
    return (Width <= 4);
...
InstClassEnum PPULoadStoreOptimizer::getInstClass(unsigned Opc) const {
  ...
  if (TII->isMUBUF(Opc)) {
    ...
    case PPU::BUFFER_LOAD_DWORD_OFFSET:
      return BUFFER_LOAD_OFFSET;
    case PPU::BUFFER_STORE_DWORD_OFFEN:
      return BUFFER_STORE_OFFEN;
    case PPU::BUFFER_STORE_DWORD_OFFSET:
      return BUFFER_STORE_OFFSET;
    case PPU::BUFFER_LOAD_DWORD_OFFEN_exact:
      return BUFFER_LOAD_OFFEN_exact;
    case PPU::BUFFER_LOAD_DWORD_OFFSET_exact:
      return BUFFER_LOAD_OFFSET_exact;
    case PPU::BUFFER_STORE_DWORD_OFFEN_exact:
      return BUFFER_STORE_OFFEN_exact;
    case PPU::BUFFER_STORE_DWORD_OFFSET_exact:
...
unsigned PPULoadStoreOptimizer::getRegs(unsigned Opc) const {
  ...
  if (TII->isMUBUF(Opc)) {
    ...
    if (PPU::getMUBUFHasVAddr(Opc)) {
      result |= VADDR;
    }

    if (PPU::getMUBUFHasSrsrc(Opc)) {
      result |= SRSRC;
    }

    if (PPU::getMUBUFHasSoffset(Opc)) {
      result |= SOFFSET;
    }
...
/* FIXME
  ...
  if (Regs & SBASE) {
    AddrOpName[NumAddresses++] = PPU::OpName::sbase;
  }

  if (Regs & SRSRC) {
    AddrOpName[NumAddresses++] = PPU::OpName::srsrc;
  }

  if (Regs & SOFFSET) {
    AddrOpName[NumAddresses++] = PPU::OpName::soffset;
  }
...
/* FIXME
    if (Match) {
      ...
      if ((CI.InstClass == DS_READ) || (CI.InstClass == DS_WRITE)) {
        ...
        CI.GLC0 = TII->getNamedOperand(*CI.I, PPU::OpName::glc)->getImm();
        CI.GLC1 = TII->getNamedOperand(*MBBI, PPU::OpName::glc)->getImm();
        if (CI.InstClass != S_BUFFER_LOAD_IMM) {
          CI.SLC0 = TII->getNamedOperand(*CI.I, PPU::OpName::slc)->getImm();
          CI.SLC1 = TII->getNamedOperand(*MBBI, PPU::OpName::slc)->getImm();
        }
        CI.DLC0 = TII->getNamedOperand(*CI.I, PPU::OpName::dlc)->getImm();
        CI.DLC1 = TII->getNamedOperand(*MBBI, PPU::OpName::dlc)->getImm();
      }
...
PPULoadStoreOptimizer::mergeSBufferLoadImmPair(CombineInfo &CI) {
  ...
  BuildMI(*MBB, CI.Paired, DL, TII->get(Opcode), DestReg)
      .add(*TII->getNamedOperand(*CI.I, PPU::OpName::sbase))
  ...
  // Copy to the old destination registers.
  const MCInstrDesc &CopyDesc = TII->get(TargetOpcode::COPY);
  const auto *Dest0 = TII->getNamedOperand(*CI.I, PPU::OpName::sdst);
  const auto *Dest1 = TII->getNamedOperand(*CI.Paired, PPU::OpName::sdst);
...
PPULoadStoreOptimizer::mergeBufferLoadPair(CombineInfo &CI) {
  ...
  if (Regs & VADDR)
    MIB.add(*TII->getNamedOperand(*CI.I, PPU::OpName::vaddr));
  ...
  MIB.add(*TII->getNamedOperand(*CI.I, PPU::OpName::srsrc))
      .add(*TII->getNamedOperand(*CI.I, PPU::OpName::soffset))
      .addImm(MergedOffset) // offset
      .addImm(CI.GLC0)      // glc
      .addImm(CI.SLC0)      // slc
      .addImm(0)            // tfe
  ...
  // Copy to the old destination registers.
  const MCInstrDesc &CopyDesc = TII->get(TargetOpcode::COPY);
  const auto *Dest0 = TII->getNamedOperand(*CI.I, PPU::OpName::vdata);
  const auto *Dest1 = TII->getNamedOperand(*CI.Paired, PPU::OpName::vdata);
...
unsigned PPULoadStoreOptimizer::getNewOpcode(const CombineInfo &CI) {
  ...
  default:
    // FIXME: Handle d16 correctly
    return PPU::getMUBUFOpcode(CI.InstClass, Width);
  case UNKNOWN:
  ...
  if (CI.Offset0 > CI.Offset1) {
    ...
    case 1:
      ...
      default:
        return std::make_pair(0, 0);
      case 1:
        return std::make_pair(PPU::sub1, PPU::sub0);
      case 2:
        return std::make_pair(PPU::sub2, PPU::sub0_sub1);
      case 3:
        return std::make_pair(PPU::sub3, PPU::sub0_sub1_sub2);
      }
    case 2:
      switch (CI.Width1) {
      default:
        return std::make_pair(0, 0);
      case 1:
        return std::make_pair(PPU::sub1_sub2, PPU::sub0);
      case 2:
        return std::make_pair(PPU::sub2_sub3, PPU::sub0_sub1);
      }
    case 3:
      switch (CI.Width1) {
      default:
        return std::make_pair(0, 0);
      case 1:
        return std::make_pair(PPU::sub1_sub2_sub3, PPU::sub0);
      }
    }
  } else {
    switch (CI.Width0) {
    ...
    case 1:
      ...
      default:
        return std::make_pair(0, 0);
      case 1:
        return std::make_pair(PPU::sub0, PPU::sub1);
      case 2:
        return std::make_pair(PPU::sub0, PPU::sub1_sub2);
      case 3:
        return std::make_pair(PPU::sub0, PPU::sub1_sub2_sub3);
      }
    case 2:
      switch (CI.Width1) {
      default:
        return std::make_pair(0, 0);
      case 1:
        return std::make_pair(PPU::sub0_sub1, PPU::sub2);
      case 2:
        return std::make_pair(PPU::sub0_sub1, PPU::sub2_sub3);
      }
    case 3:
      switch (CI.Width1) {
      default:
        return std::make_pair(0, 0);
      case 1:
        return std::make_pair(PPU::sub0_sub1_sub2, PPU::sub3);
      }
    }
  }
#endif
...
PPULoadStoreOptimizer::mergeBufferStorePair(CombineInfo &CI) {
  ...
  const auto *Src0 = TII->getNamedOperand(*CI.I, PPU::OpName::vdata);
  const auto *Src1 = TII->getNamedOperand(*CI.Paired, PPU::OpName::vdata);

  BuildMI(*MBB, CI.Paired, DL, TII->get(PPU::REG_SEQUENCE), SrcReg)
      .add(*Src0)
      .addImm(SubRegIdx0)
      .add(*Src1)
      .addImm(SubRegIdx1);
...
MachineOperand
PPULoadStoreOptimizer::createRegOrImm(int32_t Val, MachineInstr &MI) {
  APInt V(32, Val, true);
...
unsigned PPULoadStoreOptimizer::computeBase(MachineInstr &MI,
  ...
  const auto *CarryRC = TRI->getRegClass(PPU::SReg_1RegClassID);
...
Optional<int32_t>
PPULoadStoreOptimizer::extractConstOffset(const MachineOperand &Op) {
  if (Op.isImm())
    return Op.getImm();
...
void PPULoadStoreOptimizer::processBaseWithConstOffset(const MachineOperand &Base,
                                                      MemAddress &Addr) {
  if (!Base.isReg())
    return;

  MachineInstr *Def = MRI->getUniqueVRegDef(Base.getReg());
  if (!Def || Def->getOpcode() != PPU::REG_SEQUENCE
...
/* FIXME
  ...
  const auto *Src0 = TII->getNamedOperand(*BaseLoDef, PPU::OpName::src0);
  const auto *Src1 = TII->getNamedOperand(*BaseLoDef, PPU::OpName::src1);
  ...
  Src0 = TII->getNamedOperand(*BaseHiDef, PPU::OpName::src0);
  Src1 = TII->getNamedOperand(*BaseHiDef, PPU::OpName::src1);
...
bool PPULoadStoreOptimizer::promoteConstantOffsetToImm(
  ...
  if (TII->getNamedOperand(MI, PPU::OpName::offset)->getImm()) {
    LLVM_DEBUG(dbgs() << "  Const-offset is already promoted.\n";);
    return false;
  }

  // Step1: Find the base-registers and a 64bit constant offset.
  MachineOperand &Base = *TII->getNamedOperand(MI, PPU::OpName::vaddr);
  MemAddress MAddr;
  ...
  const PPUTargetLowering *TLI =
    static_cast<const PPUTargetLowering *>(STM->getTargetLowering());
  ...
  for ( ; MBBI != E; ++MBBI) {
    ...
    if (MINext.getOpcode() != MI.getOpcode() ||
        TII->getNamedOperand(MINext, PPU::OpName::offset)->getImm())
      continue;

    const MachineOperand &BaseNext =
      *TII->getNamedOperand(MINext, PPU::OpName::vaddr);
...
bool PPULoadStoreOptimizer::optimizeBlock(MachineBasicBlock &MBB) {
  ...
  for (MachineBasicBlock::iterator I = MBB.begin(), E = MBB.end(); I != E;) {
    ...
    case DS_WRITE:
      ...
      continue;
    case S_BUFFER_LOAD_IMM:
      CI.EltSize = PPU::getSMRDEncodedOffset(*STM, 4);
      if (findMatchingInst(CI)) {
        Modified = true;
...
bool PPULoadStoreOptimizer::runOnMachineFunction(MachineFunction &MF) {
  ...
  LLVM_DEBUG(dbgs() << "Running PPULoadStoreOptimizer\n");
