//===-- PPUFixupVectorISel.cpp - Fixup post ISel vector issues -------------===//
//
...
//
/// \file
/// PPUFixupVectorISel pass cleans up post ISEL Vector issues.
...
#define DEBUG_TYPE "ppu-fixup-vector-isel"

using namespace llvm;

static cl::opt<bool> EnableGlobalSGPRAddr(
  "ppu-enable-global-sgpr-addr",
...
class PPUFixupVectorISel : public MachineFunctionPass {
public:
  static char ID;

public:
  PPUFixupVectorISel() : MachineFunctionPass(ID) {
    initializePPUFixupVectorISelPass(*PassRegistry::getPassRegistry());
  }
...
INITIALIZE_PASS(PPUFixupVectorISel, DEBUG_TYPE,
                "PPU Fixup Vector ISel", false, false)

char PPUFixupVectorISel::ID = 0;

char &llvm::PPUFixupVectorISelID = PPUFixupVectorISel::ID;

FunctionPass *llvm::createPPUFixupVectorISelPass() {
  return new PPUFixupVectorISel();
}
...
static bool findSRegBaseAndIndex(MachineOperand *Op,
  ...
  while (!Worklist.empty()) {
    ...
    case PPU::V_ADD_I32_e64:
      ...
      if (DefInst->getOperand(2).getSubReg() != PPU::NoSubRegister)
        continue;
      BaseReg = DefInst->getOperand(2).getReg();
      if (DefInst->getOperand(3).getSubReg() != PPU::NoSubRegister)
        continue;
      ...
      if (PPU::getRegBitWidth(IdxRC->getID()) != 64)
        continue;
      ...
      if (PPU::getRegBitWidth(BaseRC->getID()) != 64)
        continue;
...
static bool fixupGlobalSaddr(MachineBasicBlock &MBB,
  ...
  for (I = MBB.begin(); I != MBB.end(); I = Next) {
    Next = std::next(I);
    MachineInstr &MI = *I;
    int NewOpcd = PPU::getGlobalSaddrOp(MI.getOpcode());
    if (NewOpcd < 0)
      continue;
    ...
    unsigned BaseReg = 0;
    unsigned IndexReg = 0;
    MachineOperand *Op = TII->getNamedOperand(MI, PPU::OpName::vaddr);
    if (!findSRegBaseAndIndex(Op, BaseReg, IndexReg, MRI, TRI))
      continue;
    ++NumSGPRGlobalSaddrs;
    FuncModified = true;
    // Create the new _SADDR Memory instruction.
    bool HasVdst = TII->getNamedOperand(MI, PPU::OpName::vdst) != nullptr;
    MachineOperand *VData = TII->getNamedOperand(MI, PPU::OpName::vdata);
    ...
    NewGlob->addOperand(*TII->getNamedOperand(MI, PPU::OpName::offset));

    MachineOperand *Glc = TII->getNamedOperand(MI, PPU::OpName::glc);
    // Atomics dont have a GLC, so omit the field if not there.
    if (Glc)
      NewGlob->addOperand(MF, *Glc);

    MachineOperand *DLC = TII->getNamedOperand(MI, PPU::OpName::dlc);
    if (DLC)
      NewGlob->addOperand(MF, *DLC);

    NewGlob->addOperand(*TII->getNamedOperand(MI, PPU::OpName::slc));
...
bool PPUFixupVectorISel::runOnMachineFunction(MachineFunction &MF) {
