//===-- PPUMachineScheduler.cpp - PPU Scheduler Interface -------------------===//
//
...
//
/// \file
/// PPU Machine Scheduler interface
//
//===----------------------------------------------------------------------===//
...
//
// The scheduling problem for PPU can distinguish three main parts:
...
// Some figures taken from PPU docs:
...
static const char *getReasonStr(PPUScheduleCandReason Reason) {
  switch (Reason) {
...
static bool tryLess(int TryVal, int CandVal,
                    PPUSchedulerCandidate &TryCand,
                    PPUSchedulerCandidate &Cand,
                    PPUScheduleCandReason Reason) {
  if (TryVal < CandVal) {
...
} // end namespace PPUSched
} // end namespace llvm

// PPUScheduleBlock //

void PPUScheduleBlock::addUnit(SUnit *SU) {
  NodeNum2Index[SU->NodeNum] = SUnits.size();
  SUnits.push_back(SU);
}

#ifndef NDEBUG
void PPUScheduleBlock::traceCandidate(const PPUSchedCandidate &Cand) {
...
void PPUScheduleBlock::tryCandidateTopDown(PPUSchedCandidate &Cand,
                                          PPUSchedCandidate &TryCand) {
  ...
  if (Cand.SGPRUsage > 60 &&
      PPUSched::tryLess(TryCand.SGPRUsage, Cand.SGPRUsage,
  ...
  // loading more.
  if (PPUSched::tryLess(TryCand.HasLowLatencyNonWaitedParent,
                       Cand.HasLowLatencyNonWaitedParent,
                       TryCand, Cand, PPUScheduleCandReason::Depth))
    return;

  if (PPUSched::tryGreater(TryCand.IsLowLatency, Cand.IsLowLatency,
                          TryCand, Cand, PPUScheduleCandReason::Depth))
    return;

  if (TryCand.IsLowLatency &&
      PPUSched::tryLess(TryCand.LowLatencyOffset, Cand.LowLatencyOffset,
                       TryCand, Cand, PPUScheduleCandReason::Depth))
    return;

  if (PPUSched::tryLess(TryCand.VGPRUsage, Cand.VGPRUsage,
                       TryCand, Cand, RegUsage))
    return;
...
SUnit* PPUScheduleBlock::pickNode() {
  PPUSchedCandidate TopCand;
...
void PPUScheduleBlock::fastSchedule() {
  TopReadySUs.clear();
  if (Scheduled)
    undoSchedule();
...
void PPUScheduleBlock::initRegPressure(MachineBasicBlock::iterator BeginBlock,
...
void PPUScheduleBlock::schedule(MachineBasicBlock::iterator BeginBlock,
                               MachineBasicBlock::iterator EndBlock) {
  if (!Scheduled)
    fastSchedule();
...
void PPUScheduleBlock::undoSchedule() {
  for (SUnit* SU : SUnits) {
...
void PPUScheduleBlock::undoReleaseSucc(SUnit *SU, SDep *SuccEdge) {
...
void PPUScheduleBlock::releaseSucc(SUnit *SU, SDep *SuccEdge) {
  SUnit *SuccSU = SuccEdge->getSUnit();
...
void PPUScheduleBlock::releaseSuccessors(SUnit *SU, bool InOrOutBlock) {
  for (SDep& Succ : SU->Succs) {
...
void PPUScheduleBlock::finalizeUnits() {
...
void PPUScheduleBlock::addPred(PPUScheduleBlock *Pred) {
  unsigned PredID = Pred->getID();

  // Check if not already predecessor.
  for (PPUScheduleBlock* P : Preds) {
    if (PredID == P->getID())
      return;
  }
  ...
  assert(none_of(Succs,
                 [=](std::pair<PPUScheduleBlock*,
                     PPUScheduleBlockLinkKind> S) {
                   return PredID == S.first->getID();
                    }) &&
         "Loop in the Block Graph!");
}

void PPUScheduleBlock::addSucc(PPUScheduleBlock *Succ,
                              PPUScheduleBlockLinkKind Kind) {
  unsigned SuccID = Succ->getID();

  // Check if not already predecessor.
  for (std::pair<PPUScheduleBlock*, PPUScheduleBlockLinkKind> &S : Succs) {
    if (SuccID == S.first->getID()) {
      if (S.second == PPUScheduleBlockLinkKind::NoData &&
          Kind == PPUScheduleBlockLinkKind::Data)
        S.second = Kind;
      return;
    }
  }
  ...
  assert(none_of(Preds,
                 [=](PPUScheduleBlock *P) { return SuccID == P->getID(); }) &&
         "Loop in the Block Graph!");
}

#ifndef NDEBUG
void PPUScheduleBlock::printDebug(bool full) {
  dbgs() << "Block (" << ID << ")\n";
  if (!full)
    return;
  ...
  dbgs() << "\nSuccessors:\n";
  for (std::pair<PPUScheduleBlock*, PPUScheduleBlockLinkKind> S : Succs) {
    if (S.second == PPUScheduleBlockLinkKind::Data)
      dbgs() << "(Data Dep) ";
...
// PPUScheduleBlockCreator //

PPUScheduleBlockCreator::PPUScheduleBlockCreator(PPUScheduleDAGMI *DAG) :
DAG(DAG) {
}

PPUScheduleBlockCreator::~PPUScheduleBlockCreator() = default;

PPUScheduleBlocks
PPUScheduleBlockCreator::getBlocks(PPUSchedulerBlockCreatorVariant BlockVariant) {
  std::map<PPUSchedulerBlockCreatorVariant, PPUScheduleBlocks>::iterator B =
    Blocks.find(BlockVariant);
  if (B == Blocks.end()) {
    PPUScheduleBlocks Res;
...
bool PPUScheduleBlockCreator::isSUInBlock(SUnit *SU, unsigned ID) {
  if (SU->NodeNum >= DAG->SUnits.size())
    return false;
  return CurrentBlocks[Node2CurrentBlock[SU->NodeNum]]->getID() == ID;
}

void PPUScheduleBlockCreator::colorHighLatenciesAlone() {
...
void PPUScheduleBlockCreator::colorHighLatenciesGroups() {
  unsigned DAGSize = DAG->SUnits.size();
...
void PPUScheduleBlockCreator::colorComputeReservedDependencies() {
  unsigned DAGSize = DAG->SUnits.size();
...
void PPUScheduleBlockCreator::colorAccordingToReservedDependencies() {
  unsigned DAGSize = DAG->SUnits.size();
...
void PPUScheduleBlockCreator::colorEndsAccordingToDependencies() {
  unsigned DAGSize = DAG->SUnits.size();
...
void PPUScheduleBlockCreator::colorForceConsecutiveOrderInGroup() {
  unsigned DAGSize = DAG->SUnits.size();
...
void PPUScheduleBlockCreator::colorMergeConstantLoadsNextGroup() {
  unsigned DAGSize = DAG->SUnits.size();
...
void PPUScheduleBlockCreator::colorMergeIfPossibleNextGroup() {
  unsigned DAGSize = DAG->SUnits.size();
...
void PPUScheduleBlockCreator::colorMergeIfPossibleNextGroupOnlyForReserved() {
  unsigned DAGSize = DAG->SUnits.size();
...
void PPUScheduleBlockCreator::colorMergeIfPossibleSmallGroupsToNextGroup() {
  unsigned DAGSize = DAG->SUnits.size();
...
void PPUScheduleBlockCreator::cutHugeBlocks() {
  // TODO
}

void PPUScheduleBlockCreator::regroupNoUserInstructions() {
  unsigned DAGSize = DAG->SUnits.size();
...
void PPUScheduleBlockCreator::colorExports() {
  ...
  for (unsigned SUNum : DAG->TopDownIndex2SU) {
    const SUnit &SU = DAG->SUnits[SUNum];
    if (PPUInstrInfo::isEXP(*SU.getInstr())) {
      ...
      for (unsigned j : ExpGroup) {
        ...
        for (unsigned k : SubGraph) {
          if (!PPUInstrInfo::isEXP(*DAG->SUnits[k].getInstr()))
...
void PPUScheduleBlockCreator::createBlocksForVariant(PPUSchedulerBlockCreatorVariant BlockVariant) {
  unsigned DAGSize = DAG->SUnits.size();
  ...
  if (BlockVariant == PPUSchedulerBlockCreatorVariant::LatenciesGrouped)
  ...
  if (BlockVariant == PPUSchedulerBlockCreatorVariant::LatenciesAlonePlusConsecutive)
  ...
  for (unsigned i = 0, e = DAGSize; i != e; ++i) {
    ...
    if (RealID.find(Color) == RealID.end()) {
      ...
      BlockPtrs.push_back(std::make_unique<PPUScheduleBlock>(DAG, this, ID));
  ...
  for (unsigned i = 0, e = CurrentBlocks.size(); i != e; ++i) {
    PPUScheduleBlock *Block = CurrentBlocks[i];
    Block->finalizeUnits();
  }
...
void PPUScheduleBlockCreator::topologicalSort() {
  ...
  while (!WorkList.empty()) {
    ...
    for (PPUScheduleBlock* Pred : Block->getPreds()) {
...
void PPUScheduleBlockCreator::scheduleInsideBlocks() {
  unsigned DAGSize = CurrentBlocks.size();
  ...
  for (unsigned i = 0, e = DAGSize; i != e; ++i) {
    ...
    PPUScheduleBlock *Block = CurrentBlocks[BlockIndice];
...
void PPUScheduleBlockCreator::fillStats() {
  unsigned DAGSize = CurrentBlocks.size();
...
// PPUScheduleBlockScheduler //

PPUScheduleBlockScheduler::PPUScheduleBlockScheduler(PPUScheduleDAGMI *DAG,
                                                   PPUSchedulerBlockSchedulerVariant Variant,
                                                   PPUScheduleBlocks  BlocksStruct) :
  ...
  for (unsigned i = 0, e = Blocks.size(); i != e; ++i) {
    PPUScheduleBlock *Block = Blocks[i];
  ...
  for (unsigned Reg : DAG->getOutRegs()) {
    for (unsigned i = 0, e = Blocks.size(); i != e; ++i) {
      ...
      PPUScheduleBlock *Block = Blocks[ID];
  ...
  while (PPUScheduleBlock *Block = pickBlock()) {
    BlocksScheduled.push_back(Block);
    blockScheduled(Block);
  }

  LLVM_DEBUG(dbgs() << "Block Order:"; for (PPUScheduleBlock *Block
                                            : BlocksScheduled) {
    dbgs() << ' ' << Block->getID();
  } dbgs() << '\n';);
}

bool PPUScheduleBlockScheduler::tryCandidateLatency(PPUBlockSchedCandidate &Cand,
                                                   PPUBlockSchedCandidate &TryCand) {
  if (!Cand.isValid()) {
    TryCand.Reason = NodeOrder;
    return true;
  }
  ...
  if (PPUSched::tryLess(TryCand.LastPosHighLatParentScheduled,
                 Cand.LastPosHighLatParentScheduled, TryCand, Cand, Latency))
    return true;
  // Schedule high latencies early so you can hide them better.
  if (PPUSched::tryGreater(TryCand.IsHighLatency, Cand.IsHighLatency,
                          TryCand, Cand, Latency))
    return true;
  if (TryCand.IsHighLatency && PPUSched::tryGreater(TryCand.Height, Cand.Height,
                                                   TryCand, Cand, Depth))
    return true;
  if (PPUSched::tryGreater(TryCand.NumHighLatencySuccessors,
...
bool PPUScheduleBlockScheduler::tryCandidateRegUsage(PPUBlockSchedCandidate &Cand,
                                                    PPUBlockSchedCandidate &TryCand) {
  if (!Cand.isValid()) {
    TryCand.Reason = NodeOrder;
    return true;
  ...
  if (PPUSched::tryLess(TryCand.VGPRUsageDiff > 0, Cand.VGPRUsageDiff > 0,
                       TryCand, Cand, RegUsage))
    return true;
  if (PPUSched::tryGreater(TryCand.NumSuccessors > 0,
                          Cand.NumSuccessors > 0,
                          TryCand, Cand, Successor))
    return true;
  if (PPUSched::tryGreater(TryCand.Height, Cand.Height, TryCand, Cand, Depth))
    return true;
  if (PPUSched::tryLess(TryCand.VGPRUsageDiff, Cand.VGPRUsageDiff,
                       TryCand, Cand, RegUsage))
    return true;
  return false;
}

PPUScheduleBlock *PPUScheduleBlockScheduler::pickBlock() {
  PPUBlockSchedCandidate Cand;
  std::vector<PPUScheduleBlock*>::iterator Best;
  PPUScheduleBlock *Block;
  ...
  Cand.Block = nullptr;
  for (std::vector<PPUScheduleBlock*>::iterator I = ReadyBlocks.begin(),
    ...
    if (VregCurrentUsage > 120 ||
        Variant != PPUSchedulerBlockSchedulerVariant::BlockLatencyRegUsage) {
      if (!tryCandidateRegUsage(Cand, TryCand) &&
          Variant != PPUSchedulerBlockSchedulerVariant::BlockRegUsage)
...
void PPUScheduleBlockScheduler::addLiveRegs(std::set<unsigned> &Regs) {
...
void PPUScheduleBlockScheduler::decreaseLiveRegs(PPUScheduleBlock *Block,
...
void PPUScheduleBlockScheduler::releaseBlockSuccs(PPUScheduleBlock *Parent) {
  for (const auto &Block : Parent->getSuccs()) {
    ...
    if (Parent->isHighLatencyBlock() &&
        Block.second == PPUScheduleBlockLinkKind::Data)
      LastPosHighLatencyParentScheduled[Block.first->getID()] = NumBlockScheduled;
  }
}

void PPUScheduleBlockScheduler::blockScheduled(PPUScheduleBlock *Block) {
...
std::vector<int>
PPUScheduleBlockScheduler::checkRegUsageImpact(std::set<unsigned> &InRegs,
...
struct PPUScheduleBlockResult
PPUScheduler::scheduleVariant(PPUSchedulerBlockCreatorVariant BlockVariant,
                             PPUSchedulerBlockSchedulerVariant ScheduleVariant) {
  PPUScheduleBlocks Blocks = BlockCreator.getBlocks(BlockVariant);
  PPUScheduleBlockScheduler Scheduler(DAG, ScheduleVariant, Blocks);
  std::vector<PPUScheduleBlock*> ScheduledBlocks;
  struct PPUScheduleBlockResult Res;

  ScheduledBlocks = Scheduler.getBlocks();

  for (unsigned b = 0; b < ScheduledBlocks.size(); ++b) {
    PPUScheduleBlock *Block = ScheduledBlocks[b];
    std::vector<SUnit*> SUs = Block->getScheduledUnits();
...
PPUScheduleDAGMI::PPUScheduleDAGMI(MachineSchedContext *C) :
  ScheduleDAGMILive(C, std::make_unique<GenericScheduler>(C)) {
  PPUTII = static_cast<const PPUInstrInfo*>(TII);
  PPUTRI = static_cast<const PPURegisterInfo*>(TRI);

  VGPRSetID = PPUTRI->getVGPRPressureSet();
  SGPRSetID = PPUTRI->getSGPRPressureSet();
}

PPUScheduleDAGMI::~PPUScheduleDAGMI() = default;

// Code adapted from scheduleDAG.cpp
// Does a topological sort over the SUs.
// Both TopDown and BottomUp
void PPUScheduleDAGMI::topologicalSort() {
...
void PPUScheduleDAGMI::moveLowLatencies() {
   ...
   for (unsigned i = 0, e = ScheduledSUnits.size(); i != e; ++i) {
    ...
    for (SDep& PredDep : SU->Preds) {
      SUnit *Pred = PredDep.getSUnit();
      if (PPUTII->isLowLatencyInstruction(*Pred->getInstr())) {
    ...
    if (PPUTII->isLowLatencyInstruction(*SU->getInstr())) {
    ...
    } else if (SU->getInstr()->getOpcode() == PPU::COPY) {
      bool CopyForLowLat = false;
      for (SDep& SuccDep : SU->Succs) {
        SUnit *Succ = SuccDep.getSUnit();
        if (SuccDep.isWeak() || Succ->NodeNum >= DAGSize)
          continue;
        if (PPUTII->isLowLatencyInstruction(*Succ->getInstr())) {
          CopyForLowLat = true;
        }
      }
      if (!CopyForLowLat)
...
void PPUScheduleDAGMI::restoreSULinksLeft() {
...
PPUScheduleDAGMI::fillVgprSgprCost(_Iterator First, _Iterator End,
...
void PPUScheduleDAGMI::schedule()
{
  SmallVector<SUnit*, 8> TopRoots, BotRoots;
  PPUScheduleBlockResult Best, Temp;
  ...
  for (unsigned i = 0, e = (unsigned)SUnits.size(); i != e; ++i) {
    ...
    if (PPUTII->isLowLatencyInstruction(*SU->getInstr())) {
      IsLowLatencySU[i] = 1;
      if (PPUTII->getMemOperandWithOffset(*SU->getInstr(), BaseLatOp, OffLatReg,
                                       TRI))
        LowLatencyOffset[i] = OffLatReg;
    } else if (PPUTII->isHighLatencyInstruction(*SU->getInstr()))
      IsHighLatencySU[i] = 1;
  }

  PPUScheduler Scheduler(this);
  Best = Scheduler.scheduleVariant(PPUSchedulerBlockCreatorVariant::LatenciesAlone,
                                   PPUSchedulerBlockSchedulerVariant::BlockLatencyRegUsage);

  // if VGPR usage is extremely high, try other good performing variants
  // which could lead to lower VGPR usage
  if (Best.MaxVGPRUsage > 180) {
    static const std::pair<PPUSchedulerBlockCreatorVariant,
                           PPUSchedulerBlockSchedulerVariant>
        Variants[] = {
...
//      { LatenciesAlonePlusConsecutive, BlockRegUsage }
    ...
    for (std::pair<PPUSchedulerBlockCreatorVariant, PPUSchedulerBlockSchedulerVariant> v : Variants) {
