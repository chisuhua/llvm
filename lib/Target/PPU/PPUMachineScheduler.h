//===-- PPUMachineScheduler.h - PPU Scheduler Interface -----------*- C++ -*-===//
//
...
#ifndef LLVM_LIB_TARGET_PPU_MACHINESCHEDULER_H
#define LLVM_LIB_TARGET_PPU_MACHINESCHEDULER_H
...
struct PPUSchedulerCandidate {
  // The reason for this candidate.
  PPUScheduleCandReason Reason = NoCand;

  // Set of reasons that apply to multiple candidates.
  uint32_t RepeatReasonSet = 0;

  PPUSchedulerCandidate() = default;

  bool isRepeat(PPUScheduleCandReason R) { return RepeatReasonSet & (1 << R); }
  void setRepeat(PPUScheduleCandReason R) { RepeatReasonSet |= (1 << R); }
};

class PPUScheduleDAGMI;
class PPUScheduleBlockCreator;

enum PPUScheduleBlockLinkKind {
  NoData,
  Data
};

class PPUScheduleBlock {
  PPUScheduleDAGMI *DAG;
  PPUScheduleBlockCreator *BC;
  ...
  // Unique ID, the index of the Block in the PPUScheduleDAGMI Blocks table.
  unsigned ID;

  std::vector<PPUScheduleBlock*> Preds;  // All blocks predecessors.
  // All blocks successors, and the kind of link
  std::vector<std::pair<PPUScheduleBlock*, PPUScheduleBlockLinkKind>> Succs;
  unsigned NumHighLatencySuccessors = 0;

public:
  PPUScheduleBlock(PPUScheduleDAGMI *DAG, PPUScheduleBlockCreator *BC,
                  unsigned ID):
    DAG(DAG), BC(BC), TopRPTracker(TopPressure), ID(ID) {}

  ~PPUScheduleBlock() = default;
  ...
  void addPred(PPUScheduleBlock *Pred);
  void addSucc(PPUScheduleBlock *Succ, PPUScheduleBlockLinkKind Kind);

  const std::vector<PPUScheduleBlock*>& getPreds() const { return Preds; }
  ArrayRef<std::pair<PPUScheduleBlock*, PPUScheduleBlockLinkKind>>
...
private:
  struct PPUSchedCandidate : PPUSchedulerCandidate {
    ...
    PPUSchedCandidate() = default;

    bool isValid() const { return SU; }

    // Copy the status of another candidate without changing policy.
    void setBest(PPUSchedCandidate &Best) {
  ...
  void tryCandidateTopDown(PPUSchedCandidate &Cand, PPUSchedCandidate &TryCand);
  void tryCandidateBottomUp(PPUSchedCandidate &Cand, PPUSchedCandidate &TryCand);
  SUnit* pickNode();
  void traceCandidate(const PPUSchedCandidate &Cand);
  void initRegPressure(MachineBasicBlock::iterator BeginBlock,
                       MachineBasicBlock::iterator EndBlock);
};

struct PPUScheduleBlocks {
  std::vector<PPUScheduleBlock*> Blocks;
  std::vector<int> TopDownIndex2Block;
  std::vector<int> TopDownBlock2Index;
};

enum PPUSchedulerBlockCreatorVariant {
  LatenciesAlone,
  LatenciesGrouped,
...
class PPUScheduleBlockCreator {
  ...
  std::vector<std::unique_ptr<PPUScheduleBlock>> BlockPtrs;
  std::map<PPUSchedulerBlockCreatorVariant,
           PPUScheduleBlocks> Blocks;
  std::vector<PPUScheduleBlock*> CurrentBlocks;
...
public:
  PPUScheduleBlockCreator(PPUScheduleDAGMI *DAG);
  ~PPUScheduleBlockCreator();

  PPUScheduleBlocks
  getBlocks(PPUSchedulerBlockCreatorVariant BlockVariant);
...
private:
  ...
  void createBlocksForVariant(PPUSchedulerBlockCreatorVariant BlockVariant);
...
enum PPUSchedulerBlockSchedulerVariant {
  BlockLatencyRegUsage,
  BlockRegUsageLatency,
  BlockRegUsage
};

class PPUScheduleBlockScheduler {
  PPUScheduleDAGMI *DAG;
  ...
  std::vector<PPUScheduleBlock*> BlocksScheduled;
  unsigned NumBlockScheduled;
  std::vector<PPUScheduleBlock*> ReadyBlocks;
...
public:
  PPUScheduleBlockScheduler(PPUScheduleDAGMI *DAG,
                           PPUSchedulerBlockSchedulerVariant Variant,
                           PPUScheduleBlocks BlocksStruct);
  ~PPUScheduleBlockScheduler() = default;

  std::vector<PPUScheduleBlock*> getBlocks() { return BlocksScheduled; }

  unsigned getVGPRUsage() { return maxVregUsage; }
  unsigned getSGPRUsage() { return maxSregUsage; }

private:
  struct PPUBlockSchedCandidate : PPUSchedulerCandidate {
    // The best Block candidate.
    PPUScheduleBlock *Block = nullptr;
    ...
    PPUBlockSchedCandidate() = default;

    bool isValid() const { return Block; }

    // Copy the status of another candidate without changing policy.
    void setBest(PPUBlockSchedCandidate &Best) {
      assert(Best.Reason != NoCand && "uninitialized Sched candidate");
      Block = Best.Block;
      Reason = Best.Reason;
  ...
  bool tryCandidateLatency(PPUBlockSchedCandidate &Cand,
                           PPUBlockSchedCandidate &TryCand);
  bool tryCandidateRegUsage(PPUBlockSchedCandidate &Cand,
                            PPUBlockSchedCandidate &TryCand);
  PPUScheduleBlock *pickBlock();

  void addLiveRegs(std::set<unsigned> &Regs);
  void decreaseLiveRegs(PPUScheduleBlock *Block, std::set<unsigned> &Regs);
  void releaseBlockSuccs(PPUScheduleBlock *Parent);
  void blockScheduled(PPUScheduleBlock *Block);
...
class PPUScheduler {
  PPUScheduleDAGMI *DAG;
  PPUScheduleBlockCreator BlockCreator;

public:
  PPUScheduler(PPUScheduleDAGMI *DAG) : DAG(DAG), BlockCreator(DAG) {}

  ~PPUScheduler() = default;

  struct PPUScheduleBlockResult
  scheduleVariant(PPUSchedulerBlockCreatorVariant BlockVariant,
                  PPUSchedulerBlockSchedulerVariant ScheduleVariant);
};

class PPUScheduleDAGMI final : public ScheduleDAGMILive {
  const PPUInstrInfo *PPUTII;
  const PPURegisterInfo *PPUTRI;
...
public:
  PPUScheduleDAGMI(MachineSchedContext *C);

  ~PPUScheduleDAGMI() override;
...
#endif // LLVM_LIB_TARGET_PPU_MACHINESCHEDULER_H
