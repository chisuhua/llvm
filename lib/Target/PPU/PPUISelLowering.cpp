//===-- PPUISelLowering.cpp - PPU DAG Lowering Implementation ---------------===//
//
...
static cl::opt<bool> DisableLoopAlignment(
  "ppu-disable-loop-alignment",
  cl::desc("Do not align and prefetch loops"),
  cl::init(false));

static unsigned findFirstFreeSGPR(CCState &CCInfo) {
  unsigned NumSGPRs = PPU::SPR_32RegClass.getNumRegs();
  for (unsigned Reg = 0; Reg < NumSGPRs; ++Reg) {
    if (!CCInfo.isAllocated(PPU::SPR0 + Reg)) {
      return PPU::SPR0 + Reg;
    }
  }
...
PPUTargetLowering::PPUTargetLowering(const TargetMachine &TM,
                                   const PPUSubtarget &STI)
    : PPUBaseTargetLowering(TM, STI),
      Subtarget(&STI) {
  addRegisterClass(MVT::i1, &PPU::VReg_1RegClass);
  addRegisterClass(MVT::i64, &PPU::SReg_64RegClass);

  addRegisterClass(MVT::i32, &PPU::SReg_32RegClass);
  addRegisterClass(MVT::f32, &PPU::VPR_32RegClass);
...
/* TODO
  ...
  addRegisterClass(MVT::v2i64, &PPU::SReg_128RegClass);
  addRegisterClass(MVT::v2f64, &PPU::SReg_128RegClass);
...
*/
  if (Subtarget->has16BitInsts()) {
    addRegisterClass(MVT::i16, &PPU::SReg_32RegClass);
    addRegisterClass(MVT::f16, &PPU::SReg_32RegClass);

    // Unless there are also VOP3P operations, not operations are really legal.
    addRegisterClass(MVT::v2i16, &PPU::SReg_32RegClass);
    addRegisterClass(MVT::v2f16, &PPU::SReg_32RegClass);
    addRegisterClass(MVT::v4i16, &PPU::SReg_64RegClass);
    addRegisterClass(MVT::v4f16, &PPU::SReg_64RegClass);
  }
/* TODO
  if (Subtarget->hasMAIInsts()) {
    addRegisterClass(MVT::v32i32, &PPU::VReg_1024RegClass);
    addRegisterClass(MVT::v32f32, &PPU::VReg_1024RegClass);
  }
*/
  ...
  // BUFFER/FLAT_ATOMIC_CMP_SWAP on PPU GPUs needs input marshalling,
...
const PPUSubtarget *PPUTargetLowering::getSubtarget() const {
  return Subtarget;
}
...
bool PPUTargetLowering::isFPExtFoldable(unsigned Opcode,
...
bool PPUTargetLowering::isShuffleMaskLegal(ArrayRef<int>, EVT) const {
...
unsigned PPUTargetLowering::getVectorTypeBreakdownForCallingConv(
...
/*
bool PPUTargetLowering::getTgtMemIntrinsic(IntrinsicInfo &Info,
                                          const CallInst &CI,
                                          MachineFunction &MF,
                                          unsigned IntrID) const {
  if (const PPU::RsrcIntrinsic *RsrcIntr =
          PPU::lookupRsrcIntrinsic(IntrID)) {
    ...
    if (RsrcIntr->IsImage) {
      Info.ptrVal = MFI->getImagePSV(
        *MF.getSubtarget<PPUSubtarget>().getInstrInfo(),
  ...
  switch (IntrID) {
  case Intrinsic::ppu_atomic_inc:
  case Intrinsic::ppu_atomic_dec:
  case Intrinsic::ppu_ds_ordered_add:
  case Intrinsic::ppu_ds_ordered_swap:
  case Intrinsic::ppu_ds_fadd:
  case Intrinsic::ppu_ds_fmin:
  case Intrinsic::ppu_ds_fmax: {
    Info.opc = ISD::INTRINSIC_W_CHAIN;
    Info.memVT = MVT::getVT(CI.getType());
  ...
  case Intrinsic::ppu_buffer_atomic_fadd: {
    PPUMachineFunctionInfo *MFI = MF.getInfo<PPUMachineFunctionInfo>();
  ...
  case Intrinsic::ppu_global_atomic_fadd: {
    Info.opc = ISD::INTRINSIC_VOID;
    Info.memVT = MVT::getVT(CI.getOperand(0)->getType()
  ...
  case Intrinsic::ppu_ds_append:
  case Intrinsic::ppu_ds_consume: {
    Info.opc = ISD::INTRINSIC_W_CHAIN;
    Info.memVT = MVT::getVT(CI.getType());
    Info.ptrVal = CI.getOperand(0);
    Info.align.reset();
  ...
  case Intrinsic::ppu_ds_gws_init:
  case Intrinsic::ppu_ds_gws_barrier:
  case Intrinsic::ppu_ds_gws_sema_v:
  case Intrinsic::ppu_ds_gws_sema_br:
  case Intrinsic::ppu_ds_gws_sema_p:
  case Intrinsic::ppu_ds_gws_sema_release_all: {
    Info.opc = ISD::INTRINSIC_VOID;

    PPUMachineFunctionInfo *MFI = MF.getInfo<PPUMachineFunctionInfo>();
    Info.ptrVal =
        MFI->getGWSPSV(*MF.getSubtarget<PPUSubtarget>().getInstrInfo());
    ...
    Info.flags = MachineMemOperand::MOStore;
    if (IntrID == Intrinsic::ppu_ds_gws_barrier)
      Info.flags = MachineMemOperand::MOLoad;
    return true;
  }
  default:
...
bool PPUTargetLowering::getAddrModeArguments(IntrinsicInst *II,
...
bool PPUTargetLowering::isLegalFlatAddressingMode(const AddrMode &AM) const {
...
bool PPUTargetLowering::isLegalGlobalAddressingMode(const AddrMode &AM) const {
...
bool PPUTargetLowering::isLegalMUBUFAddressingMode(const AddrMode &AM) const {
...
bool PPUTargetLowering::isLegalAddressingMode(const DataLayout &DL,
...
/* FIXM
    if (Subtarget->getGeneration() == PPUSubtarget::SOUTHERN_ISLANDS) {
      ...
      if (!isUInt<8>(AM.BaseOffs / 4))
        return false;
    } else if (Subtarget->getGeneration() == PPUSubtarget::SEA_ISLANDS) {
      // On CI+, this can also be a 32-bit literal constant offset. If it fits
      // in 8-bits, it can use a smaller encoding.
      if (!isUInt<32>(AM.BaseOffs / 4))
        return false;
    } else if (Subtarget->getGeneration() >= PPUSubtarget::VOLCANIC_ISLANDS) {
...
bool PPUTargetLowering::canMergeStoresTo(unsigned AS, EVT MemVT,
...
bool PPUTargetLowering::allowsMisalignedMemoryAccesses(
...
EVT PPUTargetLowering::getOptimalMemOpType(
...
bool PPUTargetLowering::isNoopAddrSpaceCast(unsigned SrcAS,
                                           unsigned DestAS) const {
  return isFlatGlobalAddrSpace(SrcAS) && isFlatGlobalAddrSpace(DestAS);
}

bool PPUTargetLowering::isMemOpHasNoClobberedMemOperand(const SDNode *N) const {
...
bool PPUTargetLowering::isFreeAddrSpaceCast(unsigned SrcAS,
                                           unsigned DestAS) const {
...
bool PPUTargetLowering::isMemOpUniform(const SDNode *N) const {
  const MemSDNode *MemNode = cast<MemSDNode>(N);

  return PPUInstrInfo::isUniformMMO(MemNode->getMemOperand());
}

TargetLoweringBase::LegalizeTypeAction
PPUTargetLowering::getPreferredVectorAction(MVT VT) const {
...
bool PPUTargetLowering::shouldConvertConstantLoadToIntImm(const APInt &Imm,
                                                         Type *Ty) const {
  // FIXME: Could be smarter if called for vector constants.
  return true;
}

bool PPUTargetLowering::isTypeDesirableForOp(unsigned Op, EVT VT) const {
...
SDValue PPUTargetLowering::getImplicitArgPtr(SelectionDAG &DAG,
...
void PPUTargetLowering::allocateSpecialEntryInputVGPRs(CCState &CCInfo,
  ...
  if (Info.hasWorkItemIDX()) {
    Register Reg = PPU::VPR0;
    MRI.setType(MF.addLiveIn(Reg, &PPU::VPR_32RegClass), S32);
...
static ArgDescriptor allocateVGPR32Input(CCState &CCInfo, unsigned Mask = ~0u,
  ...
  ArrayRef<MCPhysReg> ArgVGPRs
    = makeArrayRef(PPU::VPR_32RegClass.begin(), 32);
  ...
  assert(Reg != PPU::NoRegister);

  MachineFunction &MF = CCInfo.getMachineFunction();
  MF.addLiveIn(Reg, &PPU::VPR_32RegClass);
...
static ArgDescriptor allocateSGPR32Input(CCState &CCInfo) {
  return allocateSGPR32InputImpl(CCInfo, &PPU::SPR_32RegClass, 32);
}

static ArgDescriptor allocateSGPR64Input(CCState &CCInfo) {
  return allocateSGPR32InputImpl(CCInfo, &PPU::SPR_64RegClass, 16);
}

void PPUTargetLowering::allocateSpecialInputVGPRs(CCState &CCInfo,
                                                 MachineFunction &MF,
                                                 const PPURegisterInfo &TRI,
                                                 PPUMachineFunctionInfo &Info) const {
...
void PPUTargetLowering::allocateSpecialInputSGPRs(
  CCState &CCInfo,
  MachineFunction &MF,
  const PPURegisterInfo &TRI,
  PPUMachineFunctionInfo &Info) const {
...
void PPUTargetLowering::allocateHSAUserSGPRs(CCState &CCInfo,
                                            MachineFunction &MF,
                                            const PPURegisterInfo &TRI,
                                            PPUMachineFunctionInfo &Info) const {
  ...
  if (Info.hasPrivateSegmentBuffer()) {
    ...
    MF.addLiveIn(PrivateSegmentBufferReg, &PPU::SPR_64RegClass);  // TODO i change 128 to 64
  ...
  if (Info.hasKernargSegmentPtr()) {
    ...
    Register VReg = MF.addLiveIn(InputPtrReg, &PPU::SPR_64RegClass);
...
void PPUTargetLowering::allocateSystemSGPRs(CCState &CCInfo,
                                           MachineFunction &MF,
                                           PPUMachineFunctionInfo &Info,
                                           CallingConv::ID CallConv,
                                           bool IsShader) const {
  if (Info.hasWorkGroupIDX()) {
    ...
    MF.addLiveIn(Reg, &PPU::SReg_32RegClass);
    CCInfo.AllocateReg(Reg);
  }
  ...
  if (Info.hasPrivateSegmentWaveByteOffset()) {
    ...
    if (IsShader) {
      ...
      // location.
      if (PrivateSegmentWaveByteOffsetReg == PPU::NoRegister) {
    ...
    MF.addLiveIn(PrivateSegmentWaveByteOffsetReg, &PPU::SPR_32RegClass);
...
static void reservePrivateMemoryRegs(const TargetMachine &TM,
  ...
  if (RequiresStackAccess && ST.isPPSOS()) {
    ...
    Register PrivateSegmentBufferReg =
        Info.getPreloadedReg(PPUFunctionArgInfo::PRIVATE_SEGMENT_BUFFER);
  ...
  if (ST.getFrameLowering()->hasFP(MF)) {
    ...
    // registers.
    if (!MRI.isLiveIn(PPU::SPR32)) {
      Info.setStackPtrOffsetReg(PPU::SPR32);
    } else {
      assert(!PPU::isKernel(MF.getFunction().getCallingConv()));

      if (MFI.hasCalls())
        report_fatal_error("call in graphics shader with too many input SGPRs");

      for (unsigned Reg : PPU::SPR_32RegClass) {
        if (!MRI.isLiveIn(Reg)) {
    ...
    if (MFI.hasCalls()) {
      Info.setScratchWaveOffsetReg(PPU::SPR33);  // FIXME
      Info.setFrameOffsetReg(PPU::SPR33);
    } else {
...
bool PPUTargetLowering::supportSplitCSR(MachineFunction *MF) const {
  const PPUMachineFunctionInfo *Info = MF->getInfo<PPUMachineFunctionInfo>();
  return !Info->isEntryFunction();
}

void PPUTargetLowering::initializeSplitCSR(MachineBasicBlock *Entry) const {

}

void PPUTargetLowering::insertCopiesSplitCSR(
  MachineBasicBlock *Entry,
  const SmallVectorImpl<MachineBasicBlock *> &Exits) const {
  const PPURegisterInfo *TRI = getSubtarget()->getRegisterInfo();
  ...
  for (const MCPhysReg *I = IStart; *I; ++I) {
    ...
    if (PPU::SReg_64RegClass.contains(*I))
      RC = &PPU::SPR_64RegClass;
    else if (PPU::SReg_32RegClass.contains(*I))
...
SDValue PPUTargetLowering::LowerFormalArguments(
  ...
  bool IsShader = PPU::isShader(CallConv);
  ...
  auto &ArgUsageInfo =
    DAG.getPass()->getAnalysis<PPUArgumentUsageInfo>();
...
bool PPUTargetLowering::CanLowerReturn(
  CallingConv::ID CallConv,
  ...
  if (PPU::isEntryFunctionCC(CallConv))
    return true;
...
SDValue
PPUTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                              bool isVarArg,
                              const SmallVectorImpl<ISD::OutputArg> &Outs,
  ...
  if (!PPU::isCompute(CallConv)) {
    return PPUBaseTargetLowering::LowerReturn(Chain, CallConv, isVarArg, Outs,
                                             OutVals, DL, DAG);
  }

  if (PPU::isKernel(CallConv)) {
    return PPUBaseTargetLowering::LowerReturn_compute(Chain, CallConv, isVarArg, Outs,
                                             OutVals, DL, DAG);
  }
  ...
  if (!Info->isEntryFunction()) {
    ...
    SDValue ReturnAddrReg = CreateLiveInRegister(
      DAG, &PPU::SReg_64RegClass, TRI->getReturnAddressReg(MF), MVT::i64);

    SDValue ReturnAddrVirtualReg = DAG.getRegister(
        MF.getRegInfo().createVirtualRegister(&PPU::CCR_SPR_64RegClass),
        MVT::i64);
    Chain =
  ...
  unsigned Opc = PPUISD::ENDPGM;
  if (!IsWaveEnd)
    Opc = IsShader ? PPUISD::RETURN_TO_EPILOG : PPUISD::RET_FLAG;
  return DAG.getNode(Opc, DL, MVT::Other, RetOps);
}

SDValue PPUTargetLowering::LowerCallResult(
...
void PPUTargetLowering::passSpecialInputs(
  ...
  auto &ArgUsageInfo =
    DAG.getPass()->getAnalysis<PPUArgumentUsageInfo>();
  const PPUFunctionArgInfo &CalleeArgInfo
    = ArgUsageInfo.lookupFuncArgInfo(*CalleeFunc);

  const PPUFunctionArgInfo &CallerArgInfo = Info.getArgInfo();

  // TODO: Unify with private memory register handling. This is complicated by
  // the fact that at least in kernels, the input argument is not necessarily
  // in the same location as the input.
  PPUFunctionArgInfo::PreloadedValue InputRegs[] = {
    PPUFunctionArgInfo::DISPATCH_PTR,
    PPUFunctionArgInfo::QUEUE_PTR,
    PPUFunctionArgInfo::KERNARG_SEGMENT_PTR,
    PPUFunctionArgInfo::DISPATCH_ID,
  ...
  for (auto InputID : InputRegs) {
    ...
    if (IncomingArg) {
      ...
      assert(InputID == PPUFunctionArgInfo::IMPLICIT_ARG_PTR);
  ...
  std::tie(OutgoingArg, ArgRC) =
    CalleeArgInfo.getPreloadedValue(PPUFunctionArgInfo::WORKITEM_ID_X);
  if (!OutgoingArg)
    std::tie(OutgoingArg, ArgRC) =
      CalleeArgInfo.getPreloadedValue(PPUFunctionArgInfo::WORKITEM_ID_Y);
  if (!OutgoingArg)
    std::tie(OutgoingArg, ArgRC) =
      CalleeArgInfo.getPreloadedValue(PPUFunctionArgInfo::WORKITEM_ID_Z);
  if (!OutgoingArg)
    return;

  const ArgDescriptor *IncomingArgX
    = CallerArgInfo.getPreloadedValue(PPUFunctionArgInfo::WORKITEM_ID_X).first;
  const ArgDescriptor *IncomingArgY
    = CallerArgInfo.getPreloadedValue(PPUFunctionArgInfo::WORKITEM_ID_Y).first;
  const ArgDescriptor *IncomingArgZ
    = CallerArgInfo.getPreloadedValue(PPUFunctionArgInfo::WORKITEM_ID_Z).first;
...
bool PPUTargetLowering::isEligibleForTailCallOptimization(
...
bool PPUTargetLowering::mayBeEmittedAsTailCall(const CallInst *CI) const {
  if (!CI->isTailCall())
    return false;

  const Function *ParentFn = CI->getParent()->getParent();
  if (PPU::isEntryFunctionCC(ParentFn->getCallingConv()))
    return false;
...
SDValue PPUTargetLowering::LowerCall(CallLoweringInfo &CLI,
  ...
  if (PPU::isShader(MF.getFunction().getCallingConv())) {
  ...
  if (IsTailCall) {
    MFI.setHasTailCall();
    return DAG.getNode(PPUISD::TC_RETURN, DL, NodeTys, Ops);
  }

  // Returns a chain and a flag for retval copy to use.
  SDValue Call = DAG.getNode(PPUISD::CALL, DL, NodeTys, Ops);
...
unsigned PPUTargetLowering::getRegisterByName(const char* RegName, EVT VT,
  ...
  unsigned Reg = StringSwitch<unsigned>(RegName)
    ...
    .Default(PPU::NoRegister);

  if (Reg == PPU::NoRegister) {
  ...
  if (!Subtarget->hasFlatScrRegister() &&
       Subtarget->getRegisterInfo()->regsOverlap(Reg, PPU::FLAT_SCR)) {
...
// proper terminator.
MachineBasicBlock *PPUTargetLowering::splitKillBlock(MachineInstr &MI,
...
void PPUTargetLowering::bundleInstWithWaitcnt(MachineInstr &MI) const {
  MachineBasicBlock *MBB = MI.getParent();
  const PPUInstrInfo *TII = getSubtarget()->getInstrInfo();
  auto I = MI.getIterator();
  auto E = std::next(I);

  BuildMI(*MBB, E, MI.getDebugLoc(), TII->get(PPU::S_WAITCNT))
    .addImm(0);
...
/* TODO 
MachineBasicBlock *
PPUTargetLowering::emitGWSMemViolTestLoop(MachineInstr &MI,
                                         MachineBasicBlock *BB) const {
  ...
  if (MachineOperand *Src = TII->getNamedOperand(MI, PPU::OpName::data0))
    Src->setIsKill(false);
  ...
  const unsigned EncodedReg = PPU::Hwreg::encodeHwreg(
    PPU::Hwreg::ID_TRAPSTS, PPU::Hwreg::OFFSET_MEM_VIOL, 1);

  // Clear TRAP_STS.MEM_VIOL
  BuildMI(*LoopBB, LoopBB->begin(), DL, TII->get(PPU::S_SETREG_IMM32_B32))
    .addImm(0)
    .addImm(EncodedReg);
  ...
  BuildMI(*LoopBB, I, DL, TII->get(PPU::S_GETREG_B32), Reg)
    .addImm(EncodedReg);

  // FIXME: Do we need to use an isel pseudo that may clobber scc?
  BuildMI(*LoopBB, I, DL, TII->get(PPU::S_CMP_LG_U32))
    .addReg(Reg, RegState::Kill)
    .addImm(0);
  BuildMI(*LoopBB, I, DL, TII->get(PPU::S_CBRANCH_SCC1))
    .addMBB(LoopBB);
...
static MachineBasicBlock::iterator emitLoadM0FromVGPRLoop(
  ...
  BuildMI(LoopBB, I, DL, TII->get(PPU::S_AND_SAVETMSK_B32),
          NewExec)
  ...
  if (UseGPRIdxMode) {
    ...
    if (Offset == 0) {
      ...
      IdxReg = MRI.createVirtualRegister(&PPU::SPR_32RegClass);
      BuildMI(LoopBB, I, DL, TII->get(PPU::S_ADD_I32), IdxReg)
...
static MachineBasicBlock::iterator loadM0FromVGPR(const PPUInstrInfo *TII,
  ...
  const auto *BoolXExecRC = TRI->getRegClass(PPU::SReg_1RegClassID);
...
static unsigned getMOVRELDPseudo(const PPURegisterInfo &TRI,
  ...
  case 32: // 4 bytes
    return PPU::V_MOVRELD_B32_V1;
  case 64: // 8 bytes
    return PPU::V_MOVRELD_B32_V2;
  case 128: // 16 bytes
    return PPU::V_MOVRELD_B32_V4;
  case 256: // 32 bytes
    return PPU::V_MOVRELD_B32_V8;
  case 512: // 64 bytes
    return PPU::V_MOVRELD_B32_V16;
  default:
...
/*  case PPU::GET_GROUPSTATICSIZE: {
    ...
    BuildMI(*BB, MI, DL, TII->get(PPU::S_MOV_B32))
        .add(MI.getOperand(0))
  ...
  case PPU::V_CNDMASK_B64_PSEUDO: {
    ...
    BuildMI(*BB, MI, DL, TII->get(PPU::COPY), SrcCondCopy)
      .addReg(SrcCond);
    BuildMI(*BB, MI, DL, TII->get(PPU::V_CNDMASK_B32_e64), DstLo)
      .addImm(0)
      .addReg(Src0, 0, PPU::sub0)
      .addImm(0)
      .addReg(Src1, 0, PPU::sub0)
      .addReg(SrcCondCopy);
    BuildMI(*BB, MI, DL, TII->get(PPU::V_CNDMASK_B32_e64), DstHi)
      .addImm(0)
      .addReg(Src0, 0, PPU::sub1)
      .addImm(0)
      .addReg(Src1, 0, PPU::sub1)
      .addReg(SrcCondCopy);

    BuildMI(*BB, MI, DL, TII->get(PPU::REG_SEQUENCE), Dst)
      .addReg(DstLo)
      .addImm(PPU::sub0)
      .addReg(DstHi)
      .addImm(PPU::sub1);
  ...
  case PPU::SI_BR_UNDEF: {
    const PPUInstrInfo *TII = getSubtarget()->getInstrInfo();
    const DebugLoc &DL = MI.getDebugLoc();
    MachineInstr *Br = BuildMI(*BB, MI, DL, TII->get(PPU::S_CBRANCH_SCC1))
                           .add(MI.getOperand(0));
  ...
  /*case PPU::SI_CALL_ISEL: {
    ...
    MIB = BuildMI(*BB, MI, DL, TII->get(PPU::SI_CALL), ReturnAddrReg);
...
/*
  ...
  case PPU::DS_GWS_SEMA_P:
  case PPU::DS_GWS_SEMA_RELEASE_ALL:
  case PPU::DS_GWS_BARRIER:
...
bool PPUTargetLowering::hasBitPreservingFPLogic(EVT VT) const {
  return isTypeLegal(VT.getScalarType());
}

bool PPUTargetLowering::enableAggressiveFMAFusion(EVT VT) const {
...
EVT PPUTargetLowering::getSetCCResultType(const DataLayout &DL, LLVMContext &Ctx,
                                         EVT VT) const {
  if (!VT.isVector()) {
    return MVT::i1;
  }
...
MVT PPUTargetLowering::getScalarShiftAmountTy(const DataLayout &, EVT VT) const {
...
SDValue PPUTargetLowering::splitUnaryVectorOp(SDValue Op,
                                             SelectionDAG &DAG) const {
...
// Work around LegalizeDAG doing the wrong thing and fully scalarizing if the
// wider vector type is legal.
SDValue PPUTargetLowering::splitBinaryVectorOp(SDValue Op,
                                              SelectionDAG &DAG) const {
  unsigned Opc = Op.getOpcode();
  EVT VT = Op.getValueType();
...
SDValue PPUTargetLowering::splitTernaryVectorOp(SDValue Op,
                                              SelectionDAG &DAG) const {
  unsigned Opc = Op.getOpcode();
  EVT VT = Op.getValueType();
  assert(VT == MVT::v4i16 || VT == MVT::v4f16);
...
SDValue PPUTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  ...
  switch (Op.getOpcode()) {
  default: return PPUBaseTargetLowering::LowerOperation_compute(Op, DAG);
...
SDValue PPUTargetLowering::adjustLoadValueType(unsigned Opcode,
                                              MemSDNode *M,
                                              SelectionDAG &DAG,
...
SDValue PPUTargetLowering::lowerIntrinsicLoad(MemSDNode *M, bool IsFormat,
                                             SelectionDAG &DAG,
  ...
  unsigned Opc =
      IsFormat ? PPUISD::BUFFER_LOAD_FORMAT : PPUISD::BUFFER_LOAD;

  if (IsD16) {
    return adjustLoadValueType(PPUISD::BUFFER_LOAD_FORMAT_D16, M, DAG, Ops);
  }
...
static SDValue lowerICMPIntrinsic(const PPUTargetLowering &TLI,
  ...
  SDValue SetCC = DAG.getNode(PPUISD::SETCC, DL, CCVT, LHS, RHS,
...
static SDValue lowerFCMPIntrinsic(const PPUTargetLowering &TLI,
                                  SDNode *N, SelectionDAG &DAG) {
  EVT VT = N->getValueType(0);
  const auto *CD = cast<ConstantSDNode>(N->getOperand(3));
  ...
  unsigned WavefrontSize = TLI.getSubtarget()->getWavefrontSize();
  EVT CCVT = EVT::getIntegerVT(*DAG.getContext(), WavefrontSize);
  SDValue SetCC = DAG.getNode(PPUISD::SETCC, SL, CCVT, Src0,
...
void PPUTargetLowering::ReplaceNodeResults(SDNode *N,
...
unsigned PPUTargetLowering::isCFIntrinsic(const SDNode *Intr) const {
  if (Intr->getOpcode() == ISD::INTRINSIC_W_CHAIN) {
    switch (cast<ConstantSDNode>(Intr->getOperand(1))->getZExtValue()) {
    case Intrinsic::ppu_if:
      return PPUISD::IF;
    case Intrinsic::ppu_else:
      return PPUISD::ELSE;
    case Intrinsic::ppu_loop:
      return PPUISD::LOOP;
    case Intrinsic::ppu_end_cf:
...
bool PPUTargetLowering::shouldEmitFixup(const GlobalValue *GV) const {
  const Triple &TT = getTargetMachine().getTargetTriple();
  return (GV->getType()->getAddressSpace() == AMDGPUAS::CONSTANT_ADDRESS ||
          GV->getType()->getAddressSpace() == AMDGPUAS::CONSTANT_ADDRESS_32BIT) &&
         PPU::shouldEmitConstantsToTextSection(TT);
}

bool PPUTargetLowering::shouldEmitGOTReloc(const GlobalValue *GV) const {
...
bool PPUTargetLowering::shouldEmitPCReloc(const GlobalValue *GV) const {
  return !shouldEmitFixup(GV) && !shouldEmitGOTReloc(GV);
}

/// This transforms the control flow intrinsics to get the branch destination as
/// last parameter, also switches branch target with BR if the need arise
SDValue PPUTargetLowering::LowerBRCOND(SDValue BRCOND,
                                      SelectionDAG &DAG) const {
  SDLoc DL(BRCOND);
  ...
  // eg: i1,ch = llvm.ppu.loop t0, TargetConstant:i32<6271>, t3
  // =>     t9: ch = llvm.ppu.loop t0, TargetConstant:i32<6271>, t3, BasicBlock:ch<bb1 0x7fee5286d088>
...
SDValue PPUTargetLowering::LowerRETURNADDR(SDValue Op,
                                          SelectionDAG &DAG) const {
...
SDValue PPUTargetLowering::lowerFP_ROUND(SDValue Op, SelectionDAG &DAG) const {
...
SDValue PPUTargetLowering::lowerFMINNUM_FMAXNUM(SDValue Op,
                                               SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
...
SDValue PPUTargetLowering::lowerTRAP(SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
  SDValue Chain = Op.getOperand(0);

  if (Subtarget->getTrapHandlerAbi() != PPUSubtarget::TrapHandlerAbiHsa ||
      !Subtarget->isTrapHandlerEnabled())
    return DAG.getNode(PPUISD::ENDPGM, SL, MVT::Other, Chain);
  ...
  SDValue Ops[] = {
    ToReg,
    DAG.getTargetConstant(PPUSubtarget::TrapIDLLVMTrap, SL, MVT::i16),
    SGPR01,
    ToReg.getValue(1)
  };
...
SDValue PPUTargetLowering::lowerDEBUGTRAP(SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
  SDValue Chain = Op.getOperand(0);
  MachineFunction &MF = DAG.getMachineFunction();
  ...
  SDValue Ops[] = {
    Chain,
    DAG.getTargetConstant(PPUSubtarget::TrapIDLLVMDebugTrap, SL, MVT::i16)
  };
  return DAG.getNode(PPUISD::TRAP, SL, MVT::Other, Ops);
}
...
SDValue PPUTargetLowering::getSegmentAperture(unsigned AS, const SDLoc &DL,
  ...
  if (Subtarget->hasApertureRegs()) {
    unsigned Offset = AS == AMDGPUAS::LOCAL_ADDRESS ?
        PPU::Hwreg::OFFSET_SRC_SHARED_BASE :
        PPU::Hwreg::OFFSET_SRC_PRIVATE_BASE;
    unsigned WidthM1 = AS == AMDGPUAS::LOCAL_ADDRESS ?
        PPU::Hwreg::WIDTH_M1_SRC_SHARED_BASE :
        PPU::Hwreg::WIDTH_M1_SRC_PRIVATE_BASE;
    unsigned Encoding =
        PPU::Hwreg::ID_MEM_BASES << PPU::Hwreg::ID_SHIFT_ |
        Offset << PPU::Hwreg::OFFSET_SHIFT_ |
        WidthM1 << PPU::Hwreg::WIDTH_M1_SHIFT_;

    SDValue EncodingImm = DAG.getTargetConstant(Encoding, DL, MVT::i16);
    SDValue ApertureReg = SDValue(
        DAG.getMachineNode(PPU::S_GETREG_B32, DL, MVT::i32, EncodingImm), 0);
...
SDValue PPUTargetLowering::lowerADDRSPACECAST(SDValue Op,
  ...
  const PPUTargetMachine &TM =
    static_cast<const PPUTargetMachine &>(getTargetMachine());
...
SDValue PPUTargetLowering::lowerINSERT_VECTOR_ELT(SDValue Op,
                                                 SelectionDAG &DAG) const {
  SDValue Vec = Op.getOperand(0);
...
SDValue PPUTargetLowering::lowerVECTOR_SHUFFLE(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDLoc SL(Op);
  EVT ResultVT = Op.getValueType();
...
bool
PPUTargetLowering::isOffsetFoldingLegal(const GlobalAddressSDNode *GA) const {
...
SDValue PPUTargetLowering::LowerGlobalAddress(PPUMachineFunction *MFI,
                                             SDValue Op,
                                             SelectionDAG &DAG) const {
  ...
  if (GSD->getAddressSpace() == AMDGPUAS::LOCAL_ADDRESS) {
    SDValue GA = DAG.getTargetGlobalAddress(GV, DL, MVT::i32, GSD->getOffset(),
                                            PPUInstrInfo::MO_ABS32_LO);
    return DAG.getNode(PPUISD::LDS, DL, MVT::i32, GA);
  }
...
SDValue PPUTargetLowering::copyToM0(SelectionDAG &DAG, SDValue Chain,
  ...
  SDNode *M0 = DAG.getMachineNode(PPU::SI_INIT_M0, DL, MVT::Other, MVT::Glue,
                                  V, Chain);
  return SDValue(M0, 0);
}

SDValue PPUTargetLowering::lowerImplicitZextParam(SelectionDAG &DAG,
                                                 SDValue Op,
                                                 MVT VT,
...
SDValue PPUTargetLowering::lowerImage(SDValue Op,
  ...
  const PPU::MIMGBaseOpcodeInfo *BaseOpcode =
      PPU::getMIMGBaseOpcodeInfo(Intr->BaseOpcode);
  const PPU::MIMGDimInfo *DimInfo = PPU::getMIMGDimInfo(Intr->Dim);
  const PPU::MIMGLZMappingInfo *LZMappingInfo =
      PPU::getMIMGLZMappingInfo(Intr->BaseOpcode);
  const PPU::MIMGMIPMappingInfo *MIPMappingInfo =
      PPU::getMIMGMIPMappingInfo(Intr->BaseOpcode);
  ...
  if (((VAddrScalarVT == MVT::f16) || (VAddrScalarVT == MVT::i16)) &&
      ST->hasFeature(PPU::FeatureR128A16)) {
    IsA16 = true;
  ...
  bool UseNSA =
      ST->hasFeature(PPU::FeatureNSAEncoding) && VAddrs.size() >= 3;
  SDValue VAddr;
  if (!UseNSA)
  ...
  Ops.push_back(IsA16 &&  // a16 or r128
                ST->hasFeature(PPU::FeatureR128A16) ? True : False);
  ...
  if (IsGFX10) {
    Opcode = PPU::getMIMGOpcode(IntrOpcode,
                                   UseNSA ? PPU::MIMGEncGfx10NSA
                                          : PPU::MIMGEncGfx10Default,
                                   NumVDataDwords, NumVAddrDwords);
  } else {
    if (Subtarget->getGeneration() >= PPUSubtarget::VOLCANIC_ISLANDS)
      Opcode = PPU::getMIMGOpcode(IntrOpcode, PPU::MIMGEncGfx8,
                                     NumVDataDwords, NumVAddrDwords);
    if (Opcode == -1)
      Opcode = PPU::getMIMGOpcode(IntrOpcode, PPU::MIMGEncGfx6,
                                     NumVDataDwords, NumVAddrDwords);
  }
  assert(Opcode != -1);
...
SDValue PPUTargetLowering::lowerSBuffer(EVT VT, SDLoc DL, SDValue Rsrc,
  ...
  if (!Offset->isDivergent()) {
    ...
    return DAG.getMemIntrinsicNode(PPUISD::SBUFFER_LOAD, DL,
  ...
  for (unsigned i = 0; i < NumLoads; ++i) {
    ...
    Loads.push_back(DAG.getMemIntrinsicNode(PPUISD::BUFFER_LOAD, DL, VTList,
                                            Ops, LoadVT, MMO));
  }
...
SDValue PPUTargetLowering::LowerINTRINSIC_WO_CHAIN(SDValue Op,
                                                  SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  auto MFI = MF.getInfo<PPUMachineFunctionInfo>();
  ...
  */
  ...
  case Intrinsic::ppu_queue_ptr: {
    ...
    auto RegID = IntrinsicID == Intrinsic::ppu_dispatch_ptr ?
      PPUFunctionArgInfo::DISPATCH_PTR : PPUFunctionArgInfo::QUEUE_PTR;
  ...
  case Intrinsic::ppu_dispatch_id: {
    return getPreloadedValue(DAG, *MFI, VT, PPUFunctionArgInfo::DISPATCH_ID);
  }
  case Intrinsic::ppu_rcp:
  ...
  case Intrinsic::ppu_rsq_legacy:
    ...
    return DAG.getNode(PPUISD::RSQ_LEGACY, DL, VT, Op.getOperand(1));
  case Intrinsic::ppu_rcp_legacy:
    if (Subtarget->getGeneration() >= PPUSubtarget::VOLCANIC_ISLANDS)
      return emitRemovedIntrinsicError(DAG, DL, VT);
    return DAG.getNode(PPUISD::RCP_LEGACY, DL, VT, Op.getOperand(1));
  case Intrinsic::ppu_rsq_clamp: {
    if (Subtarget->getGeneration() < PPUSubtarget::VOLCANIC_ISLANDS)
      return DAG.getNode(PPUISD::RSQ_CLAMP, DL, VT, Op.getOperand(1));

    Type *Type = VT.getTypeForEVT(*DAG.getContext());
    APFloat Max = APFloat::getLargest(Type->getFltSemantics());
    APFloat Min = APFloat::getLargest(Type->getFltSemantics(), true);

    SDValue Rsq = DAG.getNode(PPUISD::RSQ, DL, VT, Op.getOperand(1));
  ...
  */
  ...
  case Intrinsic::ppu_interp_mov: {
    ...
    return DAG.getNode(PPUISD::INTERP_MOV, DL, MVT::f32, Op.getOperand(1),
                       Op.getOperand(2), Op.getOperand(3), Glue);
  }
  case Intrinsic::ppu_interp_p1: {
    SDValue M0 = copyToM0(DAG, DAG.getEntryNode(), DL, Op.getOperand(4));
    SDValue Glue = M0.getValue(1);
    return DAG.getNode(PPUISD::INTERP_P1, DL, MVT::f32, Op.getOperand(1),
                       Op.getOperand(2), Op.getOperand(3), Glue);
  }
  case Intrinsic::ppu_interp_p2: {
    SDValue M0 = copyToM0(DAG, DAG.getEntryNode(), DL, Op.getOperand(5));
    SDValue Glue = SDValue(M0.getNode(), 1);
    return DAG.getNode(PPUISD::INTERP_P2, DL, MVT::f32, Op.getOperand(1),
                       Op.getOperand(2), Op.getOperand(3), Op.getOperand(4),
                       Glue);
  }
  case Intrinsic::ppu_interp_p1_f16: {
    SDValue M0 = copyToM0(DAG, DAG.getEntryNode(), DL, Op.getOperand(5));
    SDValue Glue = M0.getValue(1);
    if (getSubtarget()->getLDSBankCount() == 16) {
      // 16 bank LDS
      SDValue S = DAG.getNode(PPUISD::INTERP_MOV, DL, MVT::f32,
      ...
      return DAG.getNode(PPUISD::INTERP_P1LV_F16, DL, MVT::f32, Ops);
    } else {
      // 32 bank LDS
      SDValue Ops[] = {
        Op.getOperand(1), // Src0
      ...
      return DAG.getNode(PPUISD::INTERP_P1LL_F16, DL, MVT::f32, Ops);
    }
  }
  case Intrinsic::ppu_interp_p2_f16: {
    ...
    return DAG.getNode(PPUISD::INTERP_P2_F16, DL, MVT::f16, Ops);
  }
  case Intrinsic::ppu_sin:
    return DAG.getNode(PPUISD::SIN_HW, DL, VT, Op.getOperand(1));
...
/*
  ...
  case Intrinsic::ppu_ldexp:
    return DAG.getNode(PPUISD::LDEXP, DL, VT,
                       Op.getOperand(1), Op.getOperand(2));

  case Intrinsic::ppu_fract:
    return DAG.getNode(PPUISD::FRACT, DL, VT, Op.getOperand(1));

  case Intrinsic::ppu_class:
    return DAG.getNode(PPUISD::FP_CLASS, DL, VT,
                       Op.getOperand(1), Op.getOperand(2));
  case Intrinsic::ppu_div_fmas:
    return DAG.getNode(PPUISD::DIV_FMAS, DL, VT,
                       Op.getOperand(1), Op.getOperand(2), Op.getOperand(3),
                       Op.getOperand(4));

  case Intrinsic::ppu_div_fixup:
    return DAG.getNode(PPUISD::DIV_FIXUP, DL, VT,
                       Op.getOperand(1), Op.getOperand(2), Op.getOperand(3));

  case Intrinsic::ppu_trig_preop:
    return DAG.getNode(PPUISD::TRIG_PREOP, DL, VT,
                       Op.getOperand(1), Op.getOperand(2));
  case Intrinsic::ppu_div_scale: {
...
/*
  ...
  case Intrinsic::ppu_fdot2:
    return DAG.getNode(PPUISD::FDOT2, DL, VT,
                       Op.getOperand(1), Op.getOperand(2), Op.getOperand(3),
                       Op.getOperand(4));
  case Intrinsic::ppu_fmul_legacy:
    return DAG.getNode(PPUISD::FMUL_LEGACY, DL, VT,
                       Op.getOperand(1), Op.getOperand(2));
...
*/
  ...
  case Intrinsic::ppu_cvt_pk_u16: {
    ...
    if (IntrinsicID == Intrinsic::ppu_cvt_pkrtz)
      Opcode = PPUISD::CVT_PKRTZ_F16_F32;
    else if (IntrinsicID == Intrinsic::ppu_cvt_pknorm_i16)
      Opcode = PPUISD::CVT_PKNORM_I16_F32;
    else if (IntrinsicID == Intrinsic::ppu_cvt_pknorm_u16)
      Opcode = PPUISD::CVT_PKNORM_U16_F32;
    else if (IntrinsicID == Intrinsic::ppu_cvt_pk_i16)
      Opcode = PPUISD::CVT_PK_I16_I32;
    else
      Opcode = PPUISD::CVT_PK_U16_U32;
...
*/
  ...
  case Intrinsic::ppu_groupstaticsize: {
    ...
    const GlobalValue *GV =
        M->getNamedValue(Intrinsic::getName(Intrinsic::ppu_groupstaticsize));
...
SDValue PPUTargetLowering::LowerINTRINSIC_W_CHAIN(SDValue Op,
                                                 SelectionDAG &DAG) const {
  ...
  case Intrinsic::ppu_ds_ordered_swap: {
    ...
    if (Subtarget->getGeneration() >= PPUSubtarget::GFX10) {
    ...
    return DAG.getMemIntrinsicNode(PPUISD::DS_ORDERED_COUNT, DL,
  ...
  */
  ...
  case Intrinsic::ppu_ds_fmax: {
    ...
    switch (IntrID) {
    case Intrinsic::ppu_atomic_inc:
      Opc = PPUISD::ATOMIC_INC;
      break;
    case Intrinsic::ppu_atomic_dec:
      Opc = PPUISD::ATOMIC_DEC;
      break;
    case Intrinsic::ppu_ds_fmin:
  ...
  case Intrinsic::ppu_buffer_load:
  case Intrinsic::ppu_buffer_load_format: {
    ...
    unsigned Opc = (IntrID == Intrinsic::ppu_buffer_load) ?
        PPUISD::BUFFER_LOAD : PPUISD::BUFFER_LOAD_FORMAT;
    ...
    if (LoadVT.getScalarType() == MVT::f16)
      return adjustLoadValueType(PPUISD::BUFFER_LOAD_FORMAT_D16,
                                 M, DAG, Ops);
  ...
  case Intrinsic::ppu_raw_buffer_load:
  case Intrinsic::ppu_raw_buffer_load_format: {
    const bool IsFormat = IntrID == Intrinsic::ppu_raw_buffer_load_format;
  ...
  case Intrinsic::ppu_struct_buffer_load:
  case Intrinsic::ppu_struct_buffer_load_format: {
    const bool IsFormat = IntrID == Intrinsic::ppu_struct_buffer_load_format;
  ...
  case Intrinsic::ppu_tbuffer_load: {
    MemSDNode *M = cast<MemSDNode>(Op);
    EVT LoadVT = Op.getValueType();
    ...
    if (LoadVT.getScalarType() == MVT::f16)
      return adjustLoadValueType(PPUISD::TBUFFER_LOAD_FORMAT_D16,
                                 M, DAG, Ops);
    return getMemIntrinsicNode(PPUISD::TBUFFER_LOAD_FORMAT, DL,
                               Op->getVTList(), Ops, LoadVT, M->getMemOperand(),
                               DAG);
  }
  case Intrinsic::ppu_raw_tbuffer_load: {
    MemSDNode *M = cast<MemSDNode>(Op);
    EVT LoadVT = Op.getValueType();
    auto Offsets = splitBufferOffsets(Op.getOperand(3), DAG);
  ...
  case Intrinsic::ppu_struct_tbuffer_load: {
    MemSDNode *M = cast<MemSDNode>(Op);
    EVT LoadVT = Op.getValueType();
    auto Offsets = splitBufferOffsets(Op.getOperand(4), DAG);
  ...
  case Intrinsic::ppu_buffer_atomic_swap:
  case Intrinsic::ppu_buffer_atomic_add:
  case Intrinsic::ppu_buffer_atomic_sub:
  case Intrinsic::ppu_buffer_atomic_smin:
  case Intrinsic::ppu_buffer_atomic_umin:
  case Intrinsic::ppu_buffer_atomic_smax:
  case Intrinsic::ppu_buffer_atomic_umax:
  case Intrinsic::ppu_buffer_atomic_and:
  case Intrinsic::ppu_buffer_atomic_or:
  case Intrinsic::ppu_buffer_atomic_xor: {
    unsigned Slc = cast<ConstantSDNode>(Op.getOperand(6))->getZExtValue();
    unsigned IdxEn = 1;
    ...
    switch (IntrID) {
    case Intrinsic::ppu_buffer_atomic_swap:
      Opcode = PPUISD::BUFFER_ATOMIC_SWAP;
      break;
    case Intrinsic::ppu_buffer_atomic_add:
      Opcode = PPUISD::BUFFER_ATOMIC_ADD;
      break;
    case Intrinsic::ppu_buffer_atomic_sub:
      Opcode = PPUISD::BUFFER_ATOMIC_SUB;
      break;
    case Intrinsic::ppu_buffer_atomic_smin:
      Opcode = PPUISD::BUFFER_ATOMIC_SMIN;
      break;
    case Intrinsic::ppu_buffer_atomic_umin:
      Opcode = PPUISD::BUFFER_ATOMIC_UMIN;
      break;
    case Intrinsic::ppu_buffer_atomic_smax:
      Opcode = PPUISD::BUFFER_ATOMIC_SMAX;
      break;
    case Intrinsic::ppu_buffer_atomic_umax:
      Opcode = PPUISD::BUFFER_ATOMIC_UMAX;
      break;
    case Intrinsic::ppu_buffer_atomic_and:
      Opcode = PPUISD::BUFFER_ATOMIC_AND;
      break;
    case Intrinsic::ppu_buffer_atomic_or:
      Opcode = PPUISD::BUFFER_ATOMIC_OR;
      break;
    case Intrinsic::ppu_buffer_atomic_xor:
      Opcode = PPUISD::BUFFER_ATOMIC_XOR;
      break;
    default:
  ...
  case Intrinsic::ppu_raw_buffer_atomic_swap:
  case Intrinsic::ppu_raw_buffer_atomic_add:
  case Intrinsic::ppu_raw_buffer_atomic_sub:
  case Intrinsic::ppu_raw_buffer_atomic_smin:
  case Intrinsic::ppu_raw_buffer_atomic_umin:
  case Intrinsic::ppu_raw_buffer_atomic_smax:
  case Intrinsic::ppu_raw_buffer_atomic_umax:
  case Intrinsic::ppu_raw_buffer_atomic_and:
  case Intrinsic::ppu_raw_buffer_atomic_or:
  case Intrinsic::ppu_raw_buffer_atomic_xor:
  case Intrinsic::ppu_raw_buffer_atomic_inc:
  case Intrinsic::ppu_raw_buffer_atomic_dec: {
    auto Offsets = splitBufferOffsets(Op.getOperand(4), DAG);
    SDValue Ops[] = {
      Op.getOperand(0), // Chain
      Op.getOperand(2), // vdata
    ...
    case Intrinsic::ppu_raw_buffer_atomic_xor:
      ...
      break;
    case Intrinsic::ppu_raw_buffer_atomic_inc:
      Opcode = PPUISD::BUFFER_ATOMIC_INC;
      break;
    case Intrinsic::ppu_raw_buffer_atomic_dec:
      Opcode = PPUISD::BUFFER_ATOMIC_DEC;
      break;
    default:
      llvm_unreachable("unhandled atomic opcode");
    }
  ...
  case Intrinsic::ppu_struct_buffer_atomic_swap:
  case Intrinsic::ppu_struct_buffer_atomic_add:
  case Intrinsic::ppu_struct_buffer_atomic_sub:
  case Intrinsic::ppu_struct_buffer_atomic_smin:
  case Intrinsic::ppu_struct_buffer_atomic_umin:
  case Intrinsic::ppu_struct_buffer_atomic_smax:
  case Intrinsic::ppu_struct_buffer_atomic_umax:
  case Intrinsic::ppu_struct_buffer_atomic_and:
  case Intrinsic::ppu_struct_buffer_atomic_or:
  case Intrinsic::ppu_struct_buffer_atomic_xor:
  case Intrinsic::ppu_struct_buffer_atomic_inc:
  case Intrinsic::ppu_struct_buffer_atomic_dec: {
  ...
  case Intrinsic::ppu_buffer_atomic_cmpswap: {
    ...
    return DAG.getMemIntrinsicNode(PPUISD::BUFFER_ATOMIC_CMPSWAP, DL,
                                   Op->getVTList(), Ops, VT, M->getMemOperand());
  }
  case Intrinsic::ppu_raw_buffer_atomic_cmpswap: {
    auto Offsets = splitBufferOffsets(Op.getOperand(5), DAG);
    SDValue Ops[] = {
      Op.getOperand(0), // Chain
      Op.getOperand(2), // src
  ...
  case Intrinsic::ppu_struct_buffer_atomic_cmpswap: {
...
SDValue PPUTargetLowering::handleD16VData(SDValue VData,
                                         SelectionDAG &DAG) const {
...
SDValue PPUTargetLowering::LowerINTRINSIC_VOID(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDLoc DL(Op);
  SDValue Chain = Op.getOperand(0);
  ...
  */
  ...
  case Intrinsic::ppu_barrier: {
    if (getTargetMachine().getOptLevel() > CodeGenOpt::None) {
      ...
      if (WGSize <= ST.getWavefrontSize())
        return SDValue(DAG.getMachineNode(PPU::WAVE_BARRIER, DL, MVT::Other,
                                          Op.getOperand(0)), 0);
    }
    return SDValue();
  };
  case Intrinsic::ppu_tbuffer_store: {
    ...
    unsigned Opc = IsD16 ? PPUISD::TBUFFER_STORE_FORMAT_D16 :
                           PPUISD::TBUFFER_STORE_FORMAT;
    MemSDNode *M = cast<MemSDNode>(Op);
    return DAG.getMemIntrinsicNode(Opc, DL, Op->getVTList(), Ops,
                                   M->getMemoryVT(), M->getMemOperand());
  }

  case Intrinsic::ppu_struct_tbuffer_store: {
    SDValue VData = Op.getOperand(2);
    bool IsD16 = (VData.getValueType().getScalarType() == MVT::f16);
    if (IsD16)
      VData = handleD16VData(VData, DAG);
  ...
  case Intrinsic::ppu_raw_tbuffer_store: {
    SDValue VData = Op.getOperand(2);
    bool IsD16 = (VData.getValueType().getScalarType() == MVT::f16);
    if (IsD16)
      VData = handleD16VData(VData, DAG);
  ...
  case Intrinsic::ppu_buffer_store:
  case Intrinsic::ppu_buffer_store_format: {
    SDValue VData = Op.getOperand(2);
    bool IsD16 = (VData.getValueType().getScalarType() == MVT::f16);
    if (IsD16)
      VData = handleD16VData(VData, DAG);
    ...
    setBufferOffsets(Op.getOperand(5), DAG, &Ops[4]);
    unsigned Opc = IntrinsicID == Intrinsic::ppu_buffer_store ?
                   PPUISD::BUFFER_STORE : PPUISD::BUFFER_STORE_FORMAT;
    Opc = IsD16 ? PPUISD::BUFFER_STORE_FORMAT_D16 : Opc;
    MemSDNode *M = cast<MemSDNode>(Op);
  ...
  case Intrinsic::ppu_raw_buffer_store:
  case Intrinsic::ppu_raw_buffer_store_format: {
    const bool IsFormat =
        IntrinsicID == Intrinsic::ppu_raw_buffer_store_format;
    ...
    unsigned Opc =
        IsFormat ? PPUISD::BUFFER_STORE_FORMAT : PPUISD::BUFFER_STORE;
    Opc = IsD16 ? PPUISD::BUFFER_STORE_FORMAT_D16 : Opc;
    MemSDNode *M = cast<MemSDNode>(Op);
  ...
  case Intrinsic::ppu_struct_buffer_store:
  case Intrinsic::ppu_struct_buffer_store_format: {
    const bool IsFormat =
        IntrinsicID == Intrinsic::ppu_struct_buffer_store_format;
    ...
    unsigned Opc = IntrinsicID == Intrinsic::ppu_struct_buffer_store ?
                   PPUISD::BUFFER_STORE : PPUISD::BUFFER_STORE_FORMAT;
    Opc = IsD16 ? PPUISD::BUFFER_STORE_FORMAT_D16 : Opc;
    MemSDNode *M = cast<MemSDNode>(Op);
  ...
  case Intrinsic::ppu_buffer_atomic_fadd: {
    ...
    auto *M = cast<MemSDNode>(Op);
    unsigned Opcode = VT.isVector() ? PPUISD::BUFFER_ATOMIC_PK_FADD
                                    : PPUISD::BUFFER_ATOMIC_FADD;
  ...
  case Intrinsic::ppu_global_atomic_fadd: {
    ...
    auto *M = cast<MemSDNode>(Op);
    unsigned Opcode = VT.isVector() ? PPUISD::ATOMIC_PK_FADD
                                    : PPUISD::ATOMIC_FADD;
  ...
  case Intrinsic::ppu_end_cf:
    return SDValue(DAG.getMachineNode(PPU::SI_END_CF, DL, MVT::Other,
...
std::pair<SDValue, SDValue> PPUTargetLowering::splitBufferOffsets(
...
// Analyze a combined offset from an ppu_buffer_ intrinsic and store the
// three offsets (voffset, soffset and instoffset) into the SDValue[3] array
// pointed to by Offsets.
void PPUTargetLowering::setBufferOffsets(SDValue CombinedOffset,
  ...
  if (auto C = dyn_cast<ConstantSDNode>(CombinedOffset)) {
    ...
    if (PPU::splitMUBUFOffset(Imm, SOffset, ImmOffset, Subtarget, Align)) {
  ...
  if (DAG.isBaseWithConstantOffset(CombinedOffset)) {
    ...
    if (Offset >= 0 && PPU::splitMUBUFOffset(Offset, SOffset, ImmOffset,
                                                Subtarget, Align)) {
      Offsets[0] = N0;
      Offsets[1] = DAG.getConstant(SOffset, DL, MVT::i32);
      Offsets[2] = DAG.getConstant(ImmOffset, DL, MVT::i32);
...
SDValue PPUTargetLowering::handleByteShortBufferLoads(SelectionDAG &DAG,
                                                     EVT LoadVT, SDLoc DL,
                                                     ArrayRef<SDValue> Ops,
                                                     MemSDNode *M) const {
  EVT IntVT = LoadVT.changeTypeToInteger();
  unsigned Opc = (LoadVT.getScalarType() == MVT::i8) ?
         PPUISD::BUFFER_LOAD_UBYTE : PPUISD::BUFFER_LOAD_USHORT;
...
SDValue PPUTargetLowering::handleByteShortBufferStores(SelectionDAG &DAG,
  ...
  unsigned Opc = (VDataType == MVT::i8) ? PPUISD::BUFFER_STORE_BYTE :
                                 PPUISD::BUFFER_STORE_SHORT;
...
SDValue PPUTargetLowering::widenLoad(LoadSDNode *Ld, DAGCombinerInfo &DCI) const {
...
SDValue PPUTargetLowering::lowerFastUnsafeFDIV(SDValue Op,
  ...
  if (Unsafe) {
    ...
    // x / y -> x * (1.0 / y)
    SDValue Recip = DAG.getNode(PPUISD::RCP, SL, VT, RHS);
...
SDValue PPUTargetLowering::lowerFDIV_FAST(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue r0 = DAG.getNode(PPUISD::RCP, SL, MVT::f32, r1);
...
static const SDValue getSPDenormModeValue(int SPDenormMode, SelectionDAG &DAG,
                                          const SDLoc &SL, const PPUSubtarget *ST) {
...
SDValue PPUTargetLowering::LowerFDIV32(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue DenominatorScaled = DAG.getNode(PPUISD::DIV_SCALE, SL, ScaleVT,
                                          RHS, RHS, LHS);
  SDValue NumeratorScaled = DAG.getNode(PPUISD::DIV_SCALE, SL, ScaleVT,
                                        LHS, RHS, LHS);

  // Denominator is scaled to not be denormal, so using rcp is ok.
  SDValue ApproxRcp = DAG.getNode(PPUISD::RCP, SL, MVT::f32,
                                  DenominatorScaled);
  ...
  return DAG.getNode(PPUISD::DIV_FIXUP, SL, MVT::f32, Fmas, RHS, LHS);
}
...
SDValue PPUTargetLowering::LowerSTORE(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue PPUTargetLowering::LowerTrig(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  EVT VT = Op.getValueType();
  ...
  if (Subtarget->hasTrigReducedRange()) {
    ...
    TrigVal = DAG.getNode(PPUISD::FRACT, DL, VT, MulVal);
  } else {
  ...
  switch (Op.getOpcode()) {
  case ISD::FCOS:
    return DAG.getNode(PPUISD::COS_HW, SDLoc(Op), VT, TrigVal);
  case ISD::FSIN:
    return DAG.getNode(PPUISD::SIN_HW, SDLoc(Op), VT, TrigVal);
  default:
    llvm_unreachable("Wrong trig opcode");
  }
}

SDValue PPUTargetLowering::LowerATOMIC_CMP_SWAP(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getMemIntrinsicNode(PPUISD::ATOMIC_CMP_SWAP, DL, Op->getVTList(),
...
//
SDValue PPUTargetLowering::performSHLPtrCombine(SDNode *N,
                                               unsigned AddrSpace,
                                               EVT MemVT,
                                               DAGCombinerInfo &DCI) const {
...
SDValue PPUTargetLowering::performMemSDNodeCombine(MemSDNode *N,
                                                  DAGCombinerInfo &DCI) const {
...
// immediate.
SDValue PPUTargetLowering::splitBinaryBitConstantOp(
  DAGCombinerInfo &DCI,
  const SDLoc &SL,
...
SDValue PPUTargetLowering::performAndCombine(SDNode *N,
                                            DAGCombinerInfo &DCI) const {
...
SDValue PPUTargetLowering::performOrCombine(SDNode *N,
                                           DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  SDValue LHS = N->getOperand(0);
  SDValue RHS = N->getOperand(1);
...
SDValue PPUTargetLowering::performXorCombine(SDNode *N,
                                            DAGCombinerInfo &DCI) const {
  EVT VT = N->getValueType(0);
  if (VT != MVT::i64)
    return SDValue();
...
SDValue PPUTargetLowering::performZeroExtendCombine(SDNode *N,
                                                   DAGCombinerInfo &DCI) const {
  if (!Subtarget->has16BitInsts() ||
...
SDValue PPUTargetLowering::performSignExtendInRegCombine(SDNode *N,
                                                        DAGCombinerInfo &DCI)
                                                        const {
  SDValue Src = N->getOperand(0);
...
SDValue PPUTargetLowering::performClassCombine(SDNode *N,
                                              DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
...
SDValue PPUTargetLowering::performRcpCombine(SDNode *N,
  ...
  return PPUBaseTargetLowering::performRcpCombine(N, DCI);
}

bool PPUTargetLowering::isCanonicalized(SelectionDAG &DAG, SDValue Op,
...
SDValue PPUTargetLowering::getCanonicalConstantFP(
...
SDValue PPUTargetLowering::performIntMed3ImmCombine(
  ...
  unsigned Med3Opc = Signed ? PPUISD::SMED3 : PPUISD::UMED3;
...
SDValue PPUTargetLowering::performFPMed3ImmCombine(SelectionDAG &DAG,
  ...
  if (Info->getMode().DX10Clamp) {
    ...
    if (K1->isExactlyValue(1.0) && K0->isExactlyValue(0.0))
      return DAG.getNode(PPUISD::CLAMP, SL, VT, Op0.getOperand(0));
  }
  ...
  if (VT == MVT::f32 || (VT == MVT::f16 && Subtarget->hasMed3_16())) {
    ...
    if ((!K0->hasOneUse() ||
      ...
      return DAG.getNode(PPUISD::FMED3, SL, K0->getValueType(0),
...
SDValue PPUTargetLowering::performMinMaxCombine(SDNode *N,
  ...
  if (Opc != PPUISD::FMIN_LEGACY && Opc != PPUISD::FMAX_LEGACY &&
      !VT.isVector() &&
...
/* TODO
  ...
  if (((Opc == ISD::FMINNUM && Op0.getOpcode() == ISD::FMAXNUM) ||
       ...
       (Opc == PPUISD::FMIN_LEGACY &&
        Op0.getOpcode() == PPUISD::FMAX_LEGACY)) &&
...
SDValue PPUTargetLowering::performFMed3Combine(SDNode *N,
                                              DAGCombinerInfo &DCI) const {
  EVT VT = N->getValueType(0);
  ...
  if (isClampZeroToOne(Src0, Src1)) {
    ...
    // nans.
    // FIXME: Should this be allowing -0.0?
    return DAG.getNode(PPUISD::CLAMP, SL, VT, Src2);
  }
  ...
  if (Info->getMode().DX10Clamp) {
    ...
    if (isClampZeroToOne(Src1, Src2))
      return DAG.getNode(PPUISD::CLAMP, SL, VT, Src0);
  }

  return SDValue();
}

SDValue PPUTargetLowering::performCvtPkRTZCombine(SDNode *N,
                                                 DAGCombinerInfo &DCI) const {
  SDValue Src0 = N->getOperand(0);
  SDValue Src1 = N->getOperand(1);
...
SDValue PPUTargetLowering::performExtractVectorEltCombine(
...
unsigned PPUTargetLowering::getFusedOpcode(const SelectionDAG &DAG,
                                          const SDNode *N0,
...
SDValue PPUTargetLowering::reassociateScalarOps(SDNode *N,
                                               SelectionDAG &DAG) const {
  EVT VT = N->getValueType(0);
...
static SDValue getMad64_32(SelectionDAG &DAG, const SDLoc &SL,
                           ...
                           bool Signed) {
  unsigned MadOpc = Signed ? PPUISD::MAD_I64_I32 : PPUISD::MAD_U64_U32;
...
SDValue PPUTargetLowering::performFMACombine(SDNode *N,
                                            DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  EVT VT = N->getValueType(0);
  SDLoc SL(N);
...
SDValue PPUTargetLowering::performSetCCCombine(SDNode *N,
                                              DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  SDLoc SL(N);
...
SDValue PPUTargetLowering::performClampCombine(SDNode *N,
                                              DAGCombinerInfo &DCI) const {
  ...
  if (Cmp0 == APFloat::cmpLessThan ||
      (Cmp0 == APFloat::cmpUnordered &&
       MF.getInfo<PPUMachineFunctionInfo>()->getMode().DX10Clamp)) {
...
static unsigned SubIdx2Lane(unsigned Idx) {
  switch (Idx) {
  default: return 0;
  case PPU::sub0: return 0;
  case PPU::sub1: return 1;
  case PPU::sub2: return 2;
  case PPU::sub3: return 3;
  case PPU::sub4: return 4; // Possible with TFE/LWE
  }
}
...
SDNode *PPUTargetLowering::adjustWritemask(MachineSDNode *&Node,
  ...
  int D16Idx = PPU::getNamedOperandIdx(Opcode, PPU::OpName::d16) - 1;
  if (D16Idx >= 0 && Node->getConstantOperandVal(D16Idx))
    return Node; // not implemented for D16

  SDNode *Users[5] = { nullptr };
  unsigned Lane = 0;
  unsigned DmaskIdx = PPU::getNamedOperandIdx(Opcode, PPU::OpName::dmask) - 1;
  unsigned OldDmask = Node->getConstantOperandVal(DmaskIdx);
  unsigned NewDmask = 0;
  unsigned TFEIdx = PPU::getNamedOperandIdx(Opcode, PPU::OpName::tfe) - 1;
  unsigned LWEIdx = PPU::getNamedOperandIdx(Opcode, PPU::OpName::lwe) - 1;
  ...
  int NewOpcode =
      PPU::getMaskedMIMGOp(Node->getMachineOpcode(), NewChannels);
  assert(NewOpcode != -1 &&
  ...
  for (unsigned i = 0, Idx = PPU::sub0; i < 5; ++i) {
    SDNode *User = Users[i];
    if (!User) {
    ...
    switch (Idx) {
    default: break;
    case PPU::sub0: Idx = PPU::sub1; break;
    case PPU::sub1: Idx = PPU::sub2; break;
    case PPU::sub2: Idx = PPU::sub3; break;
    case PPU::sub3: Idx = PPU::sub4; break;
    }
  }
...
SDNode *PPUTargetLowering::legalizeTargetIndependentNode(SDNode *Node,
                                                        SelectionDAG &DAG) const {
  if (Node->getOpcode() == ISD::CopyToReg) {
    ...
    if (SrcVal.getValueType() == MVT::i1 &&
      ...
      SDValue VReg = DAG.getRegister(
        MRI.createVirtualRegister(&PPU::VReg_1RegClass), MVT::i1);
  ...
  for (unsigned i = 0; i < Node->getNumOperands(); ++i) {
    ...
    SDLoc DL(Node);
    Ops.push_back(SDValue(DAG.getMachineNode(PPU::S_MOV_B32, DL,
...
SDNode *PPUTargetLowering::PostISelFolding(MachineSDNode *Node,
                                          SelectionDAG &DAG) const {
  const PPUInstrInfo *TII = getSubtarget()->getInstrInfo();
  unsigned Opcode = Node->getMachineOpcode();
/*
...
*/
  ...
  switch (Opcode) {
  case PPU::V_DIV_SCALE_F32:
  case PPU::V_DIV_SCALE_F64: {
    ...
    if ((Src0.isMachineOpcode() &&
         Src0.getMachineOpcode() != PPU::IMPLICIT_DEF) &&
        (Src0 == Src1 || Src0 == Src2))
      break;
    ...
    if (Src0.isMachineOpcode() &&
      ...
      if (Src1.isMachineOpcode() &&
          Src1.getMachineOpcode() != PPU::IMPLICIT_DEF)
        Src0 = Src1;
      else if (Src2.isMachineOpcode() &&
               Src2.getMachineOpcode() != PPU::IMPLICIT_DEF)
        Src0 = Src2;
      else {
        assert(Src1.getMachineOpcode() == PPU::IMPLICIT_DEF);
        Src0 = UndefReg;
        Src1 = UndefReg;
      }
    } else
  ...
  case PPU::V_PERMLANE16_B32:
  case PPU::V_PERMLANEX16_B32: {
    ...
    if (VDstIn.isMachineOpcode()
        && VDstIn.getMachineOpcode() == PPU::IMPLICIT_DEF)
      break;
...
void PPUTargetLowering::AdjustInstrPostInstrSelection(MachineInstr &MI,
                                                     SDNode *Node) const {
  const PPUInstrInfo *TII = getSubtarget()->getInstrInfo();
  ...
  if (TII->isVOP3(MI.getOpcode())) {
    ...
    if (const MCOperandInfo *OpInfo = MI.getDesc().OpInfo) {
      unsigned Opc = MI.getOpcode();
      const PPURegisterInfo *TRI = Subtarget->getRegisterInfo();
      for (auto I : { PPU::getNamedOperandIdx(Opc, PPU::OpName::src0),
  ...
  int NoRetAtomicOp = PPU::getAtomicNoRetOp(MI.getOpcode());
  if (NoRetAtomicOp != -1) {
    ...
    if ((Node->hasNUsesOfValue(1, 0) &&
         ...
         Node->use_begin()->getMachineOpcode() == PPU::EXTRACT_SUBREG &&
...
MachineSDNode *PPUTargetLowering::wrapAddr64Rsrc(SelectionDAG &DAG,
  ...
  const SDValue Ops1[] = {
    ...
    Ptr,
    DAG.getTargetConstant(PPU::sub0_sub1, DL, MVT::i32),
    SubRegHi,
...
MachineSDNode *PPUTargetLowering::buildRSRC(SelectionDAG &DAG, const SDLoc &DL,
  ...
  if (RsrcDword1) {
    PtrHi = SDValue(DAG.getMachineNode(PPU::S_OR_B32, DL, MVT::i32, PtrHi,
  ...
  const SDValue Ops[] = {
    ...
    DAG.getTargetConstant(PPU::sub1, DL, MVT::i32),
    DataLo,
    DAG.getTargetConstant(PPU::sub2, DL, MVT::i32),
    DataHi,
    DAG.getTargetConstant(PPU::sub3, DL, MVT::i32)
  };
...
PPUTargetLowering::ConstraintType
PPUTargetLowering::getConstraintType(StringRef Constraint) const {
  if (Constraint.size() == 1) {
    switch (Constraint[0]) {
    default: break;
    case 's':
...
// calls are present.
void PPUTargetLowering::finalizeLowering(MachineFunction &MF) const {
  MachineRegisterInfo &MRI = MF.getRegInfo();
  PPUMachineFunctionInfo *Info = MF.getInfo<PPUMachineFunctionInfo>();
  const PPUSubtarget &ST = MF.getSubtarget<PPUSubtarget>();
  const PPURegisterInfo *TRI = Subtarget->getRegisterInfo();
  ...
  if (ST.isWave32() && !MF.empty()) {
    ...
    BuildMI(MBB, I, DL, TII->get(TargetOpcode::IMPLICIT_DEF), PPU::VCC_HI);
...
void PPUTargetLowering::computeKnownBitsForFrameIndex(const SDValue Op,
                                                     KnownBits &Known,
...
unsigned PPUTargetLowering::getPrefLoopAlignment(MachineLoop *ML) const {
  ...
  if (!ML || DisableLoopAlignment ||
      (getSubtarget()->getGeneration() < PPUSubtarget::GFX10) ||
  ...
  for (MachineLoop *P = ML->getParentLoop(); P; P = P->getParentLoop()) {
    if (MachineBasicBlock *Exit = P->getExitBlock()) {
      ...
      if (I != Exit->end() && I->getOpcode() == PPU::S_INST_PREFETCH)
        return CacheLineAlign;
    }
  }
...
bool PPUTargetLowering::isSDNodeSourceOfDivergence(const SDNode * N,
  FunctionLoweringInfo * FLI, LegacyDivergenceAnalysis * KDA) const
{
  switch (N->getOpcode()) {
    case ISD::CopyFromReg:
    {
      ...
      const PPUSubtarget &ST = MF->getSubtarget<PPUSubtarget>();
      const MachineRegisterInfo &MRI = MF->getRegInfo();
      const PPURegisterInfo &TRI = ST.getInstrInfo()->getRegisterInfo();
...
bool PPUTargetLowering::denormalsEnabledForType(EVT VT) const {
...
bool PPUTargetLowering::isKnownNeverNaNForTargetNode(SDValue Op,
                                                    const SelectionDAG &DAG,
                                                    bool SNaN,
                                                    unsigned Depth) const {
  if (Op.getOpcode() == PPUISD::CLAMP) {
    const MachineFunction &MF = DAG.getMachineFunction();
    const PPUMachineFunctionInfo *Info = MF.getInfo<PPUMachineFunctionInfo>();
  ...
  return PPUBaseTargetLowering::isKnownNeverNaNForTargetNode(Op, DAG,
                                                            SNaN, Depth);
}

TargetLowering::AtomicExpansionKind
PPUTargetLowering::shouldExpandAtomicRMWInIR(AtomicRMWInst *RMW) const {
  ...
  return PPUBaseTargetLowering::shouldExpandAtomicRMWInIR(RMW);
}
