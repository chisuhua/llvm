//==- PPUArgumentrUsageInfo.h - Function Arg Usage Info -------*- C++ -*-==//
//
...
#ifndef LLVM_LIB_TARGET_PPU_PPUARGUMENTUSAGEINFO_H
#define LLVM_LIB_TARGET_PPU_PPUARGUMENTUSAGEINFO_H
...
private:
  friend struct PPUFunctionArgInfo;
  friend class PPUArgumentUsageInfo;
...
class PPUArgumentUsageInfo : public ImmutablePass {
private:
  static const PPUFunctionArgInfo ExternFunctionInfo;
  DenseMap<const Function *, PPUFunctionArgInfo> ArgInfoMap;

public:
  static char ID;

  PPUArgumentUsageInfo() : ImmutablePass(ID) { }
  ...
  void setFuncArgInfo(const Function &F, const PPUFunctionArgInfo &ArgInfo) {
    ArgInfoMap[&F] = ArgInfo;
  }

  const PPUFunctionArgInfo &lookupFuncArgInfo(const Function &F) const {
