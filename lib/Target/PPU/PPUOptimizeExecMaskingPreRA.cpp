...
#define DEBUG_TYPE "bi-optimize-exec-masking-pre-ra"

namespace {

class PPUOptimizeExecMaskingPreRA : public MachineFunctionPass {
private:
...
public:
  ...
  PPUOptimizeExecMaskingPreRA() : MachineFunctionPass(ID) {
    initializePPUOptimizeExecMaskingPreRAPass(*PassRegistry::getPassRegistry());
  }

  bool runOnMachineFunction(MachineFunction &MF) override;

  StringRef getPassName() const override {
    return "PPU optimize exec mask operations pre-RA";
  }
...
INITIALIZE_PASS_BEGIN(PPUOptimizeExecMaskingPreRA, DEBUG_TYPE,
                      "PPU optimize exec mask operations pre-RA", false, false)
INITIALIZE_PASS_DEPENDENCY(LiveIntervals)
INITIALIZE_PASS_END(PPUOptimizeExecMaskingPreRA, DEBUG_TYPE,
                    "PPU optimize exec mask operations pre-RA", false, false)

char PPUOptimizeExecMaskingPreRA::ID = 0;

char &llvm::PPUOptimizeExecMaskingPreRAID = PPUOptimizeExecMaskingPreRA::ID;

FunctionPass *llvm::createPPUOptimizeExecMaskingPreRAPass() {
  return new PPUOptimizeExecMaskingPreRA();
}

static bool isEndCF(const MachineInstr& MI, const PPURegisterInfo* TRI,
                    const PPUSubtarget &ST) {
  return MI.getOpcode() == PPU::OR &&
         MI.modifiesRegister(PPU::TMSK, TRI);
}

static bool isFullExecCopy(const MachineInstr& MI, const PPUSubtarget& ST) {

  if (MI.isCopy() && MI.getOperand(1).getReg() == PPU::TMSK) {
...
static unsigned getOrNonExecReg(const MachineInstr &MI,
                                const PPUInstrInfo &TII,
                                const PPUSubtarget& ST) {
    /* FIXME on OpName::src1
  auto Op = TII.getNamedOperand(MI, PPU::OpName::src1);
  if (Op->isReg() && Op->getReg() != PPU::TMSK)
     return Op->getReg();
  Op = TII.getNamedOperand(MI, PPU::OpName::src0);
  if (Op->isReg() && Op->getReg() != PPU::TMSK)
     return Op->getReg();
     */
...
static MachineInstr* getOrExecSource(const MachineInstr &MI,
  ...
  if (SavedExec == PPU::NoRegister)
    return nullptr;
  auto SaveTmskInst = MRI.getUniqueVRegDef(SavedExec);
  if (!SaveTmskInst || !isFullExecCopy(*SaveTmskInst, ST))
    return nullptr;
  return SaveTmskInst;
}

/// Skip over instructions that don't care about the exec mask.
MachineBasicBlock::iterator PPUOptimizeExecMaskingPreRA::skipIgnoreExecInsts(
  MachineBasicBlock::iterator I, MachineBasicBlock::iterator E) const {
  for ( ; I != E; ++I) {
...
PPUOptimizeExecMaskingPreRA::skipIgnoreExecInstsTrivialSucc(
  MachineBasicBlock *&MBB,
  MachineBasicBlock::iterator It) const {
...
static unsigned optimizeVcndVcmpPair(MachineBasicBlock &MBB,
  ...
  const unsigned AndOpc = PPU::AND;
  const unsigned Andn2Opc =PPU::AND; // FIXME PPU::S_ANDN2_B32
  const unsigned CondReg = PPU::VCC;
  const unsigned ExecReg = PPU::TMSK;

  auto I = llvm::find_if(MBB.terminators(), [](const MachineInstr &MI) {
                           unsigned Opc = MI.getOpcode();
                           return Opc == PPU::CBRANCH_VCCZ ||
                                  Opc == PPU::CBRANCH_VCCNZ; });
  if (I == MBB.terminators().end())
    return PPU::NoRegister;

  auto *And = TRI->findReachingDef(CondReg, PPU::NoSubRegister,
                                   *I, MRI, LIS);
  ...
  if (!Cmp || !(Cmp->getOpcode() == PPU::V_CMP_NE_U32_e32 ||
                Cmp->getOpcode() == PPU::V_CMP_NE_U32_e64) ||
      Cmp->getParent() != And->getParent())
    return PPU::NoRegister;

  MachineOperand *Op1 = TII->getNamedOperand(*Cmp, PPU::OpName::src0);
  MachineOperand *Op2 = TII->getNamedOperand(*Cmp, PPU::OpName::src1);
  ...
  if (!Sel || Sel->getOpcode() != PPU::V_CNDMASK_B32_e64)
    return PPU::NoRegister;

  if (TII->hasModifiersSet(*Sel, PPU::OpName::src0_modifiers) ||
      TII->hasModifiersSet(*Sel, PPU::OpName::src1_modifiers))
    return PPU::NoRegister;

  Op1 = TII->getNamedOperand(*Sel, PPU::OpName::src0);
  Op2 = TII->getNamedOperand(*Sel, PPU::OpName::src1);
  MachineOperand *CC = TII->getNamedOperand(*Sel, PPU::OpName::src2);
...
bool PPUOptimizeExecMaskingPreRA::runOnMachineFunction(MachineFunction &MF) {
  ...
  DenseSet<unsigned> RecalcRegs({PPU::TMSK});
...
/*
    if (unsigned Reg = optimizeVcndVcmpPair(MBB, ST, MRI, LIS)) {
      ...
      RecalcRegs.insert(PPU::VCC);
      RecalcRegs.insert(PPU::SCC);
      Changed = true;
    }
*/
    ...
    if (MBB.succ_empty()) {
      ...
      if (Term.getOpcode() != PPU::ENDPGM || Term.getNumOperands() != 1)
        continue;
      ...
      while (!Blocks.empty()) {
        ...
        if (I != E) {
          if (I->isUnconditionalBranch() || I->getOpcode() == PPU::ENDPGM)
            ++I;
          else if (I->isBranch())
            continue;
        }
