...
#define DEBUG_TYPE "ppu-aa"

// Register this pass...
char PPUAAWrapperPass::ID = 0;
char PPUExternalAAWrapper::ID = 0;

INITIALIZE_PASS(PPUAAWrapperPass, "ppu-aa",
                "PPU Address space based Alias Analysis", false, true)

INITIALIZE_PASS(PPUExternalAAWrapper, "ppu-aa-wrapper",
                "PPU Address space based Alias Analysis Wrapper", false, true)


ImmutablePass *llvm::createPPUAAWrapperPass() {
  return new PPUAAWrapperPass();
}

ImmutablePass *llvm::createPPUExternalAAWrapperPass() {
  return new PPUExternalAAWrapper();
}


void PPUAAWrapperPass::getAnalysisUsage(AnalysisUsage &AU) const {
  AU.setPreservesAll();
}
...
AliasResult PPUAAResult::alias(const MemoryLocation &LocA,
...
bool PPUAAResult::pointsToConstantMemory(const MemoryLocation &Loc,
