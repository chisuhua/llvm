//===-- PPURegBankReassign.cpp - Reassign registers after regalloc --------===//
//
...
static cl::opt<unsigned> VerifyStallCycles("ppu-verify-regbanks-reassign",
  cl::desc("Verify stall cycles in the regbanks reassign pass"),
  cl::value_desc("0|1|2"),
  cl::init(0), cl::Hidden);

#define DEBUG_TYPE "ppu-regbanks-reassign"
...
class PPURegBankReassign : public MachineFunctionPass {
  ...
  public:
    ...
    void dump(const PPURegBankReassign *P) const {
      MI->dump();
...
public:
  PPURegBankReassign() : MachineFunctionPass(ID) {
    initializePPURegBankReassignPass(*PassRegistry::getPassRegistry());
  }

  bool runOnMachineFunction(MachineFunction &MF) override;

  StringRef getPassName() const override { return "PPU RegBank Reassign"; }
...
private:
  ...
  unsigned analyzeInst(const MachineInstr& MI, unsigned& UsedBanks,
                       unsigned Reg = PPU::NoRegister, int Bank = -1);
...
INITIALIZE_PASS_BEGIN(PPURegBankReassign, DEBUG_TYPE, "PPU RegBank Reassign",
                      false, false)
INITIALIZE_PASS_DEPENDENCY(LiveIntervals)
INITIALIZE_PASS_DEPENDENCY(MachineLoopInfo)
INITIALIZE_PASS_DEPENDENCY(VirtRegMap)
INITIALIZE_PASS_DEPENDENCY(LiveRegMatrix)
INITIALIZE_PASS_END(PPURegBankReassign, DEBUG_TYPE, "PPU RegBank Reassign",
                    false, false)


char PPURegBankReassign::ID = 0;

char &llvm::PPURegBankReassignID = PPURegBankReassign::ID;

unsigned PPURegBankReassign::getPhysRegBank(unsigned Reg) const {
  assert(Register::isPhysicalRegister(Reg));

  const TargetRegisterClass *RC = TRI->getMinimalPhysRegClass(Reg);
  unsigned Size = TRI->getRegSizeInBits(*RC);
  if (Size > 32)
    Reg = TRI->getSubReg(Reg, PPU::sub0);
...
unsigned PPURegBankReassign::getRegBankMask(unsigned Reg, unsigned SubReg,
                                            int Bank) {
  ...
  Reg = TRI->getEncodingValue(Reg) / 2;
  unsigned StartBit = PPU::VPR_32RegClass.getNumRegs();
...
unsigned PPURegBankReassign::analyzeInst(const MachineInstr& MI,
                                         unsigned& UsedBanks,
                                         unsigned Reg,
                                         int Bank) {
...
unsigned PPURegBankReassign::getOperandGatherWeight(const MachineInstr& MI,
                                                    unsigned Reg1,
                                                    unsigned Reg2,
                                                    unsigned StallCycles) const
{
...
bool PPURegBankReassign::isReassignable(unsigned Reg) const {
  ...
  unsigned Size = TRI->getRegSizeInBits(*RC);
  if (Size > 32)
    PhysReg = TRI->getSubReg(PhysReg, PPU::sub0);

  return PPU::SPR_32RegClass.contains(PhysReg);
}

unsigned PPURegBankReassign::getFreeBanks(unsigned Mask,
                                          unsigned UsedBanks) const {
...
unsigned PPURegBankReassign::getFreeBanks(unsigned Reg,
                                          unsigned SubReg,
                                          unsigned Mask,
                                          unsigned UsedBanks) const {
...
void PPURegBankReassign::collectCandidates(MachineInstr& MI,
                                           unsigned UsedBanks,
                                           unsigned StallCycles) {
  LLVM_DEBUG(MI.dump());
...
unsigned PPURegBankReassign::computeStallCycles(unsigned SrcReg,
...
unsigned PPURegBankReassign::scavengeReg(LiveInterval& LI,
                                         unsigned Bank) const {
  const TargetRegisterClass *RC = MRI->getRegClass(LI.reg);
  unsigned MaxNumRegs = (Bank < NUM_VGPR_BANKS) ? MaxNumVGPRs
                                                : MaxNumSGPRs;
  unsigned MaxReg = MaxNumRegs + (Bank < NUM_VGPR_BANKS ? PPU::VPR0
                                                        : PPU::SPR0);
...
unsigned PPURegBankReassign::tryReassign(Candidate &C) {
...
unsigned PPURegBankReassign::collectCandidates(MachineFunction &MF,
                                               bool Collect) {
  unsigned TotalStallCycles = 0;
...
void PPURegBankReassign::removeCandidates(unsigned Reg) {
  Candidates.remove_if([Reg, this](const Candidate& C) {
    return C.MI->readsRegister(Reg, TRI);
  });
}

bool PPURegBankReassign::verifyCycles(MachineFunction &MF,
...
bool PPURegBankReassign::runOnMachineFunction(MachineFunction &MF) {
  ...
  RegsUsed.resize(PPU::VPR_32RegClass.getNumRegs() +
                  TRI->getEncodingValue(PPU::SPR_NULL) / 2 + 1);
  ...
  while (!Candidates.empty()) {
    ...
    if (LocalCyclesSaved) {
      ...
      computeStallCycles(C.Reg, PPU::NoRegister, -1, true);
      Candidates.sort();
