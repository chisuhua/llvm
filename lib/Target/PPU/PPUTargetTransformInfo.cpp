//===-- PPUTargetTransformInfo.cpp - PPU specific TTI ----------------===//
//
...
int PPUTTIImpl::getIntImmCost(const APInt &Imm, Type *Ty) {
  ...
  return PPUMatInt::getIntMatCost(Imm, DL.getTypeSizeInBits(Ty),
                                    getST()->is64Bit());
}

int PPUTTIImpl::getIntImmCost(unsigned Opcode, unsigned Idx, const APInt &Imm,
                                Type *Ty) {
  assert(Ty->isIntegerTy() &&
         "getIntImmCost can only estimate cost of materialising integers");
  ...
  // Some instructions in PPU can take a 12-bit immediate. Some of these are
...
int PPUTTIImpl::getIntImmCost(Intrinsic::ID IID, unsigned Idx,
...
unsigned PPUTTIImpl::getHardwareNumberOfRegisters(bool Vec) const {
  // The concept of vector registers doesn't really exist. Some packed vector
  // operations operate on the normal 32-bit registers.
  return 256;
}

unsigned PPUTTIImpl::getNumberOfRegisters(bool Vec) const {
...
unsigned PPUTTIImpl::getRegisterBitWidth(bool Vector) const {
  return 32;
}

unsigned PPUTTIImpl::getMinVectorRegisterBitWidth() const {
  return 32;
}

unsigned PPUTTIImpl::getLoadVectorFactor(unsigned VF, unsigned LoadSize,
...
unsigned PPUTTIImpl::getStoreVectorFactor(unsigned VF, unsigned StoreSize,
                                             unsigned ChainSizeInBytes,
                                             VectorType *VecTy) const {
...
unsigned PPUTTIImpl::getLoadStoreVecRegBitWidth(unsigned AddrSpace) const {
...
bool PPUTTIImpl::isLegalToVectorizeMemChain(unsigned ChainSizeInBytes,
                                               unsigned Alignment,
...
bool PPUTTIImpl::isLegalToVectorizeLoadChain(unsigned ChainSizeInBytes,
                                                unsigned Alignment,
                                                unsigned AddrSpace) const {
  return isLegalToVectorizeMemChain(ChainSizeInBytes, Alignment, AddrSpace);
}

bool PPUTTIImpl::isLegalToVectorizeStoreChain(unsigned ChainSizeInBytes,
                                                 unsigned Alignment,
                                                 unsigned AddrSpace) const {
  return isLegalToVectorizeMemChain(ChainSizeInBytes, Alignment, AddrSpace);
}

unsigned PPUTTIImpl::getMaxInterleaveFactor(unsigned VF) {
...
bool PPUTTIImpl::getTgtMemIntrinsic(IntrinsicInst *Inst,
                                       MemIntrinsicInfo &Info) const {
  switch (Inst->getIntrinsicID()) {
      assert("FIXME");
  /* FIXME we need add below back
...
int PPUTTIImpl::getArithmeticInstrCost(
  ...
  assert("PPUTTIImpl::getArithmeticInstrCost:FIXME");
  /*
  // Legalize the type.
...
unsigned PPUTTIImpl::getCFInstrCost(unsigned Opcode) {
...
int PPUTTIImpl::getArithmeticReductionCost(unsigned Opcode, Type *Ty,
                                              bool IsPairwise) {
  EVT OrigTy = TLI->getValueType(DL, Ty);
...
int PPUTTIImpl::getMinMaxReductionCost(Type *Ty, Type *CondTy,
                                          bool IsPairwise,
                                          bool IsUnsigned) {
  EVT OrigTy = TLI->getValueType(DL, Ty);
...
int PPUTTIImpl::getVectorInstrCost(unsigned Opcode, Type *ValTy,
                                      unsigned Index) {
  switch (Opcode) {
...
bool PPUTTIImpl::isSourceOfDivergence(const Value *V) const {
  ...
  if (const IntrinsicInst *Intrinsic = dyn_cast<IntrinsicInst>(V))
    return PPU::isIntrinsicSourceOfDivergence(Intrinsic->getIntrinsicID());
...
bool PPUTTIImpl::isAlwaysUniform(const Value *V) const {
  if (const IntrinsicInst *Intrinsic = dyn_cast<IntrinsicInst>(V)) {
...
bool PPUTTIImpl::collectFlatAddressOperands(SmallVectorImpl<int> &OpIndexes,
...
bool PPUTTIImpl::rewriteIntrinsicWithAddressSpace(
...
unsigned PPUTTIImpl::getShuffleCost(TTI::ShuffleKind Kind, Type *Tp, int Index,
                                       Type *SubTp) {

  return BaseT::getShuffleCost(Kind, Tp, Index, SubTp);
}

bool PPUTTIImpl::areInlineCompatible(const Function *Caller,
...
void PPUTTIImpl::getUnrollingPreferences(Loop *L, ScalarEvolution &SE,
