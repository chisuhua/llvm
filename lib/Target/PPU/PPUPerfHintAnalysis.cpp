//===- PPUPerfHintAnalysis.cpp - analysis of functions memory traffic --===//
//
...
#define DEBUG_TYPE "ppu-perf-hint"

static cl::opt<unsigned>
    MemBoundThresh("ppu-membound-threshold", cl::init(50), cl::Hidden,
                   cl::desc("Function mem bound threshold in %"));

static cl::opt<unsigned>
    LimitWaveThresh("ppu-limit-wave-threshold", cl::init(50), cl::Hidden,
                    cl::desc("Kernel limit wave threshold in %"));

static cl::opt<unsigned>
    IAWeight("ppu-indirect-access-weight", cl::init(1000), cl::Hidden,
             cl::desc("Indirect access memory instruction weight"));

static cl::opt<unsigned>
    LSWeight("ppu-large-stride-weight", cl::init(1000), cl::Hidden,
             cl::desc("Large stride memory access weight"));

static cl::opt<unsigned>
    LargeStrideThresh("ppu-large-stride-threshold", cl::init(64), cl::Hidden,
...
char llvm::PPUPerfHintAnalysis::ID = 0;
char &llvm::PPUPerfHintAnalysisID = PPUPerfHintAnalysis::ID;

INITIALIZE_PASS(PPUPerfHintAnalysis, DEBUG_TYPE,
                "Analysis if a function is memory bound", true, true)

namespace {

struct PPUPerfHint {
  friend PPUPerfHintAnalysis;

public:
  PPUPerfHint(PPUPerfHintAnalysis::FuncInfoMap &FIM_,
...
private:
  ...
  PPUPerfHintAnalysis::FuncInfoMap &FIM;

  const DataLayout *DL;

  const TargetLowering *TLI;

  PPUPerfHintAnalysis::FuncInfo *visit(const Function &F);
  static bool isMemBound(const PPUPerfHintAnalysis::FuncInfo &F);
  static bool needLimitWave(const PPUPerfHintAnalysis::FuncInfo &F);
...
bool PPUPerfHint::isIndirectAccess(const Instruction *Inst) const {
...
PPUPerfHintAnalysis::FuncInfo *PPUPerfHint::visit(const Function &F) {
  PPUPerfHintAnalysis::FuncInfo &FI = FIM[&F];

  LLVM_DEBUG(dbgs() << "[PPUPerfHint] process " << F.getName() << '\n');
...
bool PPUPerfHint::runOnFunction(Function &F) {
  const Module &M = *F.getParent();
  DL = &M.getDataLayout();

  if (F.hasFnAttribute("ppu-wave-limiter") &&
      F.hasFnAttribute("ppu-memory-bound"))
    return false;

  const PPUPerfHintAnalysis::FuncInfo *Info = visit(F);
  ...
  if (isMemBound(*Info)) {
    ...
    NumMemBound++;
    F.addFnAttr("ppu-memory-bound", "true");
  }

  if (PPU::isEntryFunctionCC(F.getCallingConv()) && needLimitWave(*Info)) {
    LLVM_DEBUG(dbgs() << F.getName() << " needs limit wave\n");
    NumLimitWave++;
    F.addFnAttr("ppu-wave-limiter", "true");
  }

  return true;
}

bool PPUPerfHint::isMemBound(const PPUPerfHintAnalysis::FuncInfo &FI) {
  return FI.MemInstCount * 100 / FI.InstCount > MemBoundThresh;
}

bool PPUPerfHint::needLimitWave(const PPUPerfHintAnalysis::FuncInfo &FI) {
  return ((FI.MemInstCount + FI.IAMInstCount * IAWeight +
           FI.LSMInstCount * LSWeight) *
          100 / FI.InstCount) > LimitWaveThresh;
}

bool PPUPerfHint::isGlobalAddr(const Value *V) const {
...
bool PPUPerfHint::isLocalAddr(const Value *V) const {
  if (auto PT = dyn_cast<PointerType>(V->getType()))
    return PT->getAddressSpace() == AMDGPUAS::LOCAL_ADDRESS;
  return false;
}

bool PPUPerfHint::isLargeStride(const Instruction *Inst) {
...
PPUPerfHint::MemAccessInfo
PPUPerfHint::makeMemAccessInfo(Instruction *Inst) const {
  MemAccessInfo MAI;
...
bool PPUPerfHint::isConstantAddr(const Value *V) const {
  if (auto PT = dyn_cast<PointerType>(V->getType())) {
    unsigned As = PT->getAddressSpace();
...
bool PPUPerfHint::MemAccessInfo::isLargeStride(
...
bool PPUPerfHintAnalysis::runOnSCC(CallGraphSCC &SCC) {
  ...
  for (CallGraphNode *I : SCC) {
    ...
    PPUPerfHint Analyzer(FIM, ST->getTargetLowering());
...
bool PPUPerfHintAnalysis::isMemoryBound(const Function *F) const {
  auto FI = FIM.find(F);
  if (FI == FIM.end())
    return false;

  return PPUPerfHint::isMemBound(FI->second);
}

bool PPUPerfHintAnalysis::needsWaveLimiter(const Function *F) const {
  auto FI = FIM.find(F);
  if (FI == FIM.end())
    return false;

  return PPUPerfHint::needLimitWave(FI->second);
}
