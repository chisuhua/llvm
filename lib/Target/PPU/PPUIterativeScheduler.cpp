...
std::vector<const SUnit *> makePPUILPScheduler(ArrayRef<const SUnit *> BotRoots,
    const ScheduleDAG &DAG);
}
...
LLVM_DUMP_METHOD
void PPUIterativeScheduler::printRegions(raw_ostream &OS) const {
  const auto &ST = MF.getSubtarget<PPUSubtarget>();
...
LLVM_DUMP_METHOD
void PPUIterativeScheduler::printSchedResult(raw_ostream &OS,
                                            const Region *R,
...
LLVM_DUMP_METHOD
void PPUIterativeScheduler::printSchedRP(raw_ostream &OS,
...
// DAG builder helper
class PPUIterativeScheduler::BuildDAG {
  PPUIterativeScheduler &Sch;
  SmallVector<SUnit *, 8> TopRoots;

  SmallVector<SUnit*, 8> BotRoots;

public:
  BuildDAG(const Region &R, PPUIterativeScheduler &_Sch)
    : Sch(_Sch) {
...
PPUIterativeScheduler::PPUIterativeScheduler(MachineSchedContext *C,
                                           StrategyKind S)
...
PPURegPressure PPUIterativeScheduler::getRegionPressure(MachineBasicBlock::iterator Begin,
...
template <typename Range> PPURegPressure
PPUIterativeScheduler::getSchedulePressure(const Region &R,
                                          Range &&Schedule) const {
  auto const BBEnd = R.Begin->getParent()->end();
  PPUUpwardRPTracker RPTracker(*LIS);
  if (R.End != BBEnd) {
...
void PPUIterativeScheduler::enterRegion(MachineBasicBlock *BB, // overriden
                                        MachineBasicBlock::iterator Begin,
                                        MachineBasicBlock::iterator End,
  ...
  if (NumRegionInstrs > 2) {
    Regions.push_back(
      new (Alloc.Allocate())
      Region { Begin, End, NumRegionInstrs, getRegionPressure(Begin, End), nullptr});
  }
}

void PPUIterativeScheduler::schedule() { // overriden
  // do nothing
  LLVM_DEBUG(printLivenessInfo(dbgs(), RegionBegin, RegionEnd, LIS);
             if (!Regions.empty() && Regions.back()->Begin == RegionBegin) {
               ...
               Regions.back()->MaxPressure.print(
                   dbgs(), &MF.getSubtarget<PPUSubtarget>());
             } dbgs()
             << '\n';);
}

void PPUIterativeScheduler::finalizeSchedule() { // overriden
  if (Regions.empty())
    return;
  switch (Strategy) {
...
PPUIterativeScheduler::detachSchedule(ScheduleRef Schedule) const {
...
void PPUIterativeScheduler::setBestSchedule(Region &R, ScheduleRef Schedule,
                                           const PPURegPressure &MaxRP) {
  R.BestSchedule.reset(new TentativeSchedule{detachSchedule(Schedule), MaxRP});
}

void PPUIterativeScheduler::scheduleBest(Region &R) {
...
void PPUIterativeScheduler::scheduleRegion(Region &R, Range &&Schedule,
                                          const PPURegPressure &MaxRP) {
...
void PPUIterativeScheduler::sortRegionsByPressure(unsigned TargetOcc) {
  const auto &ST = MF.getSubtarget<PPUSubtarget>();
...
unsigned PPUIterativeScheduler::tryMaximizeOccupancy(unsigned TargetOcc) {
...
void PPUIterativeScheduler::scheduleLegacyMaxOccupancy(
  ...
  LLVM_DEBUG(dbgs() << "Scheduling using default scheduler, "
                       "target occupancy = "
                    << TgtOcc << '\n');
  PPUMaxOccupancySchedStrategy LStrgy(Context);
...
void PPUIterativeScheduler::scheduleMinReg(bool force) {
  const auto &ST = MF.getSubtarget<PPUSubtarget>();
...
void PPUIterativeScheduler::scheduleILP(bool TryMaximizeOccupancy) {
  const auto &ST = MF.getSubtarget<PPUSubtarget>();
  PPUMachineFunctionInfo *MFI = MF.getInfo<PPUMachineFunctionInfo>();
  auto TgtOcc = MFI->getMinAllowedOccupancy();
  ...
  for (auto R : Regions) {
    BuildDAG DAG(*R, *this);
    const auto ILPSchedule = makePPUILPScheduler(DAG.getBottomRoots(), *this);
