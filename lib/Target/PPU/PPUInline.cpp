//===- PPUInline.cpp - Code to perform simple function inlining --------===//
//
...
//
/// \file
/// This is PPU specific replacement of the standard inliner.
/// The main purpose is to account for the fact that calls not only expensive
/// on the PPU, but much more expensive if a private memory pointer is
...
static cl::opt<int>
ArgAllocaCost("ppu-inline-arg-alloca-cost", cl::Hidden, cl::init(1500),
...
ArgAllocaCutoff("ppu-inline-arg-alloca-cutoff", cl::Hidden, cl::init(256),
                cl::desc("Maximum alloca size to use for inline cost"));

// Inliner constraint to achieve reasonable compilation time
static cl::opt<size_t>
MaxBB("ppu-inline-max-bb", cl::Hidden, cl::init(300),
      cl::desc("Maximum BB number allowed in a function after inlining"
               " (compile time constraint)"));

namespace {

class PPUInliner : public LegacyInlinerBase {

public:
  PPUInliner() : LegacyInlinerBase(ID) {
    initializePPUInlinerPass(*PassRegistry::getPassRegistry());
...
char PPUInliner::ID = 0;
INITIALIZE_PASS_BEGIN(PPUInliner, "ppu-inline",
                "PPU Function Integration/Inlining", false, false)
INITIALIZE_PASS_DEPENDENCY(AssumptionCacheTracker)
INITIALIZE_PASS_DEPENDENCY(CallGraphWrapperPass)
INITIALIZE_PASS_DEPENDENCY(ProfileSummaryInfoWrapperPass)
INITIALIZE_PASS_DEPENDENCY(TargetTransformInfoWrapperPass)
INITIALIZE_PASS_DEPENDENCY(TargetLibraryInfoWrapperPass)
INITIALIZE_PASS_END(PPUInliner, "ppu-inline",
                "PPU Function Integration/Inlining", false, false)

Pass *llvm::createPPUFunctionInliningPass() { return new PPUInliner(); }

bool PPUInliner::runOnSCC(CallGraphSCC &SCC) {
  TTIWP = &getAnalysis<TargetTransformInfoWrapperPass>();
  return LegacyInlinerBase::runOnSCC(SCC);
}

void PPUInliner::getAnalysisUsage(AnalysisUsage &AU) const {
  AU.addRequired<TargetTransformInfoWrapperPass>();
  LegacyInlinerBase::getAnalysisUsage(AU);
}

unsigned PPUInliner::getInlineThreshold(CallSite CS) const {
...
InlineCost PPUInliner::getInlineCost(CallSite CS) {
  Function *Callee = CS.getCalledFunction();
  Function *Caller = CS.getCaller();
