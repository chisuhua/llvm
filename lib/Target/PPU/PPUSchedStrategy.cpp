//===-- PPUSchedStrategy.cpp - PPU Scheduler Strategy ---------------------===//
//
...
PPUMaxOccupancySchedStrategy::PPUMaxOccupancySchedStrategy(
    const MachineSchedContext *C)
    : GenericScheduler(C), TargetOccupancy(0), MF(nullptr) {}

void PPUMaxOccupancySchedStrategy::initialize(ScheduleDAGMI *DAG) {
  GenericScheduler::initialize(DAG);

  const PPURegisterInfo *SRI = static_cast<const PPURegisterInfo *>(TRI);
  ...
  SGPRExcessLimit =
      Context->RegClassInfo->getNumAllocatableRegs(&PPU::SPR_32RegClass) -
      ErrorMargin;
  VGPRExcessLimit =
      Context->RegClassInfo->getNumAllocatableRegs(&PPU::VPR_32RegClass) -
      ErrorMargin;
  if (TargetOccupancy) {
    ...
    SGPRCriticalLimit =
        SRI->getRegPressureSetLimit(DAG->MF, SRI->getSGPRPressureSet());
    VGPRCriticalLimit =
        SRI->getRegPressureSetLimit(DAG->MF, SRI->getVGPRPressureSet());
  }
...
void PPUMaxOccupancySchedStrategy::initCandidate(
    SchedCandidate &Cand, SUnit *SU, bool AtTop,
    const RegPressureTracker &RPTracker, const PPURegisterInfo *SRI,
...
void PPUMaxOccupancySchedStrategy::pickNodeFromQueue(
...
SUnit *PPUMaxOccupancySchedStrategy::pickNodeBidirectional(bool &IsTopNode) {
...
SUnit *PPUMaxOccupancySchedStrategy::pickNode(bool &IsTopNode) {
...
PPUScheduleDAGMILive::PPUScheduleDAGMILive(
    MachineSchedContext *C, std::unique_ptr<MachineSchedStrategy> S)
    : ScheduleDAGMILive(C, std::move(S)), ST(MF.getSubtarget<PPUSubtarget>()),
...
void PPUScheduleDAGMILive::schedule() {
  if (Stage == 0) {
  ...
  PPURegPressure PressureBefore;
  if (LIS) {
    PressureBefore = Pressure[RegionIdx];

    LLVM_DEBUG(dbgs() << "Pressure before scheduling:\nRegion live-ins:";
               PPURPTracker::printLiveRegs(dbgs(), LiveIns[RegionIdx], MRI);
  ...
  PPUMaxOccupancySchedStrategy &S = (PPUMaxOccupancySchedStrategy &)*SchedImpl;
...
PPURegPressure PPUScheduleDAGMILive::getRealRegPressure() const {
  PPUDownwardRPTracker RPTracker(*LIS);
  RPTracker.advance(begin(), end(), &LiveIns[RegionIdx]);
  return RPTracker.moveMaxPressure();
}

void PPUScheduleDAGMILive::computeBlockPressure(const MachineBasicBlock *MBB) {
  PPUDownwardRPTracker RPTracker(*LIS);
...
void PPUScheduleDAGMILive::finalizeSchedule() {
  PPUMaxOccupancySchedStrategy &S = (PPUMaxOccupancySchedStrategy &)*SchedImpl;
