...
#define DEBUG_TYPE "ppu-lower-kernel-attributes"
...
class PPULowerKernelAttributes : public ModulePass {
  Module *Mod = nullptr;

public:
  static char ID;

  PPULowerKernelAttributes() : ModulePass(ID) {}
...
bool PPULowerKernelAttributes::doInitialization(Module &M) {
  Mod = &M;
  return false;
}

bool PPULowerKernelAttributes::processUse(CallInst *CI) {
    ...
    for (User *U : GroupSize->users()) {
      ...
      for (User *ZextUser : ZextGroupSize->users()) {
        ...
        auto GroupIDIntrin = I == 0 ?
          m_Intrinsic<Intrinsic::ppu_workgroup_id_x>() :
            (I == 1 ? m_Intrinsic<Intrinsic::ppu_workgroup_id_y>() :
                      m_Intrinsic<Intrinsic::ppu_workgroup_id_z>());
...
bool PPULowerKernelAttributes::runOnModule(Module &M) {
  StringRef DispatchPtrName
    = Intrinsic::getName(Intrinsic::ppu_dispatch_ptr);
...
INITIALIZE_PASS_BEGIN(PPULowerKernelAttributes, DEBUG_TYPE,
                      "PPU IR optimizations", false, false)
INITIALIZE_PASS_END(PPULowerKernelAttributes, DEBUG_TYPE, "PPU IR optimizations",
                    false, false)

char PPULowerKernelAttributes::ID = 0;

ModulePass *llvm::createPPULowerKernelAttributesPass() {
  return new PPULowerKernelAttributes();
}
