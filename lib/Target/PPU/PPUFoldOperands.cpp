//===-- PPUFoldOperands.cpp - Fold operands --- ----------------------------===//
//
...
#define DEBUG_TYPE "ppu-fold-operands"
...
INITIALIZE_PASS(PPUFoldOperands, DEBUG_TYPE,
                "PPU Fold Operands", false, false)

char PPUFoldOperands::ID = 0;

char &llvm::PPUFoldOperandsID = PPUFoldOperands::ID;

// Wrapper around isInlineConstant that understands special cases when
// instruction types are replaced during operand folding.
static bool isInlineConstantIfFolded(const PPUInstrInfo *TII,
  ...
  case PPU::ML_MAC_F16: {
    ...
    if (static_cast<int>(OpNo) == Src2Idx) {
      bool IsFMA = Opc == PPU::V_FMAC_F32_e64;
      bool IsF32 = Opc == PPU::V_MAC_F32_e64;

      unsigned Opc = IsFMA ?
        PPU::V_FMA_F32 : (IsF32 ? PPU::V_MAD_F32 : PPU::V_MAD_F16);
...
static bool frameIndexMayFold(const PPUInstrInfo *TII,
                              const MachineInstr &UseMI,
                              int OpNo,
                              const MachineOperand &OpToFold) {
    /* FIXME
...
/* FIXME
  if (Fold.isImm()) {
    if (MI->getDesc().TSFlags & PPUInstrFlags::IsPacked &&
        ...
        PPU::isInlinableLiteralV216(static_cast<uint16_t>(Fold.ImmToFold),
      ...
      int ModIdx = -1;
      if (OpNo == PPU::getNamedOperandIdx(Opcode, PPU::OpName::src0))
        ModIdx = PPU::OpName::src0_modifiers;
      else if (OpNo == PPU::getNamedOperandIdx(Opcode, PPU::OpName::src1))
        ModIdx = PPU::OpName::src1_modifiers;
      else if (OpNo == PPU::getNamedOperandIdx(Opcode, PPU::OpName::src2))
        ModIdx = PPU::OpName::src2_modifiers;
      assert(ModIdx != -1);
  ...
  if ((Fold.isImm() || Fold.isFI() || Fold.isGlobal()) && Fold.needsShrink()) {
    ...
    auto Liveness = MBB->computeRegisterLiveness(&TRI, PPU::VCC, MI);
    ...
    if (HaveNonDbgCarryUse) {
      BuildMI(*MBB, MI, MI->getDebugLoc(), TII.get(PPU::COPY), Dst1.getReg())
        .addReg(PPU::VCC, RegState::Kill);
    }
    ...
    for (unsigned I = MI->getNumOperands() - 1; I > 0; --I)
      MI->RemoveOperand(I);
    MI->setDesc(TII.get(PPU::IMPLICIT_DEF));
...
static bool tryAddToFoldList(SmallVectorImpl<FoldCandidate> &FoldList,
  ...
  if (!TII->isOperandLegal(*MI, OpNo, OpToFold)) {
    ...
    if ((Opc == PPU::V_MAC_F32_e64 || Opc == PPU::V_MAC_F16_e64 ||
        ...
        (int)OpNo == PPU::getNamedOperandIdx(Opc, PPU::OpName::src2)) {
      bool IsFMA = Opc == PPU::V_FMAC_F32_e64;
      bool IsF32 = Opc == PPU::V_MAC_F32_e64;
...
/* FIXME
    if (!TII->isOperandLegal(*MI, CommuteOpNo, OpToFold)) {
      if ((Opc == PPU::V_ADD_I32_e64 ||
           ...
           Opc == PPU::V_SUBREV_I32_e64) && // FIXME
        ...
        int Op32 = PPU::getVOPe32(MaybeCommutedOpc);
...
static bool isUseSafeToFold(const PPUInstrInfo *TII,
                            const MachineInstr &MI,
...
static bool tryToFoldACImm(const PPUInstrInfo *TII,
                           const MachineOperand &OpToFold,
                           MachineInstr *UseMI,
                           unsigned UseOpIdx,
                           SmallVectorImpl<FoldCandidate> &FoldList) {
  ...
  if (OpTy < PPU::OPERAND_REG_INLINE_AC_FIRST ||
      OpTy > PPU::OPERAND_REG_INLINE_AC_LAST)
    return false;
...
void PPUFoldOperands::foldOperand(
  MachineOperand &OpToFold,
  MachineInstr *UseMI,
  int UseOpIdx,
  SmallVectorImpl<FoldCandidate> &FoldList,
  ...
  if (UseOp.isReg() && OpToFold.isReg()) {
    if (UseOp.isImplicit() || UseOp.getSubReg() != PPU::NoSubRegister)
      return;
    ...
    if (UseOp.isTied() && OpToFold.getSubReg() != PPU::NoSubRegister)
      return;
  }
...
/* FIXME
  if (frameIndexMayFold(TII, *UseMI, UseOpIdx, OpToFold)) {
    ...
    if (TII->getNamedOperand(*UseMI, PPU::OpName::srsrc)->getReg() !=
...
/* FIXME
    if (DestRC == &PPU::AGPR_32RegClass &&
        TII->isInlineConstant(OpToFold, PPU::OPERAND_REG_INLINE_C_INT32)) {
      UseMI->setDesc(TII->get(PPU::V_ACCVGPR_WRITE_B32));
...
*/
    ...
    if (UseMI->isCopy() && OpToFold.isReg() &&
      ...
      if (TRI->isAGPR(*MRI, UseMI->getOperand(0).getReg()) &&
        ...
        UseMI->setDesc(TII->get(PPU::V_ACCVGPR_READ_B32));
        */
      return;
    }
    ...
    if (UseOpc == PPU::VREAD1stLANE ||
      ...
      if (OpToFold.isReg() && TRI->isSGPRReg(*MRI, OpToFold.getReg())) {
        ...
        // =>
        // %sgpr1 = COPY %sgpr0
        UseMI->setDesc(TII->get(PPU::COPY));
        UseMI->getOperand(1).setReg(OpToFold.getReg());
        UseMI->getOperand(1).setSubReg(OpToFold.getSubReg());
        UseMI->getOperand(1).setIsKill(false);
        UseMI->RemoveOperand(2); // Remove exec read (or src1 for readlane)
  ...
  if (UseOp.getSubReg() && PPU::getRegBitWidth(FoldRC->getID()) == 64) {
    ...
    if (PPU::getRegBitWidth(UseRC->getID()) != 64)
      return;

    APInt Imm(64, OpToFold.getImm());
    if (UseOp.getSubReg() == PPU::sub0) {
      Imm = Imm.getLoBits(32);
    } else {
      assert(UseOp.getSubReg() == PPU::sub1);
...
static bool evalBinaryInstruction(unsigned Opcode, int32_t &Result,
  ...
  case PPU::S_AND_B32:
    Result = LHS & RHS;
    return true;
  case PPU::V_OR_B32_e64:
  case PPU::V_OR_B32_e32:
  case PPU::S_OR_B32:
    Result = LHS | RHS;
    return true;
  case PPU::V_XOR_B32_e64:
  case PPU::V_XOR_B32_e32:
  case PPU::S_XOR_B32:
    Result = LHS ^ RHS;
    return true;
  case PPU::V_LSHL_B32_e64:
  case PPU::V_LSHL_B32_e32:
  case PPU::S_LSHL_B32:
  ...
  case PPU::V_LSHLREV_B32_e32:
    Result = RHS << (LHS & 31);
    return true;
  case PPU::V_LSHR_B32_e64:
  case PPU::V_LSHR_B32_e32:
  case PPU::S_LSHR_B32:
    Result = LHS >> (RHS & 31);
    return true;
  ...
  case PPU::V_LSHRREV_B32_e32:
    Result = RHS >> (LHS & 31);
    return true;
  case PPU::V_ASHR_I32_e64:
  case PPU::V_ASHR_I32_e32:
  case PPU::S_ASHR_I32:
...
static MachineOperand *getImmOrMaterializedImm(MachineRegisterInfo &MRI,
  ...
  if (Op.isReg()) {
    ...
    if (Op.getSubReg() != PPU::NoSubRegister ||
...
static bool tryConstantFoldOp(MachineRegisterInfo &MRI,
  ...
  if (Opc == PPU::V_NOT_B32_e64 || Opc == PPU::V_NOT_B32_e32 ||
    ...
    mutateCopyOp(*MI, TII->get(getMovOpc(Opc == PPU::S_NOT_B32)));
    return true;
  }
  ...
  if (MI->getOpcode() == PPU::V_LSHL_OR_B32) {
    if (Src0->isImm() && Src0->getImm() == 0) {
      // v_lshl_or_b32 0, X, Y -> copy Y
      // v_lshl_or_b32 0, X, K -> v_mov_b32 K
      bool UseCopy = TII->getNamedOperand(*MI, PPU::OpName::src2)->isReg();
      MI->RemoveOperand(Src1Idx);
      MI->RemoveOperand(Src0Idx);

      MI->setDesc(TII->get(UseCopy ? PPU::COPY : PPU::V_MOV_B32_e32));
      return true;
    }
  }
  ...
  if (Opc == PPU::VOR ||
    ...
    if (Src1Val == 0) {
      // y = or x, 0 => y = copy x
      MI->RemoveOperand(Src1Idx);
      mutateCopyOp(*MI, TII->get(PPU::COPY));
    } else if (Src1Val == -1) {
...
/* FIXME
  ...
  */
  ...
  if (Opc == PPU::V_CNDMASK_B32_e32    ||
    ...
    const MachineOperand *Src0 = TII->getNamedOperand(*MI, PPU::OpName::src0);
    const MachineOperand *Src1 = TII->getNamedOperand(*MI, PPU::OpName::src1);
    int Src1ModIdx = PPU::getNamedOperandIdx(Opc, PPU::OpName::src1_modifiers);
    int Src0ModIdx = PPU::getNamedOperandIdx(Opc, PPU::OpName::src0_modifiers);
    if (Src1->isIdenticalTo(*Src0) &&
        (Src1ModIdx == -1 || !MI->getOperand(Src1ModIdx).getImm()) &&
        (Src0ModIdx == -1 || !MI->getOperand(Src0ModIdx).getImm())) {
      LLVM_DEBUG(dbgs() << "Folded " << *MI << " into ");
      auto &NewDesc =
          TII->get(Src0->isReg() ? (unsigned)PPU::COPY : getMovOpc(false));
      int Src2Idx = PPU::getNamedOperandIdx(Opc, PPU::OpName::src2);
      if (Src2Idx != -1)
        MI->RemoveOperand(Src2Idx);
      MI->RemoveOperand(PPU::getNamedOperandIdx(Opc, PPU::OpName::src1));
...
void PPUFoldOperands::foldInstOperand(MachineInstr &MI,
...
bool PPUFoldOperands::tryFoldClamp(MachineInstr &MI) {
  ...
  MachineOperand *DefClamp = TII->getNamedOperand(*Def, PPU::OpName::clamp);
  if (!DefClamp)
    return false;
...
bool PPUFoldOperands::tryFoldOMod(MachineInstr &MI) {
  ...
  // fold it.
  if (TII->hasModifiersSet(*Def, PPU::OpName::clamp))
    return false;
