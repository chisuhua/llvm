...
class PPULowerIntrinsics : public ModulePass {
private:
...
public:
  ...
  PPULowerIntrinsics() : ModulePass(ID) {}

  bool runOnModule(Module &M) override;
  bool expandMemIntrinsicUses(Function &F);
  StringRef getPassName() const override { return "PPU Lower Intrinsics"; }
...
char PPULowerIntrinsics::ID = 0;

char &llvm::PPULowerIntrinsicsID = PPULowerIntrinsics::ID;

INITIALIZE_PASS(PPULowerIntrinsics, DEBUG_TYPE, "Lower intrinsics", false, false)
...
/// This function is used to replace NVVM intrinsics with PPU instrinsics
bool PPULowerIntrinsics::lowerNVVMIntrinsics(CallInst *CI) {
...
bool PPULowerIntrinsics::expandMemIntrinsicUses(Function &F) {
...
bool PPULowerIntrinsics::makeLIDRangeMetadata(Function &F) const {
  ...
  for (auto *U : F.users()) {
    ...
    Changed |= PPUSubtarget::get(TM, F).makeLIDRangeMetadata(CI);
  }
  return Changed;
}

bool PPULowerIntrinsics::runOnModule(Module &M) {
  bool Changed = false;
...
ModulePass *llvm::createPPULowerIntrinsicsPass() {
  return new PPULowerIntrinsics();
}
