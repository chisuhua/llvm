//===- PPUFixSGPRCopies.cpp - Remove potential VGPR => SGPR copies ---------===//
//
...
#define DEBUG_TYPE "ppu-fix-sgpr-copies"
...
class PPUFixSGPRCopies : public MachineFunctionPass {
  MachineDominatorTree *MDT;

public:
  static char ID;

  PPUFixSGPRCopies() : MachineFunctionPass(ID) {}

  bool runOnMachineFunction(MachineFunction &MF) override;

  StringRef getPassName() const override { return "PPU Fix SGPR copies"; }
...
INITIALIZE_PASS_BEGIN(PPUFixSGPRCopies, DEBUG_TYPE,
                     "PPU Fix SGPR copies", false, false)
INITIALIZE_PASS_DEPENDENCY(MachineDominatorTree)
INITIALIZE_PASS_END(PPUFixSGPRCopies, DEBUG_TYPE,
                     "PPU Fix SGPR copies", false, false)

char PPUFixSGPRCopies::ID = 0;

char &llvm::PPUFixSGPRCopiesID = PPUFixSGPRCopies::ID;

FunctionPass *llvm::createPPUFixSGPRCopiesPass() {
  return new PPUFixSGPRCopies();
}
...
static bool isVGPRToSGPRCopy(const TargetRegisterClass *SrcRC,
                             ...
                             const PPURegisterInfo &TRI) {
  return SrcRC != &PPU::VReg_1RegClass && TRI.isSGPRClass(DstRC) &&
...
static bool isSGPRToVGPRCopy(const TargetRegisterClass *SrcRC,
                             const TargetRegisterClass *DstRC,
                             const PPURegisterInfo &TRI) {
  return DstRC != &PPU::VReg_1RegClass && TRI.isSGPRClass(SrcRC) &&
...
static bool foldVGPRCopyIntoRegSequence(MachineInstr &MI,
  ...
  for (unsigned I = 1, N = MI.getNumOperands(); I != N; I += 2) {
    ...
    BuildMI(*MI.getParent(), &MI, MI.getDebugLoc(), TII->get(PPU::COPY),
            TmpReg)
        .add(MI.getOperand(I));
/*
    if (IsAGPR) {
      ...
      unsigned Opc = NewSrcRC == &PPU::AGPR_32RegClass ?
        PPU::V_ACCVGPR_WRITE_B32 : PPU::COPY;
...
static bool isSafeToFoldImmIntoCopy(const MachineInstr *Copy,
  ...
  const MachineOperand *ImmOp =
      TII->getNamedOperand(*MoveImm, PPU::OpName::src0);
  if (!ImmOp->isImm())
    return false;
  ...
  case PPU::V_MOV_B32_e32:
    SMovOp = PPU::S_MOV_B32;
    break;
  case PPU::V_MOV_B64_PSEUDO:
    SMovOp = PPU::S_MOV_B64;
    break;
  }
  Imm = ImmOp->getImm();
  return true;
...
bool PPUFixSGPRCopies::runOnMachineFunction(MachineFunction &MF) {
    ...
    for (MachineBasicBlock::iterator I = MBB.begin(), E = MBB.end();
      ...
      case PPU::WWM: {
        ...
        if (!Register::isVirtualRegister(DstReg)) {
          ...
          if (DstReg == PPU::M0 && TRI->hasVectorRegisters(SrcRC)) {
            Register TmpReg
              = MRI.createVirtualRegister(&PPU::SReg_32RegClass);

            BuildMI(MBB, MI, MI.getDebugLoc(),
                    TII->get(PPU::V_READFIRSTLANE_B32), TmpReg)
              .add(MI.getOperand(1));
  ...
  if (MF.getTarget().getOptLevel() > CodeGenOpt::None && EnableM0Merge)
    hoistAndMergeSGPRInits(PPU::M0, MRI, *MDT, TII);
