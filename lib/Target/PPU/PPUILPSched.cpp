...
unsigned PPUILPScheduler::getNodePriority(const SUnit *SU) const {
...
const SUnit *PPUILPScheduler::pickBest(const SUnit *left, const SUnit *right)
{
...
PPUILPScheduler::Candidate* PPUILPScheduler::pickCandidate() {
  if (AvailQueue.empty())
    return nullptr;
...
void PPUILPScheduler::releasePending() {
...
void PPUILPScheduler::advanceToCycle(unsigned NextCycle) {
...
void PPUILPScheduler::releasePredecessors(const SUnit* SU) {
...
PPUILPScheduler::schedule(ArrayRef<const SUnit*> BotRoots,
                          const ScheduleDAG &DAG) {
