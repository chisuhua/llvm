...
#define PPU_LOG2E_F     1.44269504088896340735992468100189214f
#define PPU_LN2_F       0.693147180559945309417232121458176568f
#define PPU_LN10_F      2.30258509299404568401799145468436421f
...
#define DEBUG_TYPE "ppu-base-lower"

STATISTIC(NumTailCalls, "Number of tail calls");

PPUBaseTargetLowering::PPUBaseTargetLowering(const TargetMachine &TM,
  ...
  PPUABI::ABI ABI = Subtarget.getTargetABI();
  ...
  addRegisterClass(XLenVT, &PPU::GPRRegClass);
...
/*
  ...
  */
  ...
  // call PPT part
  PPUBaseTargetLowering_compute();
}

EVT PPUBaseTargetLowering::getSetCCResultType(const DataLayout &DL, LLVMContext &,
                                            EVT VT) const {
  if (!VT.isVector())
...
bool PPUBaseTargetLowering::getTgtMemIntrinsic(IntrinsicInfo &Info,
                                             const CallInst &I,
                                             MachineFunction &MF,
  ...
  default:
    return false;
  case Intrinsic::ppu_masked_atomicrmw_xchg_i32:
  case Intrinsic::ppu_masked_atomicrmw_add_i32:
  case Intrinsic::ppu_masked_atomicrmw_sub_i32:
  case Intrinsic::ppu_masked_atomicrmw_nand_i32:
  case Intrinsic::ppu_masked_atomicrmw_max_i32:
  case Intrinsic::ppu_masked_atomicrmw_min_i32:
  case Intrinsic::ppu_masked_atomicrmw_umax_i32:
  case Intrinsic::ppu_masked_atomicrmw_umin_i32:
  case Intrinsic::ppu_masked_cmpxchg_i32:
...
bool PPUBaseTargetLowering::isLegalAddressingMode(const DataLayout &DL,
...
bool PPUBaseTargetLowering::isLegalICmpImmediate(int64_t Imm) const {
  return isInt<12>(Imm);
}

bool PPUBaseTargetLowering::isLegalAddImmediate(int64_t Imm) const {
  return isInt<12>(Imm);
}
...
// just be used.
bool PPUBaseTargetLowering::isTruncateFree(Type *SrcTy, Type *DstTy) const {
...
bool PPUBaseTargetLowering::hasBitPreservingFPLogic(EVT VT) const {
  return (VT == MVT::f32 && Subtarget.hasStdExtF()) ||
         (VT == MVT::f64 && Subtarget.hasStdExtD());
}

// Changes the condition code and swaps operands if necessary, so the SetCC
// operation matches one of the comparisons supported directly in the PPU
// ISA.
...
// Return the PPU branch opcode that matches the given DAG integer
// condition code. The CondCode must be one of those supported by the PPU
...
SDValue PPUBaseTargetLowering::LowerOperation(SDValue Op,
...
SDValue PPUBaseTargetLowering::getAddr(NodeTy *N, SelectionDAG &DAG,
                                     bool IsLocal) const {
  SDLoc DL(N);
  ...
  if (isPositionIndependent()) {
    ...
    if (IsLocal)
      ...
      // %pcrel_lo(auipc)).
      return SDValue(DAG.getMachineNode(PPU::PseudoLLA, DL, Ty, Addr), 0);

    // Use PC-relative addressing to access the GOT for this symbol, then load
    // the address from the GOT. This generates the pattern (PseudoLA sym),
    // which expands to (ld (addi (auipc %got_pcrel_hi(sym)) %pcrel_lo(auipc))).
    return SDValue(DAG.getMachineNode(PPU::PseudoLA, DL, Ty, Addr), 0);
  }
  ...
  case CodeModel::Small: {
    ...
    SDValue AddrHi = getTargetNode(N, DL, Ty, DAG, PPUII::MO_HI);
    SDValue AddrLo = getTargetNode(N, DL, Ty, DAG, PPUII::MO_LO);
    SDValue MNHi = SDValue(DAG.getMachineNode(PPU::LUI, DL, Ty, AddrHi), 0);
    return SDValue(DAG.getMachineNode(PPU::ADDI, DL, Ty, MNHi, AddrLo), 0);
  }
...
SDValue PPUBaseTargetLowering::lowerGlobalAddress(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue PPUBaseTargetLowering::lowerBlockAddress(SDValue Op,
                                               SelectionDAG &DAG) const {
...
SDValue PPUBaseTargetLowering::lowerConstantPool(SDValue Op,
                                               SelectionDAG &DAG) const {
...
SDValue PPUBaseTargetLowering::getStaticTLSAddr(GlobalAddressSDNode *N,
                                              SelectionDAG &DAG,
                                              bool UseGOT) const {
  SDLoc DL(N);
  EVT Ty = getPointerTy(DAG.getDataLayout());
  ...
  if (UseGOT) {
    ...
    SDValue Load =
        SDValue(DAG.getMachineNode(PPU::PseudoLA_TLS_IE, DL, Ty, Addr), 0);

    // Add the thread pointer.
    SDValue TPReg = DAG.getRegister(PPU::X4, XLenVT);
  ...
  SDValue AddrHi =
      DAG.getTargetGlobalAddress(GV, DL, Ty, 0, PPUII::MO_TPREL_HI);
  SDValue AddrAdd =
      DAG.getTargetGlobalAddress(GV, DL, Ty, 0, PPUII::MO_TPREL_ADD);
  SDValue AddrLo =
      DAG.getTargetGlobalAddress(GV, DL, Ty, 0, PPUII::MO_TPREL_LO);

  SDValue MNHi = SDValue(DAG.getMachineNode(PPU::LUI, DL, Ty, AddrHi), 0);
  SDValue TPReg = DAG.getRegister(PPU::X4, XLenVT);
  SDValue MNAdd = SDValue(
      DAG.getMachineNode(PPU::PseudoAddTPRel, DL, Ty, MNHi, TPReg, AddrAdd),
      0);
  return SDValue(DAG.getMachineNode(PPU::ADDI, DL, Ty, MNAdd, AddrLo), 0);
}

SDValue PPUBaseTargetLowering::getDynamicTLSAddr(GlobalAddressSDNode *N,
                                               SelectionDAG &DAG) const {
  SDLoc DL(N);
  EVT Ty = getPointerTy(DAG.getDataLayout());
  ...
  SDValue Addr = DAG.getTargetGlobalAddress(GV, DL, Ty, 0, 0);
  SDValue Load =
      SDValue(DAG.getMachineNode(PPU::PseudoLA_TLS_GD, DL, Ty, Addr), 0);
...
SDValue PPUBaseTargetLowering::lowerGlobalTLSAddress(SDValue Op,
                                                   SelectionDAG &DAG) const {
  SDLoc DL(Op);
  EVT Ty = Op.getValueType();
  GlobalAddressSDNode *N = cast<GlobalAddressSDNode>(Op);
...
SDValue PPUBaseTargetLowering::lowerSELECT(SDValue Op, SelectionDAG &DAG) const {
  ...
  // lowered PPUISD::SELECT_CC to take advantage of the integer
  // compare+branch instructions. i.e.:
  // (select (setcc lhs, rhs, cc), truev, falsev)
  // -> (ppuisd::select_cc lhs, rhs, cc, truev, falsev)
  if (Op.getSimpleValueType() == XLenVT && CondV.getOpcode() == ISD::SETCC &&
    ...
    return DAG.getNode(PPUISD::SELECT_CC, DL, VTs, Ops);
  }

  // Otherwise:
  // (select condv, truev, falsev)
  // -> (ppuisd::select_cc condv, zero, setne, truev, falsev)
...
SDValue PPUBaseTargetLowering::lowerVASTART(SDValue Op, SelectionDAG &DAG) const {
...
SDValue PPUBaseTargetLowering::lowerFRAMEADDR(SDValue Op,
                                            SelectionDAG &DAG) const {
...
SDValue PPUBaseTargetLowering::lowerRETURNADDR(SDValue Op,
                                             SelectionDAG &DAG) const {
...
SDValue PPUBaseTargetLowering::lowerShiftLeftParts(SDValue Op,
                                                 SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue PPUBaseTargetLowering::lowerShiftRightParts(SDValue Op, SelectionDAG &DAG,
                                                  bool IsSRA) const {
  SDLoc DL(Op);
  SDValue Lo = Op.getOperand(0);
  SDValue Hi = Op.getOperand(1);
...
static PPUISD::NodeType getPPUWOpcode(unsigned Opcode) {
  switch (Opcode) {
  default:
...
static SDValue customLegalizeToWOp(SDNode *N, SelectionDAG &DAG) {
  SDLoc DL(N);
  PPUISD::NodeType WOpcode = getPPUWOpcode(N->getOpcode());
...
void PPUBaseTargetLowering::ReplaceNodeResults(SDNode *N,
  ...
  case ISD::READCYCLECOUNTER: {
    assert(!Subtarget.is64Bit() &&
           "READCYCLECOUNTER only has custom type legalization on ppu32");

    SDVTList VTs = DAG.getVTList(MVT::i32, MVT::i32, MVT::Other);
    SDValue RCW =
        DAG.getNode(PPUISD::READ_CYCLE_WIDE, DL, VTs, N->getOperand(0));
  ...
  case ISD::BITCAST: {
    ...
    if (Op0.getValueType() != MVT::f32)
      return;
    SDValue FPConv =
        DAG.getNode(PPUISD::FMV_X_ANYEXTW_RV64, DL, MVT::i64, Op0);
...
SDValue PPUBaseTargetLowering::PerformDAGCombine(SDNode *N,
  ...
  case PPUISD::SplitF64: {
    ...
    if (Op0->getOpcode() == PPUISD::BuildPairF64)
    ...
    if (!(Op0.getOpcode() == ISD::FNEG || Op0.getOpcode() == ISD::FABS) ||
      ...
      break;
    SDValue NewSplitF64 =
        DAG.getNode(PPUISD::SplitF64, DL, DAG.getVTList(MVT::i32, MVT::i32),
                    Op0.getOperand(0));
  ...
  case PPUISD::FMV_X_ANYEXTW_RV64: {
    ...
    if (Op0->getOpcode() == PPUISD::FMV_W_X_RV64) {
      SDValue AExtOp =
    ...
    if (!(Op0.getOpcode() == ISD::FNEG || Op0.getOpcode() == ISD::FABS) ||
        !Op0.getNode()->hasOneUse())
      break;
    SDValue NewFMV = DAG.getNode(PPUISD::FMV_X_ANYEXTW_RV64, DL, MVT::i64,
                                 Op0.getOperand(0));
...
SDValue PPUBaseTargetLowering::lowerINTRINSIC_WO_CHAIN(SDValue Op,
                                                     SelectionDAG &DAG) const {
...
SDValue PPUBaseTargetLowering::lowerSPLAT_VECTOR(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  EVT VT = Op.getValueType();
...
SDValue PPUBaseTargetLowering::lowerSETVL(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT XLenVT = Subtarget.getXLenVT();
...
bool PPUBaseTargetLowering::isDesirableToCommuteWithShift(
  ...
  if (Ty.isScalarInteger() &&
    ...
    if (C1 && C2) {
      ...
      // costs.
      int C1Cost = PPUMatInt::getIntMatCost(C1Int, Ty.getSizeInBits(),
                                              Subtarget.is64Bit());
      int ShiftedC1Cost = PPUMatInt::getIntMatCost(
...
unsigned PPUBaseTargetLowering::ComputeNumSignBitsForTargetNode(
...
static MachineBasicBlock *emitReadCycleWidePseudo(MachineInstr &MI,
  ...
  assert(MI.getOpcode() == PPU::ReadCycleWide && "Unexpected instruction");
  ...
  Register ReadAgainReg = RegInfo.createVirtualRegister(&PPU::GPRRegClass);
  Register LoReg = MI.getOperand(0).getReg();
  Register HiReg = MI.getOperand(1).getReg();
  DebugLoc DL = MI.getDebugLoc();

  const TargetInstrInfo *TII = MF.getSubtarget().getInstrInfo();
  BuildMI(LoopMBB, DL, TII->get(PPU::CSRRS), HiReg)
      .addImm(PPUSysReg::lookupSysRegByName("CYCLEH")->Encoding)
      .addReg(PPU::X0);
  BuildMI(LoopMBB, DL, TII->get(PPU::CSRRS), LoReg)
      .addImm(PPUSysReg::lookupSysRegByName("CYCLE")->Encoding)
      .addReg(PPU::X0);
  BuildMI(LoopMBB, DL, TII->get(PPU::CSRRS), ReadAgainReg)
      .addImm(PPUSysReg::lookupSysRegByName("CYCLEH")->Encoding)
      .addReg(PPU::X0);
...
static MachineBasicBlock *emitSplitF64Pseudo(MachineInstr &MI,
  ...
  const TargetRegisterClass *SrcRC = &PPU::FPR64RegClass;
  int FI = MF.getInfo<PPUMachineFunctionInfo>()->getMoveF64FrameIndex();
  ...
  BuildMI(*BB, MI, DL, TII.get(PPU::LW), LoReg)
      .addFrameIndex(FI)
      .addImm(0)
      .addMemOperand(MMO);
  BuildMI(*BB, MI, DL, TII.get(PPU::LW), HiReg)
      .addFrameIndex(FI)
      .addImm(4)
      .addMemOperand(MMO);
...
static MachineBasicBlock *emitBuildPairF64Pseudo(MachineInstr &MI,
                                                 MachineBasicBlock *BB) {
  assert(MI.getOpcode() == PPU::BuildPairF64Pseudo &&
  ...
  const TargetRegisterClass *DstRC = &PPU::FPR64RegClass;
  int FI = MF.getInfo<PPUMachineFunctionInfo>()->getMoveF64FrameIndex();

  MachineMemOperand *MMO =
      MF.getMachineMemOperand(MachinePointerInfo::getFixedStack(MF, FI),
                              MachineMemOperand::MOStore, 8, 8);
  BuildMI(*BB, MI, DL, TII.get(PPU::SW))
  ...
  while (SelectMBBI != SelectEnd) {
    ...
    if (isSelectPseudo(*SelectMBBI)) {
      ...
      BuildMI(*TailMBB, InsertionPoint, SelectMBBI->getDebugLoc(),
              TII.get(PPU::PHI), SelectMBBI->getOperand(0).getReg())
...
MachineBasicBlock *
PPUBaseTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
                                                 MachineBasicBlock *BB) const {
  switch (MI.getOpcode()) {
  default:
    llvm_unreachable("Unexpected instr type to insert");
  case PPU::ReadCycleWide:
    assert(!Subtarget.is64Bit() &&
           "ReadCycleWrite is only to be used on ppu32");
...
static const MCPhysReg ArgGPRs[] = {
  PPU::X10, PPU::X11, PPU::X12, PPU::X13,
  PPU::X14, PPU::X15, PPU::X16, PPU::X17
};
...
static bool CC_PPUAssign2XLen(unsigned XLen, CCState &State, CCValAssign VA1,
...
// Implements the PPU calling convention. Returns true upon failure.
static bool CC_PPU(const DataLayout &DL, PPUABI::ABI ABI, unsigned ValNo,
  ...
  // If this is a variadic argument, the PPU calling convention requires
  ...
  if (ArgFlags.isSplitEnd() && PendingLocs.size() <= 2) {
    ...
    return CC_PPUAssign2XLen(XLen, State, VA, AF, ValNo, ValVT, LocVT,
                               ArgFlags);
  }
...
void PPUBaseTargetLowering::analyzeInputArgs(
  ...
  for (unsigned i = 0; i != NumArgs; ++i) {
    ...
    PPUABI::ABI ABI = MF.getSubtarget<PPUSubtarget>().getTargetABI();
    if (CC_PPU(MF.getDataLayout(), ABI, i, ArgVT, ArgVT, CCValAssign::Full,
...
void PPUBaseTargetLowering::analyzeOutputArgs(
    MachineFunction &MF, CCState &CCInfo,
...
static SDValue convertLocVTToValVT(SelectionDAG &DAG, SDValue Val,
  ...
  case CCValAssign::BCvt:
    if (VA.getLocVT() == MVT::i64 && VA.getValVT() == MVT::f32) {
      Val = DAG.getNode(PPUISD::FMV_W_X_RV64, DL, MVT::f32, Val);
      break;
    }
...
static SDValue convertValVTToLocVT(SelectionDAG &DAG, SDValue Val,
  ...
  case CCValAssign::BCvt:
    if (VA.getLocVT() == MVT::i64 && VA.getValVT() == MVT::f32) {
      Val = DAG.getNode(PPUISD::FMV_X_ANYEXTW_RV64, DL, MVT::i64, Val);
      break;
    }
...
static SDValue unpackF64OnRV32DSoftABI(SelectionDAG &DAG, SDValue Chain,
  ...
  Register LoVReg = RegInfo.createVirtualRegister(&PPU::GPRRegClass);
  RegInfo.addLiveIn(VA.getLocReg(), LoVReg);
  SDValue Lo = DAG.getCopyFromReg(Chain, DL, LoVReg, MVT::i32);
  SDValue Hi;
  if (VA.getLocReg() == PPU::X17) {
    ...
    Register HiVReg = RegInfo.createVirtualRegister(&PPU::GPRRegClass);
    RegInfo.addLiveIn(VA.getLocReg() + 1, HiVReg);
    Hi = DAG.getCopyFromReg(Chain, DL, HiVReg, MVT::i32);
  }
  return DAG.getNode(PPUISD::BuildPairF64, DL, MVT::f64, Lo, Hi);
}

// Transform physical registers into virtual registers.
SDValue PPUBaseTargetLowering::LowerFormalArguments(
...
*/
  ...
  if (IsVarArg) {
    ...
    MachineFrameInfo &MFI = MF.getFrameInfo();
    MachineRegisterInfo &RegInfo = MF.getRegInfo();
    PPUMachineFunctionInfo *RVFI = MF.getInfo<PPUMachineFunctionInfo>();
...
bool PPUBaseTargetLowering::isEligibleForTailCallOptimization(
  ...
  // probably break this.
  // TODO: The "interrupt" attribute isn't currently defined by PPU. This
...
SDValue PPUBaseTargetLowering::LowerCall(CallLoweringInfo &CLI,
                                       SmallVectorImpl<SDValue> &InVals) const {
/*
    if (isCompute(CLI.CallConv))
        return LowerCall_compute(CLI, InVals);
*/
  ...
  for (unsigned i = 0, j = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (IsF64OnRV32DSoftABI && VA.isRegLoc()) {
      SDValue SplitF64 = DAG.getNode(
          PPUISD::SplitF64, DL, DAG.getVTList(MVT::i32, MVT::i32), ArgValue);
      ...
      if (RegLo == PPU::X17) {
        ...
        if (!StackPtr.getNode())
          StackPtr = DAG.getCopyFromReg(Chain, DL, PPU::X2, PtrVT);
        // Emit the store.
  ...
  if (GlobalAddressSDNode *S = dyn_cast<GlobalAddressSDNode>(Callee)) {
    ...
    unsigned OpFlags = PPUII::MO_CALL;
    if (!getTargetMachine().shouldAssumeDSOLocal(*GV->getParent(), GV))
      OpFlags = PPUII::MO_PLT;
  ...
  if (IsTailCall) {
    ...
    return DAG.getNode(PPUISD::TAIL, DL, NodeTys, Ops);
  }

  Chain = DAG.getNode(PPUISD::CALL, DL, NodeTys, Ops);
  Glue = Chain.getValue(1);
  ...
  for (auto &VA : RVLocs) {
    ...
    if (VA.getLocVT() == MVT::i32 && VA.getValVT() == MVT::f64) {
      ...
      RetValue = DAG.getNode(PPUISD::BuildPairF64, DL, MVT::f64, RetValue,
                             RetValue2);
    }
...
bool PPUBaseTargetLowering::CanLowerReturn(
  ...
  for (unsigned i = 0, e = Outs.size(); i != e; ++i) {
    ...
    ISD::ArgFlagsTy ArgFlags = Outs[i].Flags;
    PPUABI::ABI ABI = MF.getSubtarget<PPUSubtarget>().getTargetABI();
    if (CC_PPU(MF.getDataLayout(), ABI, i, VT, VT, CCValAssign::Full,
...
SDValue
PPUBaseTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                 bool IsVarArg,
                                 const SmallVectorImpl<ISD::OutputArg> &Outs,
                                 const SmallVectorImpl<SDValue> &OutVals,
                                 const SDLoc &DL, SelectionDAG &DAG) const {
  if (PPU::isCompute(CallConv))
      return LowerReturn_compute(Chain, CallConv, IsVarArg,
                                Outs, OutVals, DL, DAG);
  ...
  for (unsigned i = 0, e = RVLocs.size(); i < e; ++i) {
    ...
    if (VA.getLocVT() == MVT::i32 && VA.getValVT() == MVT::f64) {
      ...
      SDValue SplitF64 = DAG.getNode(PPUISD::SplitF64, DL,
  ...
  if (Func.hasFnAttribute("interrupt")) {
    ...
    unsigned RetOpc;
    if (Kind == "user")
      RetOpc = PPUISD::URET_FLAG;
    else if (Kind == "supervisor")
      RetOpc = PPUISD::SRET_FLAG;
    else
      RetOpc = PPUISD::MRET_FLAG;
...
PPUBaseTargetLowering::ConstraintType
PPUBaseTargetLowering::getConstraintType(StringRef Constraint) const {
...
PPUBaseTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
  ...
  if (Constraint.size() == 1) {
    switch (Constraint[0]) {
    case 'r':
      return std::make_pair(0U, &PPU::GPRRegClass);
    case 'f':
  ...
  if (XRegFromAlias != PPU::NoRegister)
    return std::make_pair(XRegFromAlias, &PPU::GPRRegClass);
  ...
  if (Subtarget.hasStdExtF() || Subtarget.hasStdExtD()) {
    std::pair<Register, Register> FReg =
        StringSwitch<std::pair<Register, Register>>(Constraint.lower())
            .Cases("{f0}", "{ft0}", {PPU::F0_32, PPU::F0_64})
            .Cases("{f1}", "{ft1}", {PPU::F1_32, PPU::F1_64})
            .Cases("{f2}", "{ft2}", {PPU::F2_32, PPU::F2_64})
            .Cases("{f3}", "{ft3}", {PPU::F3_32, PPU::F3_64})
            .Cases("{f4}", "{ft4}", {PPU::F4_32, PPU::F4_64})
            .Cases("{f5}", "{ft5}", {PPU::F5_32, PPU::F5_64})
            .Cases("{f6}", "{ft6}", {PPU::F6_32, PPU::F6_64})
            .Cases("{f7}", "{ft7}", {PPU::F7_32, PPU::F7_64})
            .Cases("{f8}", "{fs0}", {PPU::F8_32, PPU::F8_64})
            .Cases("{f9}", "{fs1}", {PPU::F9_32, PPU::F9_64})
            .Cases("{f10}", "{fa0}", {PPU::F10_32, PPU::F10_64})
            .Cases("{f11}", "{fa1}", {PPU::F11_32, PPU::F11_64})
            .Cases("{f12}", "{fa2}", {PPU::F12_32, PPU::F12_64})
            .Cases("{f13}", "{fa3}", {PPU::F13_32, PPU::F13_64})
            .Cases("{f14}", "{fa4}", {PPU::F14_32, PPU::F14_64})
            .Cases("{f15}", "{fa5}", {PPU::F15_32, PPU::F15_64})
            .Cases("{f16}", "{fa6}", {PPU::F16_32, PPU::F16_64})
            .Cases("{f17}", "{fa7}", {PPU::F17_32, PPU::F17_64})
            .Cases("{f18}", "{fs2}", {PPU::F18_32, PPU::F18_64})
            .Cases("{f19}", "{fs3}", {PPU::F19_32, PPU::F19_64})
            .Cases("{f20}", "{fs4}", {PPU::F20_32, PPU::F20_64})
            .Cases("{f21}", "{fs5}", {PPU::F21_32, PPU::F21_64})
            .Cases("{f22}", "{fs6}", {PPU::F22_32, PPU::F22_64})
            .Cases("{f23}", "{fs7}", {PPU::F23_32, PPU::F23_64})
            .Cases("{f24}", "{fs8}", {PPU::F24_32, PPU::F24_64})
            .Cases("{f25}", "{fs9}", {PPU::F25_32, PPU::F25_64})
            .Cases("{f26}", "{fs10}", {PPU::F26_32, PPU::F26_64})
            .Cases("{f27}", "{fs11}", {PPU::F27_32, PPU::F27_64})
            .Cases("{f28}", "{ft8}", {PPU::F28_32, PPU::F28_64})
            .Cases("{f29}", "{ft9}", {PPU::F29_32, PPU::F29_64})
            .Cases("{f30}", "{ft10}", {PPU::F30_32, PPU::F30_64})
            .Cases("{f31}", "{ft11}", {PPU::F31_32, PPU::F31_64})
            .Default({PPU::NoRegister, PPU::NoRegister});
    if (FReg.first != PPU::NoRegister)
...
unsigned
PPUBaseTargetLowering::getInlineAsmMemConstraint(StringRef ConstraintCode) const {
...
void PPUBaseTargetLowering::LowerAsmOperandForConstraint(
...
Instruction *PPUBaseTargetLowering::emitLeadingFence(IRBuilder<> &Builder,
                                                   Instruction *Inst,
...
Instruction *PPUBaseTargetLowering::emitTrailingFence(IRBuilder<> &Builder,
                                                    Instruction *Inst,
                                                    AtomicOrdering Ord) const {
...
PPUBaseTargetLowering::shouldExpandAtomicRMWInIR(AtomicRMWInst *AI) const {
...
getIntrinsicForMaskedAtomicRMWBinOp(unsigned XLen, AtomicRMWInst::BinOp BinOp) {
  if (XLen == 32) {
    ...
    case AtomicRMWInst::Xchg:
      return Intrinsic::ppu_masked_atomicrmw_xchg_i32;
    case AtomicRMWInst::Add:
      return Intrinsic::ppu_masked_atomicrmw_add_i32;
    case AtomicRMWInst::Sub:
      return Intrinsic::ppu_masked_atomicrmw_sub_i32;
    case AtomicRMWInst::Nand:
      return Intrinsic::ppu_masked_atomicrmw_nand_i32;
    case AtomicRMWInst::Max:
      return Intrinsic::ppu_masked_atomicrmw_max_i32;
    case AtomicRMWInst::Min:
      return Intrinsic::ppu_masked_atomicrmw_min_i32;
    case AtomicRMWInst::UMax:
      return Intrinsic::ppu_masked_atomicrmw_umax_i32;
    case AtomicRMWInst::UMin:
      return Intrinsic::ppu_masked_atomicrmw_umin_i32;
    }
  }
  ...
  if (XLen == 64) {
    ...
    default:
      llvm_unreachable("Unexpected AtomicRMW BinOp");
    case AtomicRMWInst::Xchg:
      return Intrinsic::ppu_masked_atomicrmw_xchg_i64;
    case AtomicRMWInst::Add:
      return Intrinsic::ppu_masked_atomicrmw_add_i64;
    case AtomicRMWInst::Sub:
      return Intrinsic::ppu_masked_atomicrmw_sub_i64;
    case AtomicRMWInst::Nand:
      return Intrinsic::ppu_masked_atomicrmw_nand_i64;
    case AtomicRMWInst::Max:
      return Intrinsic::ppu_masked_atomicrmw_max_i64;
    case AtomicRMWInst::Min:
      return Intrinsic::ppu_masked_atomicrmw_min_i64;
    case AtomicRMWInst::UMax:
      return Intrinsic::ppu_masked_atomicrmw_umax_i64;
    case AtomicRMWInst::UMin:
      return Intrinsic::ppu_masked_atomicrmw_umin_i64;
    }
  }
...
Value *PPUBaseTargetLowering::emitMaskedAtomicRMWIntrinsic(
...
TargetLowering::AtomicExpansionKind
PPUBaseTargetLowering::shouldExpandAtomicCmpXchgInIR(
...
Value *PPUBaseTargetLowering::emitMaskedAtomicCmpXchgIntrinsic(
    IRBuilder<> &Builder, AtomicCmpXchgInst *CI, Value *AlignedAddr,
    Value *CmpVal, Value *NewVal, Value *Mask, AtomicOrdering Ord) const {
  unsigned XLen = Subtarget.getXLen();
  Value *Ordering = Builder.getIntN(XLen, static_cast<uint64_t>(Ord));
  Intrinsic::ID CmpXchgIntrID = Intrinsic::ppu_masked_cmpxchg_i32;
  if (XLen == 64) {
    CmpVal = Builder.CreateSExt(CmpVal, Builder.getInt64Ty());
    NewVal = Builder.CreateSExt(NewVal, Builder.getInt64Ty());
    Mask = Builder.CreateSExt(Mask, Builder.getInt64Ty());
    CmpXchgIntrID = Intrinsic::ppu_masked_cmpxchg_i64;
  }
  Type *Tys[] = {AlignedAddr->getType()};
...
unsigned PPUBaseTargetLowering::getExceptionPointerRegister(
    const Constant *PersonalityFn) const {
  return PPU::X10;
}

unsigned PPUBaseTargetLowering::getExceptionSelectorRegister(
    const Constant *PersonalityFn) const {
  return PPU::X11;
}

bool PPUBaseTargetLowering::shouldExtendTypeInLibCall(EVT Type) const {
  // Return false to suppress the unnecessary extensions if the LibCall
  // arguments or return value is f32 type for LP64 ABI.
  PPUABI::ABI ABI = Subtarget.getTargetABI();
  if (ABI == PPUABI::ABI_LP64 && (Type == MVT::f32))
    return false;
...
//=================================================
// below are from AMDGPUISelLowering
//
// Find a larger type to do a load / store of a vector with.
EVT PPUBaseTargetLowering::getEquivalentMemType(LLVMContext &Ctx, EVT VT) {
...
unsigned PPUBaseTargetLowering::numBitsUnsigned(SDValue Op, SelectionDAG &DAG) {
  EVT VT = Op.getValueType();
  KnownBits Known = DAG.computeKnownBits(Op);
  return VT.getSizeInBits() - Known.countMinLeadingZeros();
}

unsigned PPUBaseTargetLowering::numBitsSigned(SDValue Op, SelectionDAG &DAG) {
  EVT VT = Op.getValueType();
...
void PPUBaseTargetLowering::PPUBaseTargetLowering_compute() {
   // : TargetLowering(TM), Subtarget(&STI) {
...
bool PPUBaseTargetLowering::allUsesHaveSourceMods(const SDNode *N,
...
CCAssignFn *PPUBaseTargetLowering::CCAssignFnForCall(CallingConv::ID CC,
                                                    bool IsVarArg) {
  return PPUCallLowering::CCAssignFnForCall(CC, IsVarArg);
}

CCAssignFn *PPUBaseTargetLowering::CCAssignFnForReturn(CallingConv::ID CC,
                                                      bool IsVarArg) {
...
/// the ValVT.
void PPUBaseTargetLowering::analyzeFormalArgumentsCompute(
  CCState &State,
...
SDValue PPUBaseTargetLowering::LowerReturn_compute(
  ...
  return DAG.getNode(PPUISD::ENDPGM, DL, MVT::Other, Chain);
}

SDValue PPUBaseTargetLowering::addTokenForArgument(SDValue Chain,
                                                  SelectionDAG &DAG,
...
SDValue PPUBaseTargetLowering::lowerUnhandledCall(CallLoweringInfo &CLI,
...
#if 0
// TODO it is not called by PPUISelLowering
SDValue PPUBaseTargetLowering::LowerCall_compute(CallLoweringInfo &CLI,
                                        SmallVectorImpl<SDValue> &InVals) const {
...
SDValue PPUBaseTargetLowering::LowerOperation_compute(SDValue Op,
  ...
  case ISD::FLOG:
    return LowerFLOG(Op, DAG, 1 / PPU_LOG2E_F);
  case ISD::FLOG10:
    return LowerFLOG(Op, DAG, PPU_LN2_F / PPU_LN10_F);
  case ISD::FEXP:
...
#if 0
// it is not called by PPUISelLowering
void PPUBaseTargetLowering::ReplaceNodeResults_compute(SDNode *N,
                                              SmallVectorImpl<SDValue> &Results,
                                              SelectionDAG &DAG) const {
  switch (N->getOpcode()) {
...
SDValue PPUBaseTargetLowering::LowerCONCAT_VECTORS(SDValue Op,
                                                  SelectionDAG &DAG) const {
...
SDValue PPUBaseTargetLowering::LowerEXTRACT_SUBVECTOR(SDValue Op,
                                                     SelectionDAG &DAG) const {
...
SDValue PPUBaseTargetLowering::combineFMinMaxLegacy(const SDLoc &DL, EVT VT,
  ...
  case ISD::SETULT: {
    if (LHS == True)
      return DAG.getNode(PPUISD::FMIN_LEGACY, DL, VT, RHS, LHS);
    return DAG.getNode(PPUISD::FMAX_LEGACY, DL, VT, LHS, RHS);
  }
  case ISD::SETOLE:
  case ISD::SETOLT:
  case ISD::SETLE:
  case ISD::SETLT: {
    ...
    if (LHS == True)
      return DAG.getNode(PPUISD::FMIN_LEGACY, DL, VT, LHS, RHS);
    return DAG.getNode(PPUISD::FMAX_LEGACY, DL, VT, RHS, LHS);
  }
  case ISD::SETUGE:
  case ISD::SETUGT: {
    if (LHS == True)
...
SDValue PPUBaseTargetLowering::SplitVectorLoad(const SDValue Op,
                                              SelectionDAG &DAG) const {
...
SDValue PPUBaseTargetLowering::WidenVectorLoad(SDValue Op,
                                              SelectionDAG &DAG) const {
  LoadSDNode *Load = cast<LoadSDNode>(Op);
  EVT VT = Op.getValueType();
...
SDValue PPUBaseTargetLowering::SplitVectorStore(SDValue Op,
                                               SelectionDAG &DAG) const {
...
SDValue PPUBaseTargetLowering::LowerDIVREM24(SDValue Op, SelectionDAG &DAG,
  ...
  SDValue fq = DAG.getNode(ISD::FMUL, DL, FltVT,
                           fa, DAG.getNode(PPUISD::RCP, DL, FltVT, fb));
...
void PPUBaseTargetLowering::LowerUDIVREM64(SDValue Op,
                                      SelectionDAG &DAG,
...
SDValue PPUBaseTargetLowering::LowerUDIVREM(SDValue Op,
                                           SelectionDAG &DAG) const {
  SDLoc DL(Op);
  EVT VT = Op.getValueType();
  ...
  // e is rounding error.
  SDValue RCP = DAG.getNode(PPUISD::URECIP, DL, VT, Den);
...
SDValue PPUBaseTargetLowering::LowerSDIVREM(SDValue Op,
                                           SelectionDAG &DAG) const {
  SDLoc DL(Op);
  EVT VT = Op.getValueType();
...
SDValue PPUBaseTargetLowering::LowerFREM(SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
  EVT VT = Op.getValueType();
...
SDValue PPUBaseTargetLowering::LowerFCEIL(SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
...
static SDValue extractF64Exponent(SDValue Hi, const SDLoc &SL,
  ...
  SDValue ExpPart = DAG.getNode(PPUISD::BFE_U32, SL, MVT::i32,
                                Hi,
...
SDValue PPUBaseTargetLowering::LowerFTRUNC(SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
  SDValue Src = Op.getOperand(0);
...
SDValue PPUBaseTargetLowering::LowerFRINT(SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
  SDValue Src = Op.getOperand(0);
...
SDValue PPUBaseTargetLowering::LowerFNEARBYINT(SDValue Op, SelectionDAG &DAG) const {
...
SDValue PPUBaseTargetLowering::LowerFROUND64(SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
  SDValue X = Op.getOperand(0);
...
SDValue PPUBaseTargetLowering::LowerFFLOOR(SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
  SDValue Src = Op.getOperand(0);
...
SDValue PPUBaseTargetLowering::LowerFLOG(SDValue Op, SelectionDAG &DAG,
...
// exp2(M_LOG2E_F * f);
SDValue PPUBaseTargetLowering::lowerFEXP(SDValue Op, SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
  SDLoc SL(Op);
  SDValue Src = Op.getOperand(0);
...
SDValue PPUBaseTargetLowering::LowerCTLZ_CTTZ(SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
  SDValue Src = Op.getOperand(0);
...
SDValue PPUBaseTargetLowering::LowerINT_TO_FP32(SDValue Op, SelectionDAG &DAG,
                                               bool Signed) const {
  // Unsigned
  // cul2f(ulong u)
  //{
...
SDValue PPUBaseTargetLowering::LowerINT_TO_FP64(SDValue Op, SelectionDAG &DAG,
                                               bool Signed) const {
  SDLoc SL(Op);
  SDValue Src = Op.getOperand(0);
  ...
  SDValue LdExp = DAG.getNode(PPUISD::LDEXP, SL, MVT::f64, CvtHi,
                              DAG.getConstant(32, SL, MVT::i32));
  // TODO: Should this propagate fast-math-flags?
  return DAG.getNode(ISD::FADD, SL, MVT::f64, LdExp, CvtLo);
}

SDValue PPUBaseTargetLowering::LowerUINT_TO_FP(SDValue Op,
                                               SelectionDAG &DAG) const {
...
SDValue PPUBaseTargetLowering::LowerSINT_TO_FP(SDValue Op,
                                              SelectionDAG &DAG) const {
  assert(Op.getOperand(0).getValueType() == MVT::i64 &&
         "operation should be legal");
...
SDValue PPUBaseTargetLowering::LowerFP64_TO_INT(SDValue Op, SelectionDAG &DAG,
                                               bool Signed) const {
  SDLoc SL(Op);
...
SDValue PPUBaseTargetLowering::LowerFP_TO_FP16(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue PPUBaseTargetLowering::LowerFP_TO_SINT(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDValue Src = Op.getOperand(0);
...
SDValue PPUBaseTargetLowering::LowerFP_TO_UINT(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDValue Src = Op.getOperand(0);
...
SDValue PPUBaseTargetLowering::LowerSIGN_EXTEND_INREG(SDValue Op,
                                                     SelectionDAG &DAG) const {
...
static bool isU24(SDValue Op, SelectionDAG &DAG) {
  return PPUBaseTargetLowering::numBitsUnsigned(Op, DAG) <= 24;
}
...
static bool isI24(SDValue Op, SelectionDAG &DAG) {
  ...
  return VT.getSizeInBits() >= 24 && // Types less than 24-bit should be treated
    ...
    PPUBaseTargetLowering::numBitsSigned(Op, DAG) < 24;
}
...
static MemSDNode* findMemSDNode(SDNode *N) {
  N = PPUBaseTargetLowering::stripBitcast(SDValue(N,0)).getNode();
  if (MemSDNode *MN = dyn_cast<MemSDNode>(N))
    return MN;
  assert(isa<BuildVectorSDNode>(N));
  for (SDValue V : N->op_values())
    if (MemSDNode *MN =
          dyn_cast<MemSDNode>(PPUBaseTargetLowering::stripBitcast(V)))
      return MN;
...
// type.
SDValue PPUBaseTargetLowering::performLoadCombine(SDNode *N,
                                                 DAGCombinerInfo &DCI) const {
...
// type.
SDValue PPUBaseTargetLowering::performStoreCombine(SDNode *N,
                                                  DAGCombinerInfo &DCI) const {
  if (!DCI.isBeforeLegalize())
    return SDValue();
...
// issues.
SDValue PPUBaseTargetLowering::performAssertSZExtCombine(SDNode *N,
                                                        DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  SDValue N0 = N->getOperand(0);
...
SDValue PPUBaseTargetLowering::performIntrinsicWOChainCombine(
...
SDValue PPUBaseTargetLowering::performSraCombine(SDNode *N,
                                                DAGCombinerInfo &DCI) const {
...
SDValue PPUBaseTargetLowering::performSrlCombine(SDNode *N,
                                                DAGCombinerInfo &DCI) const {
...
SDValue PPUBaseTargetLowering::performTruncateCombine(
  SDNode *N, DAGCombinerInfo &DCI) const {
  SDLoc SL(N);
  SelectionDAG &DAG = DCI.DAG;
  EVT VT = N->getValueType(0);
...
static SDValue getMul24(SelectionDAG &DAG, const SDLoc &SL,
  ...
  if (Size <= 32) {
    unsigned MulOpc = Signed ? PPUISD::MUL_I24 : PPUISD::MUL_U24;
  ...
  unsigned MulOpc = Signed ? PPUISD::MUL_LOHI_I24 : PPUISD::MUL_LOHI_U24;
...
SDValue PPUBaseTargetLowering::performMulCombine(SDNode *N,
                                                DAGCombinerInfo &DCI) const {
  EVT VT = N->getValueType(0);
...
SDValue PPUBaseTargetLowering::performMulhsCombine(SDNode *N,
                                                  DAGCombinerInfo &DCI) const {
  EVT VT = N->getValueType(0);
  ...
  SDValue Mulhi = DAG.getNode(PPUISD::MULHI_I24, DL, MVT::i32, N0, N1);
  DCI.AddToWorklist(Mulhi.getNode());
  return DAG.getSExtOrTrunc(Mulhi, DL, VT);
}

SDValue PPUBaseTargetLowering::performMulhuCombine(SDNode *N,
                                                  DAGCombinerInfo &DCI) const {
  EVT VT = N->getValueType(0);
  ...
  SDValue Mulhi = DAG.getNode(PPUISD::MULHI_U24, DL, MVT::i32, N0, N1);
  DCI.AddToWorklist(Mulhi.getNode());
  return DAG.getZExtOrTrunc(Mulhi, DL, VT);
}


SDValue PPUBaseTargetLowering::performMulLoHi24Combine(
  SDNode *N, DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  ...
  bool Signed = (N->getOpcode() == PPUISD::MUL_LOHI_I24);
...
SDValue PPUBaseTargetLowering::getFFBX_U32(SelectionDAG &DAG,
                                          SDValue Op,
                                          const SDLoc &DL,
                                          unsigned Opc) const {
  EVT VT = Op.getValueType();
...
SDValue PPUBaseTargetLowering::performCtlz_CttzCombine(const SDLoc &SL, SDValue Cond,
                                                 SDValue LHS, SDValue RHS,
                                                 DAGCombinerInfo &DCI) const {
  ...
  unsigned Opc = isCttzOpc(RHS.getOpcode()) ? PPUISD::FFBL_B32 :
                                           PPUISD::FFBH_U32;
...
SDValue PPUBaseTargetLowering::performSelectCombine(SDNode *N,
                                                   DAGCombinerInfo &DCI) const {
...
bool PPUBaseTargetLowering::isConstantCostlierToNegate(SDValue N) const {
...
static unsigned inverseMinMax(unsigned Opc) {
  ...
  case PPUISD::FMAX_LEGACY:
    return PPUISD::FMIN_LEGACY;
  case PPUISD::FMIN_LEGACY:
    return  PPUISD::FMAX_LEGACY;
    */
  default:
...
SDValue PPUBaseTargetLowering::performFNegCombine(SDNode *N,
                                                 DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  SDValue N0 = N->getOperand(0);
  EVT VT = N->getValueType(0);
...
/*case PPUISD::FMUL_LEGACY: {
  ...
  case PPUISD::FMED3: {
    ...
    SDValue Res = DAG.getNode(PPUISD::FMED3, SL, VT, Ops, N0->getFlags());
    if (Res.getOpcode() != PPUISD::FMED3)
      return SDValue(); // Op got folded away.
    if (!N0.hasOneUse())
      DAG.ReplaceAllUsesWith(N0, DAG.getNode(ISD::FNEG, SL, VT, Res));
    return Res;
...
SDValue PPUBaseTargetLowering::performFAbsCombine(SDNode *N,
                                                 DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  SDValue N0 = N->getOperand(0);
...
SDValue PPUBaseTargetLowering::performRcpCombine(SDNode *N,
                                                DAGCombinerInfo &DCI) const {
...
SDValue PPUBaseTargetLowering::PerformDAGCombine_compute(SDNode *N,
                                                DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  SDLoc DL(N);
  ...
  case PPUISD::MUL_I24:
  case PPUISD::MUL_U24:
  case PPUISD::MULHI_I24:
  case PPUISD::MULHI_U24: {
    if (SDValue V = simplifyI24(N, DCI))
      return V;
    return SDValue();
  }
  case PPUISD::MUL_LOHI_I24:
  case PPUISD::MUL_LOHI_U24:
  ...
  case PPUISD::BFE_U32: {
    ...
    bool Signed = N->getOpcode() == PPUISD::BFE_I32;
...
SDValue PPUBaseTargetLowering::CreateLiveInRegister(SelectionDAG &DAG,
                                                   const TargetRegisterClass *RC,
                                                   unsigned Reg, EVT VT,
                                                   const SDLoc &SL,
                                                   bool RawReg) const {
