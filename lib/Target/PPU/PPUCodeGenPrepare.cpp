...
//
/// \file
/// This pass does misc. PPU optimizations on IR before instruction
/// selection.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "ppu-codegenprepare"
...
static cl::opt<bool> WidenLoads(
  "ppu-codegenprepare-widen-constant-loads",
  cl::desc("Widen sub-dword constant address space loads in PPUCodeGenPrepare"),
  cl::ReallyHidden,
  cl::init(true));

static cl::opt<bool> UseMul24Intrin(
  "ppu-codegenprepare-mul24",
  cl::desc("Introduce mul24 intrinsics in PPUCodeGenPrepare"),
  cl::ReallyHidden,
  cl::init(true));

class PPUCodeGenPrepare : public FunctionPass,
                          public InstVisitor<PPUCodeGenPrepare, bool> {
...
public:
  ...
  PPUCodeGenPrepare() : FunctionPass(ID) {}
  ...
  StringRef getPassName() const override { return "PPU IR optimizations"; }
...
// unsigned PPUCodeGenPrepare::getBaseElementBitWidth(const Type *T) const {
...
Type *PPUCodeGenPrepare::getI32Ty(IRBuilder<> &B, const Type *T) const {
  assert(needsPromotionToI32(T) && "T does not need promotion to i32");
...
bool PPUCodeGenPrepare::isSigned(const BinaryOperator &I) const {
  return I.getOpcode() == Instruction::AShr ||
      I.getOpcode() == Instruction::SDiv || I.getOpcode() == Instruction::SRem;
}

bool PPUCodeGenPrepare::isSigned(const SelectInst &I) const {
  return isa<ICmpInst>(I.getOperand(0)) ?
      cast<ICmpInst>(I.getOperand(0))->isSigned() : false;
}

bool PPUCodeGenPrepare::needsPromotionToI32(const Type *T) const {
...
bool PPUCodeGenPrepare::canWidenScalarExtLoad(LoadInst &I) const {
  Type *Ty = I.getType();
...
bool PPUCodeGenPrepare::promoteUniformOpToI32(BinaryOperator &I) const {
...
bool PPUCodeGenPrepare::promoteUniformOpToI32(ICmpInst &I) const {
...
bool PPUCodeGenPrepare::promoteUniformOpToI32(SelectInst &I) const {
  assert(needsPromotionToI32(I.getType()) &&
         "I does not need promotion to i32");
...
bool PPUCodeGenPrepare::promoteUniformBitreverseToI32(
...
bool PPUCodeGenPrepare::isI24(Value *V, unsigned ScalarSize) const {
  return ScalarSize >= 24 && // Types less than 24-bit should be treated
                                     // as unsigned 24-bit values.
    numBitsSigned(V, ScalarSize) < 24;
}

bool PPUCodeGenPrepare::isU24(Value *V, unsigned ScalarSize) const {
...
bool PPUCodeGenPrepare::replaceMulWithMul24(BinaryOperator &I) const {
...
bool PPUCodeGenPrepare::visitFDiv(BinaryOperator &FDiv) {
...
Value* PPUCodeGenPrepare::expandDivRem24(IRBuilder<> &Builder,
                                            BinaryOperator &I,
                                            Value *Num, Value *Den,
                                            bool IsDiv, bool IsSigned) const {
...
Value* PPUCodeGenPrepare::expandDivRem32(IRBuilder<> &Builder,
                                            BinaryOperator &I,
                                            Value *Num, Value *Den) const {
...
bool PPUCodeGenPrepare::visitBinaryOperator(BinaryOperator &I) {
...
bool PPUCodeGenPrepare::visitLoadInst(LoadInst &I) {
  if (!WidenLoads)
    return false;
...
bool PPUCodeGenPrepare::visitICmpInst(ICmpInst &I) {
  bool Changed = false;
...
bool PPUCodeGenPrepare::visitSelectInst(SelectInst &I) {
  bool Changed = false;
...
bool PPUCodeGenPrepare::visitIntrinsicInst(IntrinsicInst &I) {
...
bool PPUCodeGenPrepare::visitBitreverseIntrinsicInst(IntrinsicInst &I) {
  bool Changed = false;
...
bool PPUCodeGenPrepare::doInitialization(Module &M) {
  Mod = &M;
  DL = &Mod->getDataLayout();
  return false;
}

bool PPUCodeGenPrepare::runOnFunction(Function &F) {
  if (skipFunction(F))
    return false;
  ...
  const PPUTargetMachine &TM = TPC->getTM<PPUTargetMachine>();
  ST = &TM.getSubtarget<PPUSubtarget>(F);
...
INITIALIZE_PASS_BEGIN(PPUCodeGenPrepare, DEBUG_TYPE,
                      "PPU IR optimizations", false, false)
INITIALIZE_PASS_DEPENDENCY(AssumptionCacheTracker)
INITIALIZE_PASS_DEPENDENCY(LegacyDivergenceAnalysis)
INITIALIZE_PASS_END(PPUCodeGenPrepare, DEBUG_TYPE, "PPU IR optimizations",
                    false, false)

char PPUCodeGenPrepare::ID = 0;

FunctionPass *llvm::createPPUCodeGenPreparePass() {
  return new PPUCodeGenPrepare();
}
