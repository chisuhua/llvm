//===----- PPUMergeBaseOffset.cpp - Optimise address calculations  ------===//
//
...
#define DEBUG_TYPE "ppu-merge-base-offset"
#define PPU_MERGE_BASE_OFFSET_NAME "PPU Merge Base Offset"
namespace {

struct PPUMergeBaseOffsetOpt : public MachineFunctionPass {
  static char ID;
  ...
  PPUMergeBaseOffsetOpt() : MachineFunctionPass(ID) {}
  ...
  StringRef getPassName() const override {
    return PPU_MERGE_BASE_OFFSET_NAME;
  }
...
char PPUMergeBaseOffsetOpt::ID = 0;
INITIALIZE_PASS(PPUMergeBaseOffsetOpt, "ppu-merge-base-offset",
                PPU_MERGE_BASE_OFFSET_NAME, false, false)
...
bool PPUMergeBaseOffsetOpt::detectLuiAddiGlobal(MachineInstr &HiLUI,
                                                  MachineInstr *&LoADDI) {
  if (HiLUI.getOpcode() != PPU::LUI ||
      HiLUI.getOperand(1).getTargetFlags() != PPUII::MO_HI ||
  ...
  if (LoADDI->getOpcode() != PPU::ADDI ||
      LoADDI->getOperand(2).getTargetFlags() != PPUII::MO_LO ||
...
// output from LoADDI.
void PPUMergeBaseOffsetOpt::foldOffset(MachineInstr &HiLUI,
                                         MachineInstr &LoADDI,
...
bool PPUMergeBaseOffsetOpt::matchLargeOffset(MachineInstr &TailAdd,
                                               Register GAReg,
                                               int64_t &Offset) {
  assert((TailAdd.getOpcode() == PPU::ADD) && "Expected ADD instruction!");
  ...
  if (OffsetTail.getOpcode() == PPU::ADDI) {
    ...
    if (AddiImmOp.getTargetFlags() != PPUII::MO_None)
      return false;
    ...
    if (OffsetLui.getOpcode() != PPU::LUI ||
        LuiImmOp.getTargetFlags() != PPUII::MO_None ||
    ...
    return true;
  } else if (OffsetTail.getOpcode() == PPU::LUI) {
...
bool PPUMergeBaseOffsetOpt::detectAndFoldOffset(MachineInstr &HiLUI,
                                                  MachineInstr &LoADDI) {
...
bool PPUMergeBaseOffsetOpt::runOnMachineFunction(MachineFunction &Fn) {
...
FunctionPass *llvm::createPPUMergeBaseOffsetOptPass() {
  return new PPUMergeBaseOffsetOpt();
}
