...
#define DEBUG_TYPE "ppu-annotate-control-flow"
...
class PPUAnnotateControlFlow : public FunctionPass {
  ...
  void initialize(Module &M, const PPUSubtarget &ST);
...
public:
  ...
  PPUAnnotateControlFlow() : FunctionPass(ID) {}

  bool runOnFunction(Function &F) override;

  StringRef getPassName() const override { return "PPU annotate control flow"; }
...
INITIALIZE_PASS_BEGIN(PPUAnnotateControlFlow, DEBUG_TYPE,
                      "Annotate PPU Control Flow", false, false)
INITIALIZE_PASS_DEPENDENCY(DominatorTreeWrapperPass)
INITIALIZE_PASS_DEPENDENCY(LegacyDivergenceAnalysis)
INITIALIZE_PASS_DEPENDENCY(TargetPassConfig)
INITIALIZE_PASS_END(PPUAnnotateControlFlow, DEBUG_TYPE,
                    "Annotate PPU Control Flow", false, false)

char PPUAnnotateControlFlow::ID = 0;

/// Initialize all the types and constants used in the pass
void PPUAnnotateControlFlow::initialize(Module &M, const PPUSubtarget &ST) {
...
void PPUAnnotateControlFlow::eraseIfUnused(PHINode *Phi) {
...
/// Open a new "If" block
void PPUAnnotateControlFlow::openIf(BranchInst *Term) {
  if (isUniform(Term))
    return;
...
void PPUAnnotateControlFlow::insertElse(BranchInst *Term) {
  if (isUniform(Term)) {
    return;
  }
...
Value *PPUAnnotateControlFlow::handleLoopCondition(
...
void PPUAnnotateControlFlow::handleLoop(BranchInst *Term) {
  if (isUniform(Term))
    return;
...
void PPUAnnotateControlFlow::closeControlFlow(BasicBlock *BB) {
  llvm::Loop *L = LI->getLoopFor(BB);
...
bool PPUAnnotateControlFlow::runOnFunction(Function &F) {
  ...
  initialize(*F.getParent(), TM.getSubtarget<PPUSubtarget>(F));
...
FunctionPass *llvm::createPPUAnnotateControlFlowPass() {
  return new PPUAnnotateControlFlow();
}
