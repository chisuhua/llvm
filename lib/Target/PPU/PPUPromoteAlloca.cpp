//===-- PPUPromoteAlloca.cpp - Promote Allocas -------------------------===//
//
...
#define DEBUG_TYPE "ppu-promote-alloca"
...
public:
  ...
  PPUPromoteAlloca() : FunctionPass(ID) {}

  bool doInitialization(Module &M) override;
  bool runOnFunction(Function &F) override;

  StringRef getPassName() const override { return "PPU Promote Alloca"; }
...
char PPUPromoteAlloca::ID = 0;

INITIALIZE_PASS(PPUPromoteAlloca, DEBUG_TYPE,
                "PPU promote alloca to vector or LDS", false, false)

char &llvm::PPUPromoteAllocaID = PPUPromoteAlloca::ID;

bool PPUPromoteAlloca::doInitialization(Module &M) {
  Mod = &M;
...
bool PPUPromoteAlloca::runOnFunction(Function &F) {
  if (skipFunction(F))
    return false;
  ...
  IsPPU = TT.getArch() == Triple::ppu;
  IsPPS = TT.getOS() == Triple::PPS;

  const PPUSubtarget &ST = PPUSubtarget::get(*TM, F);
...
PPUPromoteAlloca::getLocalSizeYZ(IRBuilder<> &Builder) {
  ...
  Function *DispatchPtrFn =
      Intrinsic::getDeclaration(Mod, Intrinsic::ppu_dispatch_ptr);
...
Value *PPUPromoteAlloca::getWorkitemID(IRBuilder<> &Builder, unsigned N) {
  const PPUSubtarget &ST =
      PPUSubtarget::get(*TM, *Builder.GetInsertBlock()->getParent());
  Intrinsic::ID IntrID = Intrinsic::ID::not_intrinsic;

  switch (N) {
  case 0:
    IntrID = Intrinsic::ppu_workitem_id_x;
    break;
  case 1:
    IntrID = Intrinsic::ppu_workitem_id_y;
    break;
  case 2:
    IntrID = Intrinsic::ppu_workitem_id_z;
    break;
  default:
...
static VectorType *arrayTypeToVecType(ArrayType *ArrayTy) {
  return VectorType::get(ArrayTy->getElementType(), ArrayTy->getNumElements());
}
...
bool PPUPromoteAlloca::binaryOpIsDerivedFromSameAlloca(Value *BaseAlloca,
                                                            Value *Val,
                                                            Instruction *Inst,
                                                            int OpIdx0,
                                                            int OpIdx1) const {
...
bool PPUPromoteAlloca::collectUsesWithPtrTypes(
...
bool PPUPromoteAlloca::hasSufficientLocalMem(const Function &F) {
  ...
  unsigned MaxOccupancy = ST.getOccupancyWithLocalMemSize(CurrentLocalMemUsage, F);
  ...
  unsigned MaxSizeWithWaveCount = ST.getMaxLocalMemSizeWithWaveCount(MaxOccupancy, F);
...
bool PPUPromoteAlloca::handleAlloca(AllocaInst &I, bool SufficientLDS) {
  ...
  const PPUSubtarget &ST = PPUSubtarget::get(*TM, ContainingFunction);
...
FunctionPass *llvm::createPPUPromoteAlloca() {
  return new PPUPromoteAlloca();
}
