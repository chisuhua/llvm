...
#define DEBUG_TYPE "ppu-unify-divergent-exit-nodes"

namespace {

class PPUUnifyDivergentExitNodes : public FunctionPass {
public:
  static char ID; // Pass identification, replacement for typeid

  PPUUnifyDivergentExitNodes() : FunctionPass(ID) {
    initializePPUUnifyDivergentExitNodesPass(*PassRegistry::getPassRegistry());
  }
...
char PPUUnifyDivergentExitNodes::ID = 0;

char &llvm::PPUUnifyDivergentExitNodesID = PPUUnifyDivergentExitNodes::ID;

INITIALIZE_PASS_BEGIN(PPUUnifyDivergentExitNodes, DEBUG_TYPE,
                     "Unify divergent function exit nodes", false, false)
INITIALIZE_PASS_DEPENDENCY(PostDominatorTreeWrapperPass)
INITIALIZE_PASS_DEPENDENCY(LegacyDivergenceAnalysis)
INITIALIZE_PASS_END(PPUUnifyDivergentExitNodes, DEBUG_TYPE,
                    "Unify divergent function exit nodes", false, false)

void PPUUnifyDivergentExitNodes::getAnalysisUsage(AnalysisUsage &AU) const{
...
bool PPUUnifyDivergentExitNodes::runOnFunction(Function &F) {
  ...
  if (!UnreachableBlocks.empty()) {
    ...
    if (!ReturningBlocks.empty()) {
      ...
      Function *UnreachableIntrin =
        Intrinsic::getDeclaration(F.getParent(), Intrinsic::ppu_unreachable);
