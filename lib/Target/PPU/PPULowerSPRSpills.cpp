...
class PPULowerSPRSpills : public MachineFunctionPass {
private:
...
public:
  ...
  PPULowerSPRSpills() : MachineFunctionPass(ID) {}
...
char PPULowerSPRSpills::ID = 0;

INITIALIZE_PASS_BEGIN(PPULowerSPRSpills, DEBUG_TYPE,
                      "PPU lower SGPR spill instructions", false, false)
INITIALIZE_PASS_DEPENDENCY(VirtRegMap)
INITIALIZE_PASS_END(PPULowerSPRSpills, DEBUG_TYPE,
                    "PPU lower SGPR spill instructions", false, false)

char &llvm::PPULowerSPRSpillsID = PPULowerSPRSpills::ID;
...
void PPULowerSPRSpills::calculateSaveRestoreBlocks(MachineFunction &MF) {
...
bool PPULowerSPRSpills::spillCalleeSavedRegs(MachineFunction &MF) {
  MachineRegisterInfo &MRI = MF.getRegInfo();
  const Function &F = MF.getFunction();
  const PPUSubtarget &ST = MF.getSubtarget<PPUSubtarget>();
  const PPUFrameLowering *TFI = ST.getFrameLowering();
...
bool PPULowerSPRSpills::runOnMachineFunction(MachineFunction &MF) {
  const PPUSubtarget &ST = MF.getSubtarget<PPUSubtarget>();
  TII = ST.getInstrInfo();
  ...
  if ((TRI->spillSGPRToVGPR() && (HasCSRs || FuncInfo->hasSpilledSGPRs())) ||
    ...
    // are spilled to SPRs, in which case we can eliminate the stack usage.
    //
    ...
    for (MachineBasicBlock &MBB : MF) {
      ...
      for (auto I = MBB.begin(), E = MBB.end(); I != E; I = Next) {
        ...
        if (SpillToVPR && TII->isVGPRSpill(MI)) {
          ...
          // finalization.
          unsigned FIOp = PPU::getNamedOperandIdx(MI.getOpcode(),
                                                     PPU::OpName::vaddr);
          int FI = MI.getOperand(FIOp).getIndex();
          Register VReg =
              TII->getNamedOperand(MI, PPU::OpName::vdata)->getReg();
        ...
        if (FuncInfo->allocateGPRSpillToSPR(MF, FI)) {
          bool Spilled = TRI->eliminateGPRToSPRSpillFrameIndex(MI, FI, nullptr);
          (void)Spilled;
          assert(Spilled && "failed to spill GPR to SPR when allocated");
        }
        */
      }
    }
