...
class PPUAnnotateUniformValues : public FunctionPass,
                       public InstVisitor<PPUAnnotateUniformValues> {
...
public:
  static char ID;
  PPUAnnotateUniformValues() :
    FunctionPass(ID) { }
  bool doInitialization(Module &M) override;
  bool runOnFunction(Function &F) override;
  StringRef getPassName() const override {
    return "PPU Annotate Uniform Values";
  }
...
INITIALIZE_PASS_BEGIN(PPUAnnotateUniformValues, DEBUG_TYPE,
                      "Add PPU uniform metadata", false, false)
INITIALIZE_PASS_DEPENDENCY(LegacyDivergenceAnalysis)
INITIALIZE_PASS_DEPENDENCY(MemoryDependenceWrapperPass)
INITIALIZE_PASS_DEPENDENCY(LoopInfoWrapperPass)
INITIALIZE_PASS_END(PPUAnnotateUniformValues, DEBUG_TYPE,
                    "Add PPU uniform metadata", false, false)

char PPUAnnotateUniformValues::ID = 0;
...
bool PPUAnnotateUniformValues::isClobberedInFunction(LoadInst * Load) {
...
void PPUAnnotateUniformValues::visitBranchInst(BranchInst &I) {
  if (DA->isUniform(&I))
    setUniformMetadata(I.getParent()->getTerminator());
}

void PPUAnnotateUniformValues::visitLoadInst(LoadInst &I) {
...
bool PPUAnnotateUniformValues::doInitialization(Module &M) {
  return false;
}

bool PPUAnnotateUniformValues::runOnFunction(Function &F) {
  if (skipFunction(F))
    return false;
  ...
  isKernelFunc = F.getCallingConv() == CallingConv::AMDGPU_KERNEL; // FIXME schi
...
FunctionPass *
llvm::createPPUAnnotateUniformValues() {
  return new PPUAnnotateUniformValues();
}
