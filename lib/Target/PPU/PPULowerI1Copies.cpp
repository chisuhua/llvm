//===-- PPULowerI1Copies.cpp - Lower I1 Copies -----------------------------===//
//
...
#define DEBUG_TYPE "ppu-i1-copies"
...
class PPULowerI1Copies : public MachineFunctionPass {
public:
  static char ID;
...
public:
  PPULowerI1Copies() : MachineFunctionPass(ID) {
    initializePPULowerI1CopiesPass(*PassRegistry::getPassRegistry());
  }

  bool runOnMachineFunction(MachineFunction &MF) override;

  StringRef getPassName() const override { return "PPU Lower i1 Copies"; }
  ...
  void analyze(MachineBasicBlock &DefBlock,
    ...
    for (MachineBasicBlock *MBB : IncomingBlocks) {
      ...
      for (MachineInstr &MI : MBB->terminators()) {
        if (MI.getOpcode() == PPU::SI_NON_UNIFORM_BRCOND_PSEUDO ||
            MI.getOpcode() == PPU::SI_IF ||
            MI.getOpcode() == PPU::SI_ELSE ||
            MI.getOpcode() == PPU::SI_LOOP) {
          Divergent = true;
          break;
        }
      }
...
INITIALIZE_PASS_BEGIN(PPULowerI1Copies, DEBUG_TYPE, "PPU Lower i1 Copies", false, false)
INITIALIZE_PASS_DEPENDENCY(MachineDominatorTree)
INITIALIZE_PASS_DEPENDENCY(MachinePostDominatorTree)
INITIALIZE_PASS_END(PPULowerI1Copies, DEBUG_TYPE, "PPU Lower i1 Copies", false, false)

char PPULowerI1Copies::ID = 0;

char &llvm::PPULowerI1CopiesID = PPULowerI1Copies::ID;

FunctionPass *llvm::createPPULowerI1CopiesPass() {
  return new PPULowerI1Copies();
}
...
static unsigned insertUndefLaneMask(MachineBasicBlock &MBB) {
  ...
  BuildMI(MBB, MBB.getFirstTerminator(), {}, TII->get(PPU::IMPLICIT_DEF),
          UndefReg);
  return UndefReg;
}
...
void PPULowerI1Copies::lowerCopiesFromI1() {
  ...
  for (MachineBasicBlock &MBB : *MF) {
    for (MachineInstr &MI : MBB) {
      ...
      BuildMI(MBB, MI, DL, TII->get(PPU::V_CNDMASK_B32_e64), DstReg)
          .addImm(0)
          .addImm(0)
          .addImm(0)
          .addImm(-1)
...
void PPULowerI1Copies::lowerPhis() {
  ...
  for (MachineBasicBlock &MBB : *MF) {
    ...
    for (MachineInstr &MI : MBB.phis()) {
      ...
      MRI->setRegClass(DstReg, IsWave32 ? &PPU::SReg_32RegClass
                                        : &PPU::SReg_64RegClass);
      ...
      for (unsigned i = 1; i < MI.getNumOperands(); i += 2) {
        ...
        if (IncomingDef->getOpcode() == PPU::COPY) {
        ...
        } else if (IncomingDef->getOpcode() == PPU::IMPLICIT_DEF) {
          continue;
        } else {
...
void PPULowerI1Copies::lowerCopiesToI1() {
  MachineSSAUpdater SSAUpdater(*MF);
  LoopFinder LF(*DT, *PDT);
  SmallVector<MachineInstr *, 4> DeadCopies;
  ...
  for (MachineBasicBlock &MBB : *MF) {
    ...
    for (MachineInstr &MI : MBB) {
      if (MI.getOpcode() != PPU::IMPLICIT_DEF &&
          MI.getOpcode() != PPU::COPY)
        continue;
      ...
      if (!Register::isVirtualRegister(SrcReg) ||
        ...
        BuildMI(MBB, MI, DL, TII->get(PPU::V_CMP_NE_U32_e64), TmpReg)
            .addReg(SrcReg)
            .addImm(0);
...
bool PPULowerI1Copies::isConstantLaneMask(unsigned Reg, bool &Val) const {
...
MachineBasicBlock::iterator
PPULowerI1Copies::getSaluInsertionAtEnd(MachineBasicBlock &MBB) const {
...
void PPULowerI1Copies::buildMergeLaneMasks(MachineBasicBlock &MBB,
  ...
  if (PrevConstant && CurConstant) {
    if (PrevVal == CurVal) {
      BuildMI(MBB, I, DL, TII->get(PPU::COPY), DstReg).addReg(CurReg);
    } else if (CurVal) {
      BuildMI(MBB, I, DL, TII->get(PPU::COPY), DstReg).addReg(ExecReg);
    } else {
  ...
  if (PrevConstant && !PrevVal) {
    BuildMI(MBB, I, DL, TII->get(PPU::COPY), DstReg)
        .addReg(CurMaskedReg);
