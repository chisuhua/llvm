...
#include "PPUInstructionSelector.h"
...
static bool isSCC(Register Reg, const MachineRegisterInfo &MRI) {
  ...
  if (RC) {
    ...
    if (RC->getID() != PPU::SReg_32RegClassID)
      return false;
  ...
  return RB->getID() == PPU::SCCRegBankID;
}

bool PPUInstructionSelector::isVCC(Register Reg,
  ...
  const RegisterBank *RB = RegClassOrBank.get<const RegisterBank *>();
  return RB->getID() == PPU::VCCRegBankID;
}

bool PPUInstructionSelector::selectCOPY(MachineInstr &I) const {
  ...
  if (isVCC(DstReg, MRI)) {
    ...
    if (!isVCC(SrcReg, MRI)) {
      ...
      BuildMI(*BB, &I, DL, TII.get(PPU::V_CMP_NE_U32_e64), DstReg)
        .addImm(0)
        .addReg(SrcReg);
...
bool PPUInstructionSelector::selectPHI(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
...
MachineOperand
PPUInstructionSelector::getSubOperand64(MachineOperand &MO,
  ...
  if (MO.isReg()) {
    ...
    BuildMI(*BB, MI, MI->getDebugLoc(), TII.get(PPU::COPY), DstReg)
...
static unsigned getLogicalBitOpcode(unsigned Opc, bool Is64) {
  switch (Opc) {
  case PPU::G_AND:
    return Is64 ? PPU::S_AND_B64 : PPU::S_AND_B32;
  case PPU::G_OR:
    return Is64 ? PPU::S_OR_B64 : PPU::S_OR_B32;
  case PPU::G_XOR:
    return Is64 ? PPU::S_XOR_B64 : PPU::S_XOR_B32;
  default:
    llvm_unreachable("not a bit op");
  }
}

bool PPUInstructionSelector::selectG_AND_OR_XOR(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
  MachineOperand &Dst = I.getOperand(0);
  ...
  if (DstRB->getID() == PPU::VCCRegBankID) {
  ...
  // the result?
  if (DstRB->getID() == PPU::SPRRegBankID) {
...
bool PPUInstructionSelector::selectG_ADD_SUB(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
  ...
  unsigned Size = RBI.getSizeInBits(DstReg, MRI, TRI);
  const RegisterBank *DstRB = RBI.getRegBank(DstReg, MRI, TRI);
  const bool IsSALU = DstRB->getID() == PPU::SPRRegBankID;
  const bool Sub = I.getOpcode() == TargetOpcode::G_SUB;

  if (Size == 32) {
    if (IsSALU) {
      const unsigned Opc = Sub ? PPU::S_SUB_U32 : PPU::S_ADD_U32;
      MachineInstr *Add =
    ...
    if (STI.hasAddNoCarry()) {
      const unsigned Opc = Sub ? PPU::V_SUB_U32_e64 : PPU::V_ADD_U32_e64;
      I.setDesc(TII.get(Opc));
      I.addOperand(*MF, MachineOperand::CreateImm(0));
      I.addOperand(*MF, MachineOperand::CreateReg(PPU::TMSK, false, true));
      return constrainSelectedInstRegOperands(I, TII, TRI, RBI);
    }

    const unsigned Opc = Sub ? PPU::V_SUB_I32_e64 : PPU::V_ADD_I32_e64;
  ...
  const TargetRegisterClass &RC
    = IsSALU ? PPU::SReg_64RegClass : PPU::VReg_64RegClass;
  const TargetRegisterClass &HalfRC
    = IsSALU ? PPU::SReg_32RegClass : PPU::VPR_32RegClass;

  MachineOperand Lo1(getSubOperand64(I.getOperand(1), HalfRC, PPU::sub0));
  MachineOperand Lo2(getSubOperand64(I.getOperand(2), HalfRC, PPU::sub0));
  MachineOperand Hi1(getSubOperand64(I.getOperand(1), HalfRC, PPU::sub1));
  MachineOperand Hi2(getSubOperand64(I.getOperand(2), HalfRC, PPU::sub1));

  Register DstLo = MRI.createVirtualRegister(&HalfRC);
  Register DstHi = MRI.createVirtualRegister(&HalfRC);

  if (IsSALU) {
    BuildMI(*BB, &I, DL, TII.get(PPU::S_ADD_U32), DstLo)
      .add(Lo1)
      .add(Lo2);
    BuildMI(*BB, &I, DL, TII.get(PPU::S_ADDC_U32), DstHi)
      .add(Hi1)
      .add(Hi2);
  } else {
    const TargetRegisterClass *CarryRC = TRI.getWaveMaskRegClass();
    Register CarryReg = MRI.createVirtualRegister(CarryRC);
    BuildMI(*BB, &I, DL, TII.get(PPU::V_ADD_I32_e64), DstLo)
      .addDef(CarryReg)
      .add(Lo1)
      .add(Lo2)
      .addImm(0);
    MachineInstr *Addc = BuildMI(*BB, &I, DL, TII.get(PPU::V_ADDC_U32_e64), DstHi)
  ...
  BuildMI(*BB, &I, DL, TII.get(PPU::REG_SEQUENCE), DstReg)
    .addReg(DstLo)
    .addImm(PPU::sub0)
    .addReg(DstHi)
    .addImm(PPU::sub1);
...
bool PPUInstructionSelector::selectG_EXTRACT(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
...
bool PPUInstructionSelector::selectG_MERGE_VALUES(MachineInstr &MI) const {
...
bool PPUInstructionSelector::selectG_UNMERGE_VALUES(MachineInstr &MI) const {
  MachineBasicBlock *BB = MI.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
...
bool PPUInstructionSelector::selectG_GEP(MachineInstr &I) const {
  return selectG_ADD_SUB(I);
}

bool PPUInstructionSelector::selectG_IMPLICIT_DEF(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
...
bool PPUInstructionSelector::selectG_INSERT(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
...
bool PPUInstructionSelector::selectG_INTRINSIC(MachineInstr &I) const {
  ...
  case Intrinsic::ppu_if_break: {
    ...
    BuildMI(*BB, &I, I.getDebugLoc(), TII.get(PPU::SI_IF_BREAK))
      .add(I.getOperand(0))
      .add(I.getOperand(2))
      .add(I.getOperand(3));
...
static int getV_CMPOpcode(CmpInst::Predicate P, unsigned Size) {
  ...
  case CmpInst::ICMP_NE:
    return Size == 32 ? PPU::V_CMP_NE_U32_e64 : PPU::V_CMP_NE_U64_e64;
  case CmpInst::ICMP_EQ:
    return Size == 32 ? PPU::V_CMP_EQ_U32_e64 : PPU::V_CMP_EQ_U64_e64;
  case CmpInst::ICMP_SGT:
    return Size == 32 ? PPU::V_CMP_GT_I32_e64 : PPU::V_CMP_GT_I64_e64;
  case CmpInst::ICMP_SGE:
    return Size == 32 ? PPU::V_CMP_GE_I32_e64 : PPU::V_CMP_GE_I64_e64;
  case CmpInst::ICMP_SLT:
    return Size == 32 ? PPU::V_CMP_LT_I32_e64 : PPU::V_CMP_LT_I64_e64;
  case CmpInst::ICMP_SLE:
    return Size == 32 ? PPU::V_CMP_LE_I32_e64 : PPU::V_CMP_LE_I64_e64;
  case CmpInst::ICMP_UGT:
    return Size == 32 ? PPU::V_CMP_GT_U32_e64 : PPU::V_CMP_GT_U64_e64;
  case CmpInst::ICMP_UGE:
    return Size == 32 ? PPU::V_CMP_GE_U32_e64 : PPU::V_CMP_GE_U64_e64;
  case CmpInst::ICMP_ULT:
    return Size == 32 ? PPU::V_CMP_LT_U32_e64 : PPU::V_CMP_LT_U64_e64;
  case CmpInst::ICMP_ULE:
    return Size == 32 ? PPU::V_CMP_LE_U32_e64 : PPU::V_CMP_LE_U64_e64;
  }
}

int PPUInstructionSelector::getS_CMPOpcode(CmpInst::Predicate P,
                                              unsigned Size) const {
  if (Size == 64) {
    ...
    switch (P) {
    case CmpInst::ICMP_NE:
      return PPU::S_CMP_LG_U64;
    case CmpInst::ICMP_EQ:
      return PPU::S_CMP_EQ_U64;
    default:
      return -1;
    }
  }
  ...
  switch (P) {
  case CmpInst::ICMP_NE:
    return PPU::S_CMP_LG_U32;
  case CmpInst::ICMP_EQ:
    return PPU::S_CMP_EQ_U32;
  case CmpInst::ICMP_SGT:
    return PPU::S_CMP_GT_I32;
  case CmpInst::ICMP_SGE:
    return PPU::S_CMP_GE_I32;
  case CmpInst::ICMP_SLT:
    return PPU::S_CMP_LT_I32;
  case CmpInst::ICMP_SLE:
    return PPU::S_CMP_LE_I32;
  case CmpInst::ICMP_UGT:
    return PPU::S_CMP_GT_U32;
  case CmpInst::ICMP_UGE:
    return PPU::S_CMP_GE_U32;
  case CmpInst::ICMP_ULT:
    return PPU::S_CMP_LT_U32;
  case CmpInst::ICMP_ULE:
    return PPU::S_CMP_LE_U32;
  default:
    llvm_unreachable("Unknown condition code!");
  }
}

bool PPUInstructionSelector::selectG_ICMP(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
  const DebugLoc &DL = I.getDebugLoc();
  ...
  if (isSCC(CCReg, MRI)) {
    ...
    MachineInstr *ICmp = BuildMI(*BB, &I, DL, TII.get(Opcode))
            .add(I.getOperand(2))
            .add(I.getOperand(3));
    BuildMI(*BB, &I, DL, TII.get(PPU::COPY), CCReg)
      .addReg(PPU::SCC);
    bool Ret =
        constrainSelectedInstRegOperands(*ICmp, TII, TRI, RBI) &&
        RBI.constrainGenericRegister(CCReg, PPU::SReg_32RegClass, MRI);
    I.eraseFromParent();
    return Ret;
  }
...
buildEXP(const TargetInstrInfo &TII, MachineInstr *Insert, unsigned Tgt,
  ...
  unsigned Opcode = Done ? PPU::EXP_DONE : PPU::EXP;
...
bool PPUInstructionSelector::selectG_INTRINSIC_W_SIDE_EFFECTS(
...
bool PPUInstructionSelector::selectG_SELECT(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
  const DebugLoc &DL = I.getDebugLoc();
  ...
  if (isSCC(CCReg, MRI)) {
    unsigned SelectOpcode = Size == 64 ? PPU::S_CSELECT_B64 :
                                         PPU::S_CSELECT_B32;
    MachineInstr *CopySCC = BuildMI(*BB, &I, DL, TII.get(PPU::COPY), PPU::SCC)
            .addReg(CCReg);
  ...
  MachineInstr *Select =
      BuildMI(*BB, &I, DL, TII.get(PPU::V_CNDMASK_B32_e64), DstReg)
              .addImm(0)
              .add(I.getOperand(3))
              .addImm(0)
              .add(I.getOperand(2))
...
bool PPUInstructionSelector::selectG_STORE(MachineInstr &I) const {
  initM0(I);
  return selectImpl(I, *CoverageInfo);
}
...
static int sizeToSubRegIndex(unsigned Size) {
  ...
  case 32:
    ...
    /*
  case 64:
    return PPU::sub0_sub1;
  case 96:
    return PPU::sub0_sub1_sub2;
  case 128:
    return PPU::sub0_sub1_sub2_sub3;
  case 256:
    return PPU::sub0_sub1_sub2_sub3_sub4_sub5_sub6_sub7;
    */
  default:
    if (Size < 32)
      return PPU::sub0;
...
bool PPUInstructionSelector::selectG_TRUNC(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
...
bool PPUInstructionSelector::selectG_SZA_EXT(MachineInstr &I) const {
  bool Signed = I.getOpcode() == PPU::G_SEXT;
  const DebugLoc &DL = I.getDebugLoc();
  ...
  if (SrcBank->getID() == PPU::SCCRegBankID) {
    if (SrcTy != S1 || DstSize > 64) // Invalid
      return false;

    unsigned Opcode =
        DstSize > 32 ? PPU::S_CSELECT_B64 : PPU::S_CSELECT_B32;
    const TargetRegisterClass *DstRC =
        DstSize > 32 ? &PPU::SReg_64RegClass : &PPU::SReg_32RegClass;

    // FIXME: Create an extra copy to avoid incorrectly constraining the result
    // of the scc producer.
    Register TmpReg = MRI.createVirtualRegister(&PPU::SReg_32RegClass);
    BuildMI(MBB, I, DL, TII.get(PPU::COPY), TmpReg)
      .addReg(SrcReg);
    BuildMI(MBB, I, DL, TII.get(PPU::COPY), PPU::SCC)
      .addReg(TmpReg);
  ...
  if (SrcBank->getID() == PPU::VCCRegBankID && DstSize <= 32) {
    if (SrcTy != S1) // Invalid
      return false;

    MachineInstr *ExtI =
      BuildMI(MBB, I, DL, TII.get(PPU::V_CNDMASK_B32_e64), DstReg)
  ...
  if (I.getOpcode() == PPU::G_ANYEXT)
    return selectCOPY(I);

  if (SrcBank->getID() == PPU::VPRRegBankID && DstSize <= 32) {
    ...
    if (!Signed && shouldUseAndMask(SrcSize, Mask)) {
      MachineInstr *ExtI =
      BuildMI(MBB, I, DL, TII.get(PPU::V_AND_B32_e32), DstReg)
        .addImm(Mask)
        .addReg(SrcReg);
      I.eraseFromParent();
      return constrainSelectedInstRegOperands(*ExtI, TII, TRI, RBI);
    ...
    const unsigned BFE = Signed ? PPU::V_BFE_I32 : PPU::V_BFE_U32;
    MachineInstr *ExtI =
  ...
  if (SrcBank->getID() == PPU::SPRRegBankID && DstSize <= 64) {
    if (!RBI.constrainGenericRegister(SrcReg, PPU::SReg_32RegClass, MRI))
      return false;

    if (Signed && DstSize == 32 && (SrcSize == 8 || SrcSize == 16)) {
      const unsigned SextOpc = SrcSize == 8 ?
        PPU::S_SEXT_I32_I8 : PPU::S_SEXT_I32_I16;
      BuildMI(MBB, I, DL, TII.get(SextOpc), DstReg)
        .addReg(SrcReg);
      I.eraseFromParent();
      return RBI.constrainGenericRegister(DstReg, PPU::SReg_32RegClass, MRI);
    }

    const unsigned BFE64 = Signed ? PPU::S_BFE_I64 : PPU::S_BFE_U64;
    const unsigned BFE32 = Signed ? PPU::S_BFE_I32 : PPU::S_BFE_U32;
    ...
    if (DstSize > 32 && SrcSize <= 32) {
      ...
      Register ExtReg = MRI.createVirtualRegister(&PPU::SReg_64RegClass);
      Register UndefReg = MRI.createVirtualRegister(&PPU::SReg_32RegClass);
      BuildMI(MBB, I, DL, TII.get(PPU::IMPLICIT_DEF), UndefReg);
      BuildMI(MBB, I, DL, TII.get(PPU::REG_SEQUENCE), ExtReg)
        .addReg(SrcReg)
        .addImm(PPU::sub0)
        .addReg(UndefReg)
        .addImm(PPU::sub1);
      ...
      I.eraseFromParent();
      return RBI.constrainGenericRegister(DstReg, PPU::SReg_64RegClass, MRI);
    }

    unsigned Mask;
    if (!Signed && shouldUseAndMask(SrcSize, Mask)) {
      BuildMI(MBB, I, DL, TII.get(PPU::S_AND_B32), DstReg)
        .addReg(SrcReg)
        .addImm(Mask);
    } else {
...
bool PPUInstructionSelector::selectG_CONSTANT(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
  ...
  if (RB) {
    IsSgpr = RB->getID() == PPU::SPRRegBankID;
  ...
  unsigned Opcode = IsSgpr ? PPU::S_MOV_B32 : PPU::V_MOV_B32_e32;
  if (Size == 32) {
  ...
  DebugLoc DL = I.getDebugLoc();
  const TargetRegisterClass *RC = IsSgpr ? &PPU::SReg_32RegClass :
                                           &PPU::VPR_32RegClass;
...
void PPUInstructionSelector::getAddrModeInfo(const MachineInstr &Load,
  ...
  for (unsigned i = 1, e = 3; i < e; ++i) {
    ...
    if (OpBank->getID() == PPU::SPRRegBankID)
...
bool PPUInstructionSelector::isInstrUniform(const MachineInstr &MI) const {
...
bool PPUInstructionSelector::hasVgprParts(ArrayRef<GEPInfo> AddrInfo) const {
...
void PPUInstructionSelector::initM0(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
  ...
  if ((AS == AMDGPUAS::LOCAL_ADDRESS || AS == AMDGPUAS::REGION_ADDRESS) &&
    ...
    BuildMI(*BB, &I, I.getDebugLoc(), TII.get(PPU::S_MOV_B32), PPU::M0)
      .addImm(-1);
  }
}

bool PPUInstructionSelector::selectG_LOAD_ATOMICRMW(MachineInstr &I) const {
  initM0(I);
  return selectImpl(I, *CoverageInfo);
}

bool PPUInstructionSelector::selectG_BRCOND(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
  ...
  if (isSCC(CondReg, MRI)) {
    CondPhysReg = PPU::SCC;
    BrOpcode = PPU::S_CBRANCH_SCC1;
    ConstrainRC = &PPU::SReg_32RegClass;
  ...
  BuildMI(*BB, &I, DL, TII.get(PPU::COPY), CondPhysReg)
    .addReg(CondReg);
...
bool PPUInstructionSelector::selectG_FRAME_INDEX(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
  ...
  Register DstReg = I.getOperand(0).getReg();
  const RegisterBank *DstRB = RBI.getRegBank(DstReg, MRI, TRI);
  const bool IsVGPR = DstRB->getID() == PPU::VPRRegBankID;
  I.setDesc(TII.get(IsVGPR ? PPU::V_MOV_B32_e32 : PPU::S_MOV_B32));
  if (IsVGPR)
    I.addOperand(*MF, MachineOperand::CreateReg(PPU::TMSK, false, true));

  return RBI.constrainGenericRegister(
    DstReg, IsVGPR ? PPU::VPR_32RegClass : PPU::SReg_32RegClass, MRI);
}
...
bool PPUInstructionSelector::select(MachineInstr &I) {
  ...
  case TargetOpcode::G_FENCE:
    ...
    I.setDesc(TII.get(PPU::ATOMIC_FENCE));
    return true;
  }
  return false;
}
...
PPUInstructionSelector::selectVCSRC(MachineOperand &Root) const {
  return {{
...
PPUInstructionSelector::selectVOP3ModsImpl(
  Register Src, const MachineRegisterInfo &MRI) const {
  unsigned Mods = 0;
  MachineInstr *MI = MRI.getVRegDef(Src);

  if (MI && MI->getOpcode() == PPU::G_FNEG) {
    Src = MI->getOperand(1).getReg();
    Mods |= PPUSrcMods::NEG;
    MI = MRI.getVRegDef(Src);
  }

  if (MI && MI->getOpcode() == PPU::G_FABS) {
    Src = MI->getOperand(1).getReg();
    Mods |= PPUSrcMods::ABS;
  }
...
InstructionSelector::ComplexRendererFns
PPUInstructionSelector::selectVSRC0(MachineOperand &Root) const {
  return {{
      [=](MachineInstrBuilder &MIB) { MIB.add(Root); }
  }};
}
...
InstructionSelector::ComplexRendererFns
PPUInstructionSelector::selectVOP3Mods0(MachineOperand &Root) const {
...
InstructionSelector::ComplexRendererFns
PPUInstructionSelector::selectVOP3OMods(MachineOperand &Root) const {
  return {{
      [=](MachineInstrBuilder &MIB) { MIB.add(Root); },
      [=](MachineInstrBuilder &MIB) { MIB.addImm(0); }, // clamp
      [=](MachineInstrBuilder &MIB) { MIB.addImm(0); }  // omod
...
InstructionSelector::ComplexRendererFns
PPUInstructionSelector::selectVOP3Mods(MachineOperand &Root) const {
  MachineRegisterInfo &MRI
    = Root.getParent()->getParent()->getParent()->getRegInfo();
...
InstructionSelector::ComplexRendererFns
PPUInstructionSelector::selectSmrdImm(MachineOperand &Root) const {
  MachineRegisterInfo &MRI =
      Root.getParent()->getParent()->getParent()->getRegInfo();
  ...
  if (!PPU::isLegalSMRDImmOffset(STI, GEPInfo.Imm))
    return None;

  unsigned PtrReg = GEPInfo.SgprParts[0];
  int64_t EncodedImm = PPU::getSMRDEncodedOffset(STI, GEPInfo.Imm);
  return {{
...
InstructionSelector::ComplexRendererFns
PPUInstructionSelector::selectSmrdImm32(MachineOperand &Root) const {
  MachineRegisterInfo &MRI =
      Root.getParent()->getParent()->getParent()->getRegInfo();
...
InstructionSelector::ComplexRendererFns
PPUInstructionSelector::selectSmrdSgpr(MachineOperand &Root) const {
  ...
  unsigned PtrReg = GEPInfo.SgprParts[0];
  Register OffsetReg = MRI.createVirtualRegister(&PPU::SReg_32RegClass);
  BuildMI(*MBB, MI, MI->getDebugLoc(), TII.get(PPU::S_MOV_B32), OffsetReg)
          .addImm(GEPInfo.Imm);
  return {{
    [=](MachineInstrBuilder &MIB) { MIB.addReg(PtrReg); },
...
InstructionSelector::ComplexRendererFns
PPUInstructionSelector::selectFlatOffsetImpl(MachineOperand &Root) const {
  MachineInstr *MI = Root.getParent();
  MachineBasicBlock *MBB = MI->getParent();
  MachineRegisterInfo &MRI = MBB->getParent()->getRegInfo();
  ...
  if (!OpDef || OpDef->getOpcode() != PPU::G_GEP)
    return Default;
...
InstructionSelector::ComplexRendererFns
PPUInstructionSelector::selectFlatOffset(MachineOperand &Root) const {
  return selectFlatOffsetImpl<false>(Root);
}

InstructionSelector::ComplexRendererFns
PPUInstructionSelector::selectFlatOffsetSigned(MachineOperand &Root) const {
...
InstructionSelector::ComplexRendererFns
PPUInstructionSelector::selectMUBUFScratchOffen(MachineOperand &Root) const {
  MachineInstr *MI = Root.getParent();
  MachineBasicBlock *MBB = MI->getParent();
  MachineFunction *MF = MBB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
  ...
  if (mi_match(Root.getReg(), MRI, m_ICst(Offset))) {
    Register HighBits = MRI.createVirtualRegister(&PPU::VPR_32RegClass);

    // TODO: Should this be inside the render function? The iterator seems to
    // move.
    BuildMI(*MBB, MI, MI->getDebugLoc(), TII.get(PPU::V_MOV_B32_e32),
            HighBits)
      .addImm(Offset & ~4095);
  ...
  if (const MachineInstr *RootDef = MRI.getVRegDef(Root.getReg())) {
    if (isBaseWithConstantOffset(Root, MRI)) {
      ...
      if (LHSDef && RHSDef) {
        ...
        if (PPUInstrInfo::isLegalMUBUFImmOffset(PossibleOffset) &&
            (!STI.privateMemoryResourceIsRangeChecked() ||
             signBitIsZero(LHS, MRI))) {
          if (LHSDef->getOpcode() == PPU::G_FRAME_INDEX)
    ...
    } else if (RootDef->getOpcode() == PPU::G_FRAME_INDEX) {
...
bool PPUInstructionSelector::isDSOffsetLegal(const MachineRegisterInfo &MRI,
...
InstructionSelector::ComplexRendererFns
PPUInstructionSelector::selectMUBUFScratchOffset(
  ...
  int64_t Offset = 0;
  if (!mi_match(Root.getReg(), MRI, m_ICst(Offset)) ||
      !PPUInstrInfo::isLegalMUBUFImmOffset(Offset))
    return {};
...
InstructionSelector::ComplexRendererFns
PPUInstructionSelector::selectDS1Addr1Offset(MachineOperand &Root) const {
  MachineInstr *MI = Root.getParent();
  MachineBasicBlock *MBB = MI->getParent();
  MachineRegisterInfo &MRI = MBB->getParent()->getRegInfo();
  ...
  } else if (RootDef->getOpcode() == PPU::G_SUB) {
