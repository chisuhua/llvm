//===-- PPUAlwaysInlinePass.cpp - Promote Allocas ----------------------===//
//
...
static cl::opt<bool> StressCalls("ppu-stress-function-calls", cl::Hidden,
                                 cl::desc("Force all functions to be noinline"),
                                 cl::init(false));

class PPUAlwaysInline : public ModulePass {
  bool GlobalOpt;
...
public:
  ...
  PPUAlwaysInline(bool GlobalOpt = false)
...
INITIALIZE_PASS(PPUAlwaysInline, "ppu-always-inline",
                "PPU Inline All Functions", false, false)

char PPUAlwaysInline::ID = 0;

void PPUAlwaysInline::recursivelyVisitUsers(
  ...
  while (!Stack.empty()) {
    ...
    if (Instruction *I = dyn_cast<Instruction>(U)) {
      ...
      if (!PPU::isEntryFunctionCC(F->getCallingConv())) {
...
bool PPUAlwaysInline::runOnModule(Module &M) {
  ...
  if (!PPUTargetMachine::EnableFunctionCalls || StressCalls) {
    auto IncompatAttr =
  ...
  // TODO verfiy can it override Inline attr
...
ModulePass *llvm::createPPUAlwaysInlinePass(bool GlobalOpt) {
  return new PPUAlwaysInline(GlobalOpt);
}
