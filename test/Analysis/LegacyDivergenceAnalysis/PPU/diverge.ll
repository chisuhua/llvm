...
define amdgpu_cs i32 @no_diverge(i32 inreg %n, i32 inreg %a, i32 inreg %b) {
; CHECK-LABEL: Printing analysis 'Legacy Divergence Analysis' for function 'no_diverge'
entry:
  %tid = call i32 @llvm.ppu.nvvm.tid.x()
...
merge:
  ...
  ;store volatile i32 %c, i32 addrspace(1)* undef
  ret i32 %c
  ;ret void
}
...
define amdgpu_cs i32 @sync(i32 inreg %a, i32 inreg %b) {
; CHECK-LABEL: Printing analysis 'Legacy Divergence Analysis' for function 'sync'
bb1:
  %tid = call i32 @llvm.ppu.nvvm.tid.y()
...
; return c;
define amdgpu_cs i32 @mixed(i32 inreg %n, i32 inreg %a, i32 inreg %b) {
; CHECK-LABEL: Printing analysis 'Legacy Divergence Analysis' for function 'mixed'
bb1:
  %tid = call i32 @llvm.ppu.nvvm.tid.z()
  %cond = icmp slt i32 %tid, 5
...
entry:
  %laneid = call i32 @llvm.ppu.nvvm.laneid()
  br label %loop
loop:
...
define amdgpu_cs i32 @unstructured_loop(i1 inreg %entry_cond) {
...
define amdgpu_cs i32 @sync_no_loop(i32 inreg %arg) {
entry:
  %0 = add i32 %arg, 1
  %tid = call i32 @llvm.ppu.nvvm.tid.x()
...
;declare i32 @llvm.ppu.nvvm.tid.x()
;declare i32 @llvm.ppu.nvvm.tid.y()
;declare i32 @llvm.ppu.nvvm.tid.z()
;declare i32 @llvm.ppu.nvvm.laneid()

;!nvvm.annotations = !{!0, !1, !2, !3, !4, !5}
;!0 = !{void (i32, i32, i32)* @no_diverge, !"kernel", i32 1}
;!1 = !{void (i32, i32)* @sync, !"kernel", i32 1}
;!2 = !{void (i32, i32, i32)* @mixed, !"kernel", i32 1}
;!3 = !{void ()* @loop, !"kernel", i32 1}
;!4 = !{void (i1)* @unstructured_loop, !"kernel", i32 1}
;!5 = !{void (i32)* @sync_no_loop, !"kernel", i32 1}
