# RUN: llvm-mc -triple ppu -mattr=+c,+f -show-encoding < %s \
# RUN: | FileCheck -check-prefixes=CHECK,CHECK-ALIAS %s
# RUN: llvm-mc -triple ppu -mattr=+c,+f -show-encoding \
# RUN: -ppu-no-aliases <%s | FileCheck -check-prefixes=CHECK,CHECK-INST %s
# RUN: llvm-mc -triple ppu -mattr=+c,+f -filetype=obj < %s \
# RUN: | llvm-objdump  -triple ppu -mattr=+c,+f -d - \
# RUN: | FileCheck -check-prefixes=CHECK-BYTES,CHECK-ALIAS %s
# RUN: llvm-mc -triple ppu -mattr=+c,+f -filetype=obj < %s \
# RUN: | llvm-objdump  -triple ppu -mattr=+c,+f -d -ppu-no-aliases - \
