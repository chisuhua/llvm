; RUN: llc -mcpu=ppu -mtriple=ppu-- -mattr=+t,+r -mattr=-flat-for-global -show-mc-encoding --verify-machineinstrs < %s | FileCheck -enable-var-scope -check-prefixes=BI,FUNC %s
...
define amdgpu_kernel void @v_add_i32(i32 addrspace(1)* %out, i32 addrspace(1)* %in) #0 {
  ...
  %result = add i32 %a, %b
  %gep_out = getelementptr inbounds i32, i32 addrspace(1)* %out, i32 %tid
  store i32 %result, i32 addrspace(1)* %gep_out
  ret void
}
